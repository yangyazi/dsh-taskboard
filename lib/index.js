...
export async function apply(ctx, config = {}) {
	...
	/** 从会话事件折叠标题 + 取最新消息预览 + 判断是否运行中。
	 * running：末尾存在未闭合的 turn/start（无对应 turn/end）→ 正在生成；否则空闲/已完成。
	 * 返回 {title, preview, running}。 */
