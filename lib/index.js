...
export async function apply(ctx, config = {}) {
	...
	async function firstPromptTitle(sessionId) {
		const id = normalizeSessionId(sessionId);
				...
				for (let k = 0; k < starts.length && k < 400; k++) {
					...
					for (const line of text.split("\n")) {
						...
						try {
							...
							if (cleaned) { title = cleaned.slice(0, MAX_TITLE_CHARS); break; }
						} catch { /* partial frame line */ }
					}
					if (title) break;
				}
			}
		} catch { /* file unreadable — fall through to null */ }
		promptTitleCache.set(id, title);
		return title;
	}
	...
	async function sessionIndex() {
		...
		try {
			...
			for (const header of headers ?? []) {
				...
				const title = meta?.title ?? (await firstPromptTitle(id)) ?? (cwd ? path.basename(cwd) : id);
				out.push({
					id,
					title,
					preview: null,
