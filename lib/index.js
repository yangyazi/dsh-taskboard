...
export async function apply(ctx, config = {}) {
	...
	function taskView(t) {
		const { pendingInjects, armNextSession, ...rest } = t;
		return rest;
	}
	...
	/** 投影缓存里以「这是任务「…」开头且被截断的种子标题不可用（截断后无法折叠），
	 * 这类标题让位给会话日志解析出的整洁标题。 */
	const SEED_TITLE_PREFIX = /^这是任务「/;
	function usableMetaTitle(title) {
		return typeof title === "string" && title.trim() !== "" && !SEED_TITLE_PREFIX.test(title.trim()) ? title : null;
	}
	...
	*/
	async function readHeadInfo(sessionId) {
		...
		// 会话开头的 user/message 里混着 Harness 自己注入的内容
		// （审批策略变更 source.kind="plugin"、system-prompt 快照、skill 目录等）。
		// 拿这些当标题会得到 "The approval policy changed from ask to never…" 之类，
		// 所以标题优先取**人类**消息（source.kind === "user"），退而求其次才用其它。
		// 同理"是否有内容"只认人类消息：只有系统注入的会话等价于空壳。
				...
				for (let k = 0; k < starts.length && k < 400; k++) {
					...
					for (const line of text.split("\n")) {
						...
						try {
							...
							// 同一轮里 Harness 会把同一条消息记录多次（rpcId 相同），跳过重复
							const key = ev.data?.source?.rpcId ?? ev.data?.id;
							if (typeof key === "string") {
								if (seenRpc.has(key)) continue;
								seenRpc.add(key);
							}
							const human = ev.data?.source?.kind === "user";
							if (human) info.hasUserMessage = true;
							const blocks = ev.data?.content ?? [];
							const raw = blocks.filter((b) => b?.type === "text").map((b) => b.text).join(" ");
							const cleaned = cleanText(raw);
							if (!cleaned) continue;
							const candidate = tidyTitle(cleaned).slice(0, MAX_TITLE_CHARS);
							if (human) { info.title = candidate; break; }
							if (fallbackTitle === null) fallbackTitle = candidate;
		...
		if (info.title === null) info.title = fallbackTitle;
	...
	async function sessionIndex() {
		...
		try {
			for (const session of ctx.sessions?.list() ?? []) {
				...
				// 活会话标题来自宿主投影：任务种子会话会被投影成被截断的种子长文
				// （"这是任务「X」的一个新对话。开始前请先阅读…"）。这里同样折叠/让位，
				// 让看板和「最新对话」显示干净的「任务「X」的新对话」。
				const metaTitle = usableMetaTitle(meta?.title);
				let title = usableMetaTitle(liveTitle) === null ? null : tidyTitle(usableMetaTitle(liveTitle));
				if (title === null) title = metaTitle ?? (await readHeadInfo(session.id)).title
					?? (cwd ? `${path.basename(cwd)} · 无标题` : session.id);
		...
		for (const [id, meta] of byId) {
			...
			const metaTitle = usableMetaTitle(meta.title);
			const info = metaTitle === null ? await readHeadInfo(id) : { title: null, hasUserMessage: true };
			const title = metaTitle ?? info.title ?? (meta.cwd ? `${path.basename(meta.cwd)} · 无标题` : id);
			out.push({
				id,
				title,
				preview: null,
		...
		try {
			...
			for (const header of headers ?? []) {
				...
				const title = usableMetaTitle(meta?.title) ?? info.title
	...
	async function buildTaskDigest(task) {
		...
		if ((task.sessionIds || []).length) {
			// 每行都带 session id：标题只是给人看的，agent 要能拿 id 反查/打开
			// （/taskboard/api/task-by-session?sessionId=… 或宿主会话列表）。
			lines.push(`\n【已关联的历史会话】（括号内是 session id，可用它反查/打开；同一任务多个新对话标题相同，用最近活跃时间区分）`);
			const fmtAt = (ts) => ts ? new Date(ts).toLocaleString("zh-CN", { month: "numeric", day: "numeric", hour: "2-digit", minute: "2-digit", hour12: false }) : "";
			try {
				const rows = new Map((await sessionIndex()).map((x) => [x.id, x]));
				for (const sid of task.sessionIds) {
					const row = rows.get(sid);
					const at = fmtAt(row?.updatedAt);
					lines.push(`- ${row?.title ?? "(未知标题)"}　（${sid}）${at ? ` · 最近 ${at}` : ""}`);
				}
			} catch {
				for (const sid of task.sessionIds) lines.push(`- (未知标题)　（${sid}）`);
			}
		}
		return lines.join("\n").slice(0, 20000);
	}
	...
	*/
	...
	ctx.effect(() => ctx.webServer.register({
		...
		handler: async (req, res) => {
			...
			try {
				...
				// POST /taskboard/api/tasks/:id/arm-session — 登记"下一个新会话属于本任务"。
				// 原生「新建对话」是草稿态：点按钮时还没有 session id，要等用户发出第一条
				// 消息会话才真正创建。所以客户端只负责登记，绑定由宿主在 session/created
				// 时完成 —— 不依赖页面内计时器，刷新页面/切走再发消息都不会漏。
				if (req.method === "POST" && seg[1] === "arm-session") {
					const at = Date.now();
					let found = false;
					await mutate((tasks) => {
						const t = tasks.find((x) => x.id === id);
	...
	// —— 新会话自动绑定到"刚点过 ＋新对话"的任务 ——
	// 只认【最近一次登记】的任务（一次新建对话只可能属于刚点的那个任务），
	// 并要求会话创建时间不早于登记时间（避免把别的会话误绑）。
	// 登记 2 小时未用即过期，避免陈旧登记把后来的新会话绑错任务。
	const ARM_TTL_MS = 2 * 60 * 60 * 1000;
	async function bindArmedSession(session) {
		try {
			const sessionId = normalizeSessionId(session?.id);
			if (!sessionId) return;
			const createdAt = session?.header?.createdAt ?? Date.now();
			const now = Date.now();
			const tasks = await loadTasks();
			const armed = tasks
				.filter((t) => t.armNextSession
					&& now - t.armNextSession <= ARM_TTL_MS          // 过期登记不算数
					&& createdAt >= t.armNextSession - 5000)         // 且会话必须晚于登记
				.sort((a, b) => b.armNextSession - a.armNextSession)[0];
			const hasStale = tasks.some((t) => t.armNextSession && now - t.armNextSession > ARM_TTL_MS);
			if (!armed && !hasStale) return;
			let bound = null;
			await mutate((list) => {
				for (const t of list) {
					if (t.armNextSession && now - t.armNextSession > ARM_TTL_MS) t.armNextSession = null;
					if (armed && t.id === armed.id) {
						if (!t.sessionIds.includes(sessionId)) t.sessionIds.push(sessionId);
						t.armNextSession = null;
						t.updatedAt = now;
						bound = t.title;
					}
				}
			});
			if (bound !== null) ctx.logger?.info?.(`dsh-taskboard: new session ${sessionId} auto-linked to task "${bound}"`);
		} catch (error) {
			ctx.logger?.warn?.(`dsh-taskboard: auto-link failed for ${session?.id}: ${String(error?.message ?? error)}`);
		}
	}
	// 注意：`session/created` 在 Harness 里是**按 carrier 作用域派发**的事件
	// （ctx.events.dispatch("emit", [carrier, "session/created", session])），
	// 插件不在该作用域内，实测收不到 —— 所以真正的发现手段是下面的轮询。
	ctx.on("session/created", (session) => { void bindArmedSession(session); });

	// 轮询实时会话列表找"新出现的会话"：纯内存比较（2s 一次），不依赖任何作用域事件。
	// 原生「新建对话」是草稿态，用户发出第一条消息时会话才诞生并进入实时列表，
	// 这里就能立刻发现并按登记信息完成绑定。
	const seenLiveSessions = new Set();
	try {
		for (const session of ctx.sessions?.list() ?? []) seenLiveSessions.add(session.id);
	} catch { /* ignore */ }
	setInterval(() => {
		try {
			for (const session of ctx.sessions?.list() ?? []) {
				if (seenLiveSessions.has(session.id)) continue;
				seenLiveSessions.add(session.id);
				void bindArmedSession(session);
			}
		} catch { /* ignore */ }
	}, 2000);
