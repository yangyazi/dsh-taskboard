	...
	async function sessionIndex() {
		...
		try {
			for (const session of ctx.sessions?.list() ?? []) {
				...
				// 无 agents 服务时回退到上面的未闭合 turn 判定。
				try {
					const agents = ctx.get?.("agents");
					if (agents?.get !== undefined) liveRunning = agents.get(session.id)?.status === "running";
				} catch { /* agents unavailable, keep fold-based running */ }
				const title = liveTitle ?? meta?.title ?? (cwd ? path.basename(cwd) : session.id);
				out.push({
					id: session.id,
					title,
					preview: livePreview ?? (title === session.id ? null : null),
					cwd,
					repo: cwd ? (repoByCwd.get(resolve(cwd)) ?? path.basename(cwd)) : null,
					turns: meta?.turns ?? 0,
					running: liveRunning,
					updatedAt: liveUpdatedAt ?? await sessionFileMtime(session.id)
				});
			}
		...
		handler: async (req, res) => {
			...
			try {
				...
				if (req.method === "POST" && seg[1] === "session") {
					...
					// 种子必须构成一个【完整闭合回合】(turn/start → user/message → turn/end)：
					// 裸的 user/message（无 turn 边界）在新版 Harness 里会被当成"回合仍在进行"，
					// 导致该会话打开后输入框被锁（需手动拖拽触发重排才恢复）。闭合回合让 GUI
					// 判定会话为 idle，输入框立即可用，agent 仍能在历史里读到任务上下文。
					const now = Date.now();
					const seed = [
