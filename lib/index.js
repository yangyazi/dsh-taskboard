...
export async function apply(ctx, config = {}) {
	...
	async function sessionFileMtime(sessionId) {
		...
		if (!raw) return null;
		const candidates = raw.startsWith("session-") ? [raw, raw.slice("session-".length)] : [raw, `session-${raw}`];
		try {
			for (const wsDir of await readdir(sessionRoot, { withFileTypes: true })) {
				if (!wsDir.isDirectory()) continue;
				for (const name of candidates) {
					try {
						const info = await stat(join(sessionRoot, wsDir.name, name, "session.jsonl.zstd"));
						return info.mtimeMs;
