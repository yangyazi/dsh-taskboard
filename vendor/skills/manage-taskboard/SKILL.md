...
用 **dsh-taskboard**（轻量任务面板插件）管理任务。Base URL：`http://127.0.0.1:3081/taskboard/api`（本机 DSH Web GUI 同源路径，也可用 `http://127.0.0.1:3081/taskboard/api`）。
...
2. **进展记录**：执行过程中定期用 `POST /tasks/:id/notes` 追加进展评论（做了什么、结果如何），这是任务圈子的**共享记忆**——任务下所有会话都看得到。
