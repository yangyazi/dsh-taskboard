...
<div id="dsh-tb-view" ${O}="">
  ...
  <div class="dsh-tb-body" id="dsh-tb-body"></div>
		...
		</div>`}function Rt(){let t=d("#dsh-tb-body"),e=d("#dsh-tb-count");if(!t)return;t.innerHTML='<div class="dsh-tb-columns"></div>';let s=d(".dsh-tb-columns",t);e&&(e.textContent=`${j.length} \u4E2A\u4EFB\u52A1`);for(let[o,h]of Object.entries(g)){let u=document.createElement("div");u.className="dsh-tb-col";let l=j.filter(c=>c.status===o);u.innerHTML=`<div class="dsh-tb-col-head">${h.label} <b>${l.length}</b></div><div class="dsh-tb-col-body"></div>`;let v=d(".dsh-tb-col-body",u);l.length?l.forEach(c=>v.insertAdjacentHTML("beforeend",Pt(c))):v.innerHTML='<div class="dsh-tb-empty">\u2014</div>',s.appendChild(u)}$(".dsh-tb-card",s).forEach(o=>o.addEventListener("click",()=>Y(o.dataset.id)))}function It(){let t=d("#dsh-tb-body"),e=d("#dsh-tb-count");if(!t)return;if(e&&(e.textContent=`${j.length} \u4E2A\u4EFB\u52A1`),!j.length){t.innerHTML='<div class="dsh-tb-ov"><div class="dsh-tb-empty">\uFF08\u6682\u65E0\u4EFB\u52A1\uFF0C\u70B9\u300C\uFF0B \u65B0\u5EFA\u300D\u521B\u5EFA\uFF09</div></div>';return}let s=j.map(o=>`<tr data-id="${r(o.id)}">
			<td>${Ot(o)}</td>
			<td>${gt(o)}</td>
			...
			<table class="dsh-tb-table">
			...
			</div>`,d("#tb-f-title",t).addEventListener("input",()=>{d("#tb-f-title",t).classList.remove("err"),d("#tb-hint-title",t).hidden=!0}),d("#tb-f-repo",t).addEventListener("change",()=>{d("#tb-f-repo",t).classList.remove("err"),d("#tb-hint-repo",t).hidden=!0}),d('[data-act="cancel"]',t).addEventListener("click",u),d('[data-act="save"]',t).addEventListener("click",async()=>{let l=d("#tb-f-title",t).value.trim(),v=d("#tb-f-repo",t).value;d("#tb-hint-title",t).hidden=!0,d("#tb-hint-repo",t).hidden=!0,$(".err",t).forEach(b=>b.classList.remove("err"));let c=!0;if(l||(s("#tb-f-title","#tb-hint-title"),c=!1),v||(s("#tb-f-repo","#tb-hint-repo"),c=!1),!!c)try{let{task:b}=await x("/tasks",{method:"POST",body:JSON.stringify({title:l,repo:v,feature:d("#tb-f-feature",t).value.trim(),description:d("#tb-f-desc",t).value})});J(),h(b)}catch(b){alert(`\u521B\u5EFA\u5931\u8D25\uFF1A${b.message}`)}}),d("#tb-f-title",t).focus()},h=l=>{t.innerHTML=`<div class="dsh-tb-modal" style="width:min(560px,92vw)">
				<h3>\u2705 \u4EFB\u52A1\u5DF2\u521B\u5EFA</h3>
				...
				<div class="dsh-tb-actions"><button data-act="cancel">\u53D6\u6D88</button></div>
						...
						</div>`)).join("");u.innerHTML=n||'<div class="dsh-tb-empty">\uFF08\u65E0\u5339\u914D\u4F1A\u8BDD\uFF09</div>',$(".dsh-tb-pick-item",o).forEach(f=>f.addEventListener("click",async()=>{let i=f.dataset.sid;try{let{task:p,injected:E,injectionNote:y}=await x(`/tasks/${t}/sessions`,{method:"POST",body:JSON.stringify({sessionId:i,action:"link"})});o.remove(),await k(),e(p),A(E?"\u2713 \u5DF2\u5173\u8054\u4F1A\u8BDD\uFF08\u5DF2\u6CE8\u5165\u4EFB\u52A1\u4E0A\u4E0B\u6587\uFF09":y||"\u5DF2\u5173\u8054\u4F1A\u8BDD")}catch(p){alert(`\u5173\u8054\u5931\u8D25\uFF1A${p.message}`)}}))};l(""),d("#tb-pick-search",o).addEventListener("input",()=>l(d("#tb-pick-search",o).value)),d('[data-act="cancel"]',o).addEventListener("click",()=>o.remove()),o.addEventListener("click",v=>{v.target===o&&o.remove()}),d("#tb-pick-search",o).focus()}).catch(()=>alert("\u83B7\u53D6\u4F1A\u8BDD\u5217\u8868\u5931\u8D25"))}async function Y(t){let{task:e}=await x(`/tasks/${t}`);dt(e)}function dt(t){let e=t.id,s=document.createElement("div");s.className="dsh-tb-modal-mask";let o=M.map(a=>`<option value="${r(a.id)}">${r(a.title||a.id)}${a.repo?` \xB7 ${r(H(a.repo))}`:""}</option>`).join("");s.innerHTML=`<div class="dsh-tb-modal" style="width:min(700px,92vw)">
			<h3>${r(t.title)}</h3>
