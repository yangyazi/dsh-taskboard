(()=>{(()=>{if(window.__dshTaskboardLoaded)return;window.__dshTaskboardLoaded=!0;let pt="/taskboard/api",x={todo:{label:"\u5F85\u529E",color:"#8b949e"},in_progress:{label:"\u8FDB\u884C\u4E2D",color:"#3478f6"},in_review:{label:"\u8BC4\u5BA1\u4E2D",color:"#bc8cff"},blocked:{label:"\u5DF2\u963B\u585E",color:"#f85149"},done:{label:"\u5DF2\u5B8C\u6210",color:"#3fb950"}},_=["todo","in_progress","in_review","blocked","done"],O={low:{label:"\u4F4E",color:"#8b949e"},medium:{label:"\u4E2D",color:"#d29922"},high:{label:"\u9AD8",color:"#e3862e"},urgent:{label:"\u7D27\u6025",color:"#f85149"}},X={none:"\u2014",pending:"\u8BC4\u5BA1\u5F85\u5904\u7406",approved:"\u8BC4\u5BA1\u901A\u8FC7",rejected:"\u8BC4\u5BA1\u9A73\u56DE"},W={none:"\u2014",pending:"\u6D4B\u8BD5\u5F85\u5904\u7406",passed:"\u6D4B\u8BD5\u901A\u8FC7",failed:"\u6D4B\u8BD5\u5931\u8D25"},st=["#79c0ff","#d2a8ff","#7ee787","#ffa657","#ff7b72","#f2cc60","#a5d6ff","#ffd7a8"],C="data-dsh-taskboard-active",z="data-dsh-taskboard-entry",M="data-dsh-taskboard-view",N="data-dsh-ssh-active",ht='<svg viewBox="0 0 16 16" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="2" y="2.5" width="12" height="11" rx="1.5"/><path d="M2 6.5h12M6.5 6.5v7"/></svg>',d=(t,e=document)=>e.querySelector(t),k=(t,e=document)=>[...e.querySelectorAll(t)],r=t=>String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e]),H=t=>{let e=String(t||"").split("/").filter(Boolean);return e.length?e[e.length-1]:String(t||"")},ft=t=>{let e=0;for(let a of String(t))e=e*31+a.charCodeAt(0)>>>0;return st[e%st.length]},R=t=>{if(!t)return"";let e=new Date(t),a=Date.now()-t;return a<6e4?"\u521A\u521A":a<36e5?`${Math.floor(a/6e4)} \u5206\u949F\u524D`:a<864e5?`${Math.floor(a/36e5)} \u5C0F\u65F6\u524D`:`${e.getMonth()+1}/${e.getDate()} ${String(e.getHours()).padStart(2,"0")}:${String(e.getMinutes()).padStart(2,"0")}`},T=[],q=[],P=[],F=[],L={q:"",repo:"",priority:"",status:"",label:""},A="overview",D={};async function g(t,e={}){let a=await fetch(pt+t,{headers:{"content-type":"application/json"},...e}),i=await a.json().catch(()=>({}));if(!a.ok)throw new Error(i.error||`HTTP ${a.status}`);return i}let vt=`
<style>
...
html[${C}]:not([${N}]) [data-pane='conversation'] > div[${M}],
html[${C}]:not([${N}]) [class*='centerCol'] > div[${M}] {
  display: flex !important; flex-direction: column;
}
html[${C}]:not([${N}]) [data-pane='conversation'] > :not([${M}]),
html[${C}]:not([${N}]) [class*='centerCol'] > :not([${M}]) {
...
[${z}]:hover { background: var(--dsw-specific-sidebar-nav-item-hover, #1b2127); color: var(--dsw-alias-label-primary, #e6edf3); }
[${z}][data-active] { background: var(--dsw-specific-sidebar-nav-item-active, #232a31); color: var(--dsw-alias-label-primary, #e6edf3); font-weight: 600; }
[${z}] span { display: inline-flex; align-items: center; justify-content: center; flex: none; }
[${z}][data-icon-only] { gap: 0; justify-content: center; padding: 0; height: 36px; margin-bottom: 12px; }
[${z}][data-icon-only] > span:not(:first-child) { display: none; }
...
</style>`;document.documentElement.insertAdjacentHTML("beforeend",vt);let ut=`
<div id="dsh-tb-view" ${M}="">
  ...
  <div class="dsh-tb-body" id="dsh-tb-body"></div>
</div>`;function xt(){let t=document.querySelector('[data-pane="sidebar"], [class*="sidebarCol"]');return t===null?void 0:t.querySelector('[class*="logoRow"]')?.parentElement??t.firstElementChild}function gt(t){let e=t.querySelector('button[class*="newSession"]');if(e!==null)return e;for(let a of t.children)if(a.tagName==="BUTTON")return a}function wt(){return document.querySelector('[data-pane="conversation"]')??document.querySelector('[class*="centerCol"]')??void 0}function mt(t){return[...t.classList].some(e=>/collapsed/i.test(e))}let w=null,B=null;function G(){return document.documentElement.hasAttribute(C)}function at(){w!==null&&(G()?w.setAttribute("data-active",""):w.removeAttribute("data-active"))}function Q(t){let e=t!==void 0?t:!G();e&&document.documentElement.removeAttribute(N),e?document.documentElement.setAttribute(C,""):document.documentElement.removeAttribute(C),at(),e&&V()}function dt(){if(B!==null)return;let t=wt();t!==void 0&&(B=document.createElement("div"),B.setAttribute(M,""),B.innerHTML=ut,t.appendChild(B),yt())}function ot(){let t=xt();if(t===void 0||(w===null&&(w=document.createElement("button"),w.type="button",w.setAttribute(z,""),w.innerHTML=`<span>${ht}</span><span>\u4EFB\u52A1\u770B\u677F</span>`,w.title="\u4EFB\u52A1\u770B\u677F",w.addEventListener("click",()=>Q()),at()),w.parentElement===t))return;let e=gt(t),a=e?.closest('[class*="logoRow"]'),i=a!=null&&a.parentElement===t?a:e;t.insertBefore(w,i?.nextElementSibling??null),(()=>{w!==null&&(mt(t)?w.setAttribute("data-icon-only",""):w.removeAttribute("data-icon-only"))})()}function yt(){d("#dsh-tb-new")?.addEventListener("click",nt),d("#dsh-tb-refresh")?.addEventListener("click",()=>V()),d("#dsh-tb-search")?.addEventListener("input",()=>{L.q=d("#dsh-tb-search").value.trim(),y()}),d("#dsh-tb-repo-filter")?.addEventListener("change",()=>{L.repo=d("#dsh-tb-repo-filter").value,y()}),d("#dsh-tb-priority-filter")?.addEventListener("change",()=>{L.priority=d("#dsh-tb-priority-filter").value,y()}),d("#dsh-tb-status-filter")?.addEventListener("change",()=>{L.status=d("#dsh-tb-status-filter").value,y()}),d("#dsh-tb-label-filter")?.addEventListener("change",()=>{L.label=d("#dsh-tb-label-filter").value,y()}),k(".dsh-tb-tab").forEach(t=>t.addEventListener("click",()=>{A=t.dataset.tbView,K()}))}async function y(){let t=new URLSearchParams;for(let a of["q","repo","priority","status","label"])L[a]&&t.set(a,L[a]);T=(await g(`/tasks?${t}`)).tasks||[],K()}async function $t(){try{let[t,e]=await Promise.all([g("/sessions"),g("/workspaces")]);q=t.sessions||[],P=e.workspaces||[];let a=d("#dsh-tb-repo-filter");if(a){let c=a.value;a.innerHTML='<option value="">\u5168\u90E8\u4ED3\u5E93</option>'+P.map(b=>`<option value="${r(b.path)}" title="${r(b.path)}">${r(H(b.path))}</option>`).join(""),c&&[...a.options].some(b=>b.value===c)&&(a.value=c)}let i=await g("/tasks").catch(()=>({tasks:[]})),u=new Set;for(let c of i.tasks||[])for(let b of c.labels||[])u.add(b);F=[...u].sort();let f=d("#dsh-tb-label-filter");if(f){let c=f.value;f.innerHTML='<option value="">\u5168\u90E8\u6807\u7B7E</option>'+F.map(b=>`<option value="${r(b)}">${r(b)}</option>`).join(""),c&&[...f.options].some(b=>b.value===c)&&(f.value=c)}let l=d("#dsh-tb-priority-filter");l&&!l.options.length&&(l.innerHTML='<option value="">\u5168\u90E8\u4F18\u5148\u7EA7</option>'+Object.entries(O).map(([c,b])=>`<option value="${c}">${b.label}</option>`).join(""));let v=d("#dsh-tb-status-filter");v&&!v.options.length&&(v.innerHTML='<option value="">\u5168\u90E8\u72B6\u6001</option>'+Object.entries(x).map(([c,b])=>`<option value="${c}">${b.label}</option>`).join(""))}catch{}}async function kt(){for(let t of P)if(!(!t.path||D[t.path]!==void 0))try{let e=await fetch(`/ide/api/git?op=status&path=${encodeURIComponent(t.path)}`).then(a=>a.json());D[t.path]=e.git?{branch:e.branch?.name||"(detached)",dirty:(e.files||[]).length}:null}catch{D[t.path]=null}}function V(){return $t().then(()=>kt()).then(y).catch(y)}function it(t){return(t.labels||[]).map(e=>`<span class="dsh-tb-pill" style="color:${ft(e)}">${r(e)}</span>`).join("")}function Et(t){let e=x[t.status]||x.todo;return`<span class="dsh-tb-pill" style="color:${e.color}">${e.label}</span>`}function rt(t){let e=O[t.priority]||O.medium;return`<span class="dsh-tb-pill" style="color:${e.color}">${e.label}</span>`}function Lt(t){let e=(a,i,u)=>`<button type="button" class="dsh-tb-statusbtn ${u||""}" data-status="${a}">${i}</button>`;switch(t.status){case"todo":return`${e("in_progress","\u25B6 \u5F00\u59CB\u6267\u884C")} ${e("blocked","\u26D4 \u963B\u585E","ghost")}`;case"in_progress":return`${e("in_review","\u63D0\u4EA4\u8BC4\u5BA1")} ${e("blocked","\u26D4 \u963B\u585E","ghost")}`;case"in_review":return`${e("done","\u2705 \u786E\u8BA4\u5B8C\u6210","primary")} ${e("in_progress","\u21A9 \u9000\u56DE\u4FEE\u6539")}`;case"done":return`${e("in_progress","\u21A9 \u91CD\u65B0\u6253\u5F00","ghost")}`;case"blocked":return`${e("in_progress","\u25B6 \u6062\u590D\u8FDB\u884C")}`;default:return""}}function J(t){try{localStorage.setItem("dsh.sessions.current",JSON.stringify({sessionId:t}))}catch{}location.reload()}function j(t){let e=document.createElement("div");e.className="dsh-tb-toast",e.textContent=t,document.body.appendChild(e),setTimeout(()=>e.remove(),1600)}function Tt(){var h;let t={};for(let o of _)t[o]=0;let e={};for(let o of T){t[o.status]!==void 0&&t[o.status]++;let p=o.repo||"\uFF08\u672A\u6307\u5B9A\uFF09";(e[p]||(e[p]=[])).push(o)}let a=T.length,i=t.done||0,u=a?Math.round(i/a*100):0,f={};for(let o of q)o.repo&&(f[h=o.repo]||(f[h]=[])).push(o);let l=[...T].sort((o,p)=>p.updatedAt-o.updatedAt).slice(0,10),c='<div class="dsh-tb-ov-stat dsh-tb-ov-add" id="dsh-tb-ov-new" title="\u65B0\u5EFA\u4EFB\u52A1"><div class="dsh-tb-ov-addbtn">\uFF0B \u65B0\u5EFA\u4EFB\u52A1</div></div>'+[["\u5168\u90E8\u4EFB\u52A1",a,"#e6edf3",""],..._.map(o=>[x[o].label,t[o]||0,x[o].color,o]),["\u5B8C\u6210\u7387",`${u}%`,"#3fb950",null]].map(([o,p,$,m])=>{let tt=o==="\u5B8C\u6210\u7387"?u:a?Math.round(p/a*100):0,U=m!==null?" dsh-tb-ov-stat-click":"",et=m===""?"\u67E5\u770B\u5168\u90E8\u4EFB\u52A1":m?`\u67E5\u770B\u300C${o}\u300D\u7684\u4EFB\u52A1`:"";return`<div class="dsh-tb-ov-stat${U}" ${m!==null?`data-status="${m}"`:""} title="${et}"><div class="n" style="color:${$}">${p}</div><div class="l">${o}</div><div class="mini"><i style="width:${tt}%;background:${$}"></i></div></div>`}).join(""),b;P.length?b=P.map(o=>{let p=e[o.path]||e[o.title]||[],$=_.map(E=>p.filter(S=>S.status===E).length),m=p.length,tt=$[_.indexOf("done")]||0,U=m?Math.round(tt/m*100):0,et=m?_.map((E,S)=>$[S]?`<i style="width:${Math.round($[S]/m*100)}%;background:${x[E].color}" title="${x[E].label} ${$[S]}"></i>`:"").join(""):"",Mt=_.map((E,S)=>$[S]?`<span class="st"><i class="dot" style="background:${x[E].color}"></i>${x[E].label} <b>${$[S]}</b></span>`:"").join(""),At=(f[o.path]||f[o.title]||[]).length,Y=D[o.path],_t=Y?`<span class="branch">\u2387 ${r(Y.branch)}</span>${Y.dirty?`<span class="branch dirty" title="${Y.dirty} \u4E2A\u672A\u63D0\u4EA4\u6587\u4EF6">\u25CF${Y.dirty}</span>`:""}`:"",bt=(f[o.path]||f[o.title]||[]).slice(0,3).map(E=>`<div class="s" data-sid="${r(E.id)}" title="\u6253\u5F00\u4F1A\u8BDD ${r(E.id)}">\u25B8 ${r(E.title)}</div>`).join("");return`<div class="dsh-tb-ov-ws" data-repo="${r(o.path)}">
					<div class="ws-head"><h4>${r(o.title)}</h4><span class="ws-total">${m} \u4E2A\u4EFB\u52A1</span></div>
					<div class="path">${r(o.path||"")}</div>
					${m?`<div class="stack">${et}</div><div class="ws-stats">${Mt}</div>`:'<div class="ws-stats" style="color:var(--dsw-alias-label-secondary,#9aa7b4)">\u6682\u65E0\u4EFB\u52A1\uFF0C\u70B9\u300C\uFF0B \u65B0\u5EFA\u300D\u521B\u5EFA</div>'}
					<div class="comp"><span class="pct">\u5B8C\u6210\u7387 ${U}%</span><div class="track"><i style="width:${U}%"></i></div></div>
					<div class="meta">${_t}<span class="sess">\u4F1A\u8BDD ${At}</span></div>
					${bt?`<div class="sesslist">${bt}</div>`:""}
				</div>`}).join(""):b='<div class="dsh-tb-empty">\uFF08\u6682\u65E0\u5DE5\u4F5C\u533A\uFF09</div>';let s=l.length?l.map(o=>{let p=x[o.status]||x.todo;return`<div class="dsh-tb-ov-item" data-id="${r(o.id)}">
				<span class="dot" style="background:${p.color}"></span>
				<span class="t">${r(o.title)}</span>
				${o.repo?`<span class="r">${r(H(o.repo))}</span>`:""}
				<span class="tm">${R(o.updatedAt)}</span>
		...
		</div>`,d("#dsh-tb-ov-new")?.addEventListener("click",nt),k(".dsh-tb-ov-stat[data-status]").forEach(o=>o.addEventListener("click",()=>{L.status=o.dataset.status,A="list",K(),y()})),k(".dsh-tb-ov-ws").forEach(o=>o.addEventListener("click",()=>{L.repo=o.dataset.repo;let p=d("#dsh-tb-repo-filter");p&&(p.value=L.repo),A="kanban",K(),y()})),k(".dsh-tb-ov-ws .s").forEach(o=>o.addEventListener("click",p=>{p.stopPropagation(),J(o.dataset.sid)})),k(".dsh-tb-ov-item").forEach(o=>o.addEventListener("click",()=>I(o.dataset.id))))}function zt(t){let e=x[t.status]||x.todo,a=O[t.priority]||O.medium,i=[];i.push(rt(t)),t.status==="in_review"&&i.push('<span class="dsh-tb-pill" style="color:#bc8cff;font-weight:600">\u25C9 \u5F85 review</span>'),t.repo&&i.push(`<span class="dsh-tb-pill" style="color:#79c0ff">${r(H(t.repo))}</span>`),t.feature&&i.push(`<span class="dsh-tb-pill" style="color:#d2a8ff">${r(t.feature)}</span>`),i.push(it(t)),t.review&&t.review!=="none"&&i.push(`<span class="dsh-tb-pill" style="color:${t.review==="approved"?"#3fb950":t.review==="rejected"?"#f85149":"#d29922"}">${X[t.review]}</span>`),t.test&&t.test!=="none"&&i.push(`<span class="dsh-tb-pill" style="color:${t.test==="passed"?"#3fb950":t.test==="failed"?"#f85149":"#d29922"}">${W[t.test]}</span>`);let u=[];return t.sessionIds?.length&&u.push(`<span>\u4F1A\u8BDD ${t.sessionIds.length}</span>`),t.notes?.length&&u.push(`<span>\u8BC4\u8BBA ${t.notes.length}</span>`),u.push(`<span>${R(t.updatedAt)}</span>`),`<div class="dsh-tb-card" data-id="${r(t.id)}" style="border-left:3px solid ${e.color}">
			...
			<div class="dsh-tb-card-foot">${u.join(" \xB7 ")}</div>
		</div>`}function jt(){let t=d("#dsh-tb-body"),e=d("#dsh-tb-count");if(!t)return;t.innerHTML='<div class="dsh-tb-columns"></div>';let a=d(".dsh-tb-columns",t);e&&(e.textContent=`${T.length} \u4E2A\u4EFB\u52A1`);for(let[i,u]of Object.entries(x)){let f=document.createElement("div");f.className="dsh-tb-col";let l=T.filter(c=>c.status===i);f.innerHTML=`<div class="dsh-tb-col-head">${u.label} <b>${l.length}</b></div><div class="dsh-tb-col-body"></div>`;let v=d(".dsh-tb-col-body",f);l.length?l.forEach(c=>v.insertAdjacentHTML("beforeend",zt(c))):v.innerHTML='<div class="dsh-tb-empty">\u2014</div>',a.appendChild(f)}k(".dsh-tb-card",a).forEach(i=>i.addEventListener("click",()=>I(i.dataset.id)))}function St(){let t=d("#dsh-tb-body"),e=d("#dsh-tb-count");if(!t)return;if(e&&(e.textContent=`${T.length} \u4E2A\u4EFB\u52A1`),!T.length){t.innerHTML='<div class="dsh-tb-ov"><div class="dsh-tb-empty">\uFF08\u6682\u65E0\u4EFB\u52A1\uFF0C\u70B9\u300C\uFF0B \u65B0\u5EFA\u300D\u521B\u5EFA\uFF09</div></div>';return}let a=T.map(i=>`<tr data-id="${r(i.id)}">
			<td>${Et(i)}</td>
			<td>${rt(i)}</td>
			<td style="max-width:340px"><div style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${r(i.title)}</div></td>
			<td>${i.repo?`<span class="dsh-tb-pill" style="color:#79c0ff">${r(H(i.repo))}</span>`:""}</td>
			...
			<td style="white-space:nowrap">${R(i.updatedAt)}</td>
		...
		</div>`,k("tr[data-id]",t).forEach(i=>i.addEventListener("click",()=>I(i.dataset.id)))}function K(){k(".dsh-tb-tab").forEach(e=>{e.dataset.tbView===A?e.classList.add("dsh-tb-tab-on"):e.classList.remove("dsh-tb-tab-on")});let t=d("#dsh-tb-toolbar");t&&(t.style.display=A==="overview"?"none":"flex"),A==="kanban"?jt():A==="list"?St():Tt()}function nt(){let t=document.createElement("div");t.className="dsh-tb-modal-mask";let e=P.map(l=>`<option value="${r(l.path)}" title="${r(l.path)}">${r(l.path)}</option>`).join(""),a=(l,v)=>{d(l,t)?.classList.add("err"),v&&(d(v,t).hidden=!1)},i=()=>{t.innerHTML=`<div class="dsh-tb-modal" style="width:min(560px,92vw)">
				<h3>\u65B0\u5EFA\u4EFB\u52A1</h3>
			...
			</div>`,d("#tb-f-title",t).addEventListener("input",()=>{d("#tb-f-title",t).classList.remove("err"),d("#tb-hint-title",t).hidden=!0}),d("#tb-f-repo",t).addEventListener("change",()=>{d("#tb-f-repo",t).classList.remove("err"),d("#tb-hint-repo",t).hidden=!0}),d('[data-act="cancel"]',t).addEventListener("click",f),d('[data-act="save"]',t).addEventListener("click",async()=>{let l=d("#tb-f-title",t).value.trim(),v=d("#tb-f-repo",t).value;d("#tb-hint-title",t).hidden=!0,d("#tb-hint-repo",t).hidden=!0,k(".err",t).forEach(b=>b.classList.remove("err"));let c=!0;if(l||(a("#tb-f-title","#tb-hint-title"),c=!1),v||(a("#tb-f-repo","#tb-hint-repo"),c=!1),!!c)try{let{task:b}=await g("/tasks",{method:"POST",body:JSON.stringify({title:l,repo:v,feature:d("#tb-f-feature",t).value.trim(),description:d("#tb-f-desc",t).value})});V(),u(b)}catch(b){alert(`\u521B\u5EFA\u5931\u8D25\uFF1A${b.message}`)}}),d("#tb-f-title",t).focus()},u=l=>{t.innerHTML=`<div class="dsh-tb-modal" style="width:min(560px,92vw)">
				<h3>\u2705 \u4EFB\u52A1\u5DF2\u521B\u5EFA</h3>
			...
			</div>`,d('[data-act="newsess"]',t).addEventListener("click",async()=>{try{let{sessionId:v}=await g(`/tasks/${l.id}/session`,{method:"POST",body:"{}"});J(v)}catch(v){alert(`\u65B0\u5EFA\u4F1A\u8BDD\u5931\u8D25\uFF1A${v.message}`)}}),d('[data-act="bind"]',t).addEventListener("click",()=>{t.remove(),I(l.id)}),d('[data-act="detail"]',t).addEventListener("click",()=>{t.remove(),I(l.id)}),d('[data-act="close"]',t).addEventListener("click",()=>t.remove())},f=()=>t.remove();t.addEventListener("click",l=>{l.target===t&&f()}),document.body.appendChild(t),i()}function Ct(t,e){g("/sessions").then(a=>{let i=document.createElement("div");i.className="dsh-tb-modal-mask",i.innerHTML=`<div class="dsh-tb-modal" style="width:min(600px,92vw)">
				<h3>\u9009\u62E9\u4F1A\u8BDD\u5173\u8054</h3>
			...
			</div>`,document.body.appendChild(i);let u=a.sessions||q,f=d("#tb-pick-list",i),l=v=>{let c=String(v||"").trim().toLowerCase(),b=u.filter(h=>!c||String(h.title||"").toLowerCase().includes(c)),s={};for(let h of b){let o=h.repo||"\uFF08\u672A\u6307\u5B9A\uFF09";(s[o]||(s[o]=[])).push(h)}let n=Object.entries(s).sort((h,o)=>o[1].length-h[1].length).map(([h,o])=>(o.sort((p,$)=>($.updatedAt||0)-(p.updatedAt||0)),`<div class="dsh-tb-pick-group">
							<div class="dsh-tb-pick-grouphead">${r(H(h))} <b>${o.length}</b></div>
							${o.map(p=>`<div class="dsh-tb-pick-item" data-sid="${r(p.id)}">
								<div class="t">${r(p.title||p.id)}${p.running?' <span class="dsh-tb-run">\u25CF \u8FD0\u884C\u4E2D</span>':""}</div>
								<div class="m">${p.updatedAt?R(p.updatedAt):""}</div>
							</div>`).join("")}
						</div>`)).join("");f.innerHTML=n||'<div class="dsh-tb-empty">\uFF08\u65E0\u5339\u914D\u4F1A\u8BDD\uFF09</div>',k(".dsh-tb-pick-item",i).forEach(h=>h.addEventListener("click",async()=>{let o=h.dataset.sid;try{let{task:p,injected:$,injectionNote:m}=await g(`/tasks/${t}/sessions`,{method:"POST",body:JSON.stringify({sessionId:o,action:"link"})});i.remove(),await y(),e(p),j($?"\u2713 \u5DF2\u5173\u8054\u4F1A\u8BDD\uFF08\u5DF2\u6CE8\u5165\u4EFB\u52A1\u4E0A\u4E0B\u6587\uFF09":m||"\u5DF2\u5173\u8054\u4F1A\u8BDD")}catch(p){alert(`\u5173\u8054\u5931\u8D25\uFF1A${p.message}`)}}))};l(""),d("#tb-pick-search",i).addEventListener("input",()=>l(d("#tb-pick-search",i).value)),d('[data-act="cancel"]',i).addEventListener("click",()=>i.remove()),i.addEventListener("click",v=>{v.target===i&&i.remove()}),d("#tb-pick-search",i).focus()}).catch(()=>alert("\u83B7\u53D6\u4F1A\u8BDD\u5217\u8868\u5931\u8D25"))}async function I(t){let{task:e}=await g(`/tasks/${t}`);Z(e)}function Z(t){let e=t.id,a=document.createElement("div");a.className="dsh-tb-modal-mask";let i=q.map(s=>`<option value="${r(s.id)}">${r(s.title||s.id)}${s.repo?` \xB7 ${r(H(s.repo))}`:""}</option>`).join("");a.innerHTML=`<div class="dsh-tb-modal" style="width:min(700px,92vw)">
			<h3>${r(t.title)}</h3>
			...
			<div class="dsh-tb-row">
				<div class="dsh-tb-field"><label>\u72B6\u6001\uFF08AI \u81EA\u52A8\u6D41\u8F6C\uFF0C\u4F60\u53EA\u9700\u5728"\u8BC4\u5BA1\u4E2D"\u65F6\u5904\u7406\uFF09</label>
					<div class="dsh-tb-statusline">
						...
						<div class="dsh-tb-statusactions">${Lt(t)}</div>
					</div>
				</div>
				<div class="dsh-tb-field"><label>\u4F18\u5148\u7EA7</label><select id="tb-d-priority">${Object.entries(O).map(([s,n])=>`<option value="${s}" ${s===t.priority?"selected":""}>${n.label}</option>`).join("")}</select></div>
			...
			<div class="dsh-tb-field"><label>\u8FDB\u5C55\u8BB0\u5F55\uFF08${(t.notes||[]).length}\uFF09</label>
				<div id="tb-d-notes">${(t.notes||[]).map(s=>`<div class="dsh-tb-note"><span class="dsh-tb-note-time">${R(s.at)}</span><br/>${r(s.text)}</div>`).join("")||'<div class="dsh-tb-note">\uFF08\u6682\u65E0\u8BB0\u5F55\uFF09</div>'}</div>
		...
		</div>`,document.body.appendChild(a);let u=async()=>{let s=d("#tb-d-ctx-body",a);if(s)try{let{context:n}=await g(`/tasks/${e}/context`);s.textContent=n||"\uFF08\u6682\u65E0\u5171\u4EAB\u4E0A\u4E0B\u6587\uFF09"}catch{s.textContent="\uFF08\u83B7\u53D6\u5931\u8D25\uFF09"}},f=async()=>{try{let{context:s}=await g(`/tasks/${e}/context`);await navigator.clipboard.writeText(s||""),j("\u2713 \u5DF2\u590D\u5236\u6700\u65B0\u5171\u4EAB\u4E0A\u4E0B\u6587")}catch{try{let{context:s}=await g(`/tasks/${e}/context`),n=document.createElement("textarea");n.value=s||"",document.body.appendChild(n),n.select(),document.execCommand("copy"),n.remove(),j("\u2713 \u5DF2\u590D\u5236\u6700\u65B0\u5171\u4EAB\u4E0A\u4E0B\u6587")}catch(s){alert(`\u590D\u5236\u5931\u8D25\uFF1A${s.message}`)}}};u(),d("#tb-d-ctx-refresh",a)?.addEventListener("click",u),d("#tb-d-ctx-copy",a)?.addEventListener("click",f);let l=()=>({repo:d("#tb-d-repo",a).value.trim(),feature:d("#tb-d-feature",a).value.trim(),priority:d("#tb-d-priority",a).value,progress:Number(d("#tb-d-progress",a).value),review:d("#tb-d-review",a).value,test:d("#tb-d-test",a).value,labels:d("#tb-d-labels",a).value.split(",").map(s=>s.trim()).filter(Boolean),description:d("#tb-d-desc",a).value}),v=()=>a.remove(),c=s=>{a.remove(),Z(s)},b=async s=>{let{task:n}=await g(`/tasks/${e}`,{method:"PATCH",body:JSON.stringify(s)});return await y(),n};a.addEventListener("click",s=>{s.target===a&&v()}),d('[data-act="cancel"]',a).addEventListener("click",v),d('[data-act="save"]',a).addEventListener("click",async()=>{try{let s=await b(l());c(s),j("\u2713 \u5DF2\u4FDD\u5B58")}catch(s){alert(`\u4FDD\u5B58\u5931\u8D25\uFF1A${s.message}`)}}),d('[data-act="del"]',a).addEventListener("click",async()=>{if(confirm(`\u5220\u9664\u4EFB\u52A1\u300C${t.title}\u300D\uFF1F`))try{await g(`/tasks/${e}`,{method:"DELETE"}),v(),await V()}catch(s){alert(`\u5220\u9664\u5931\u8D25\uFF1A${s.message}`)}}),d('[data-act="copyctx"]',a)?.addEventListener("click",f),d("#tb-d-note",a).addEventListener("keydown",async s=>{if(s.key==="Enter"&&(s.ctrlKey||s.metaKey)){let n=d("#tb-d-note",a).value.trim();if(!n)return;try{let{task:h}=await g(`/tasks/${e}/notes`,{method:"POST",body:JSON.stringify({text:n})});await y(),c(h),j("\u2713 \u8FDB\u5C55\u5DF2\u8BB0\u5F55")}catch(h){alert(`\u8BB0\u5F55\u5931\u8D25\uFF1A${h.message}`)}}}),d("#tb-d-sess-open",a)?.addEventListener("click",()=>{Ct(e,s=>{a.remove(),Z(s)})}),k("#tb-d-sessions [data-act]",a).forEach(s=>s.addEventListener("click",async()=>{let n=s.dataset.sid;if(s.dataset.act==="open")J(n);else{let{task:h}=await g(`/tasks/${e}/sessions`,{method:"POST",body:JSON.stringify({sessionId:n,action:"unlink"})});await y(),c(h),j("\u5DF2\u89E3\u9664\u4F1A\u8BDD")}})),d("#tb-d-sess-new",a)?.addEventListener("click",async()=>{try{let{sessionId:s}=await g(`/tasks/${e}/session`,{method:"POST",body:"{}"});J(s)}catch(s){alert(`\u65B0\u5EFA\u4F1A\u8BDD\u5931\u8D25\uFF1A${s.message}`)}}),k(".dsh-tb-statusbtn",a).forEach(s=>s.addEventListener("click",async()=>{try{let n=await b({status:s.dataset.status});c(n),j("\u2713 \u72B6\u6001\u5DF2\u66F4\u65B0")}catch(n){alert(`\u72B6\u6001\u6D41\u8F6C\u5931\u8D25\uFF1A${n.message}`)}})),d('[data-act="split"]',a)?.addEventListener("click",()=>{let s=document.createElement("div");s.className="dsh-tb-modal-mask",s.innerHTML=`<div class="dsh-tb-modal">
			...
			</div>`,document.body.appendChild(s);let n=()=>s.remove();d('[data-act="cancel"]',s).addEventListener("click",n),s.addEventListener("click",h=>{h.target===s&&n()}),d('[data-act="do"]',s).addEventListener("click",async()=>{let h=d("#tb-split-titles",s).value.split(`
`).map(o=>o.trim()).filter(Boolean);if(!h.length){d("#tb-split-titles",s).focus();return}try{let{tasks:o}=await g(`/tasks/${e}/split`,{method:"POST",body:JSON.stringify({titles:h})});n(),await V(),j(`\u2713 \u5DF2\u62C6\u5206\u4E3A ${o.length} \u4E2A\u5B50\u4EFB\u52A1`),o?.[0]&&I(o[0].id)}catch(o){alert(`\u62C6\u5206\u5931\u8D25\uFF1A${o.message}`)}}),d("#tb-split-titles",s).focus()})}dt(),ot(),new MutationObserver(()=>{dt(),ot()}).observe(document.body,{childList:!0,subtree:!0}),document.addEventListener("keydown",t=>{t.ctrlKey&&t.shiftKey&&(t.key==="B"||t.key==="b")&&(t.preventDefault(),Q())}),document.addEventListener("click",t=>{if(!G())return;let e=t.target;e instanceof Element&&(e.closest(`[${z}]`)||e.closest('[data-pane="sidebar"], [class*="sidebarCol"]')&&Q(!1))});function lt(){try{window.dispatchEvent(new Event("resize")),document.body?.dispatchEvent(new Event("resize"))}catch{}}let ct=[150,400,900];if(document.readyState==="loading")window.addEventListener("load",()=>{for(let t of ct)setTimeout(lt,t)});else for(let t of ct)setTimeout(lt,t)})();})();
