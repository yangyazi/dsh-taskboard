(()=>{(()=>{if(window.__dshTaskboardLoaded)return;window.__dshTaskboardLoaded=!0;let At="/taskboard/api",x={todo:{label:"\u5F85\u529E",color:"#8b949e"},in_progress:{label:"\u8FDB\u884C\u4E2D",color:"#3478f6"},in_review:{label:"\u8BC4\u5BA1\u4E2D",color:"#bc8cff"},blocked:{label:"\u5DF2\u963B\u585E",color:"#f85149"},done:{label:"\u5DF2\u5B8C\u6210",color:"#3fb950"}},P=["todo","in_progress","in_review","blocked","done"],N={low:{label:"\u4F4E",color:"#8b949e"},medium:{label:"\u4E2D",color:"#d29922"},high:{label:"\u9AD8",color:"#e3862e"},urgent:{label:"\u7D27\u6025",color:"#f85149"}},at={none:"\u2014",pending:"\u8BC4\u5BA1\u5F85\u5904\u7406",approved:"\u8BC4\u5BA1\u901A\u8FC7",rejected:"\u8BC4\u5BA1\u9A73\u56DE"},ot={none:"\u2014",pending:"\u6D4B\u8BD5\u5F85\u5904\u7406",passed:"\u6D4B\u8BD5\u901A\u8FC7",failed:"\u6D4B\u8BD5\u5931\u8D25"},bt=["#79c0ff","#d2a8ff","#7ee787","#ffa657","#ff7b72","#f2cc60","#a5d6ff","#ffd7a8"],C="data-dsh-taskboard-active",A="data-dsh-taskboard-entry",_="data-dsh-taskboard-view",Y="data-dsh-ssh-active",Mt='<svg viewBox="0 0 16 16" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="2" y="2.5" width="12" height="11" rx="1.5"/><path d="M2 6.5h12M6.5 6.5v7"/></svg>',o=(t,e=document)=>e.querySelector(t),y=(t,e=document)=>[...e.querySelectorAll(t)],l=t=>String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e]),H=t=>{let e=String(t||"").split("/").filter(Boolean);return e.length?e[e.length-1]:String(t||"")},Ot=t=>{let e=0;for(let s of String(t))e=e*31+s.charCodeAt(0)>>>0;return bt[e%bt.length]},R=t=>{if(!t)return"";let e=new Date(t),s=Date.now()-t;return s<6e4?"\u521A\u521A":s<36e5?`${Math.floor(s/6e4)} \u5206\u949F\u524D`:s<864e5?`${Math.floor(s/36e5)} \u5C0F\u65F6\u524D`:`${e.getMonth()+1}/${e.getDate()} ${String(e.getHours()).padStart(2,"0")}:${String(e.getMinutes()).padStart(2,"0")}`},z=[],M=[],q=[],nt=[],T={q:"",repo:"",priority:"",status:"",label:""},I="overview",F={};async function g(t,e={}){let s=await fetch(At+t,{headers:{"content-type":"application/json"},...e}),a=await s.json().catch(()=>({}));if(!s.ok)throw new Error(a.error||`HTTP ${s.status}`);return a}let _t=`
<style>
...
</style>`;document.documentElement.insertAdjacentHTML("beforeend",_t);let It=`
<div id="dsh-tb-view" ${_}="">
  ...
  <div class="dsh-tb-body" id="dsh-tb-body"></div>
</div>`;function pt(){let t=document.querySelector('[data-pane="sidebar"], [class*="sidebarCol"]');return t===null?void 0:t.querySelector('[class*="logoRow"]')?.parentElement??t.firstElementChild}function ht(t){let e=t.querySelector('button[class*="newSession"]');if(e!==null)return e;for(let s of t.children)if(s.tagName==="BUTTON")return s}function ft(){return document.querySelector('[data-pane="conversation"]')??document.querySelector('[class*="centerCol"]')??void 0}function ut(t){return[...t.classList].some(e=>/collapsed/i.test(e))}let m=null,V=null;function U(){return document.documentElement.hasAttribute(C)}function vt(){m!==null&&(U()?m.setAttribute("data-active",""):m.removeAttribute("data-active"))}function X(t){let e=t!==void 0?t:!U();e&&document.documentElement.removeAttribute(Y),e?document.documentElement.setAttribute(C,""):document.documentElement.removeAttribute(C),vt(),e&&B()}function gt(){if(V!==null)return;let t=ft();t!==void 0&&(V=document.createElement("div"),V.setAttribute(_,""),V.innerHTML=It,t.appendChild(V),Jt())}function xt(){let t=pt();if(t===void 0||(m===null&&(m=document.createElement("button"),m.type="button",m.setAttribute(A,""),m.innerHTML=`<span>${Mt}</span><span>\u4EFB\u52A1\u770B\u677F</span>`,m.title="\u4EFB\u52A1\u770B\u677F",m.addEventListener("click",()=>X()),vt()),m.parentElement===t))return;let e=ht(t),s=e?.closest('[class*="logoRow"]'),a=s!=null&&s.parentElement===t?s:e;t.insertBefore(m,a?.nextElementSibling??null),(()=>{m!==null&&(ut(t)?m.setAttribute("data-icon-only",""):m.removeAttribute("data-icon-only"))})()}let Pt=15,w=null,S=null,D=!1,wt=null;function W(t){let e=String(t??"");return e&&!e.startsWith("session-")?`session-${e}`:e}function it(t){return(t||[]).map(e=>e&&e.id&&W(e.id)!==e.id?{...e,id:W(e.id)}:e)}function Nt(){return g("/sessions").then(t=>it(t.sessions||[])).catch(()=>M)}function mt(t){return t.title&&String(t.title).trim()?String(t.title).trim():String(t.id)}function Ht(t){return String(t).replace(/^session-/,"").slice(0,8)}function Rt(t){let e=new Set,s=[];for(let a of[...t||[]].filter(r=>r&&r.updatedAt).sort((r,d)=>(d.updatedAt||0)-(r.updatedAt||0)))if(!e.has(a.id)&&(e.add(a.id),s.push(a),s.length>=Pt))break;return s}function yt(t){return`dsh-tb-read:${t}`}function qt(t){try{return Number(localStorage.getItem(yt(t)))||0}catch{return 0}}function Dt(t){try{localStorage.setItem(yt(t),String(Date.now()))}catch{}}function rt(t){if(S===null)return;let e=Rt(t),s=o("#dsh-recent-cnt");if(s&&(s.textContent=String(e.length)),!e.length){S.innerHTML='<div class="dsh-recent-empty">\uFF08\u6682\u65E0\u6700\u8FD1\u4F1A\u8BDD\uFF09</div>';return}let a=Z(),r=new Map;for(let d of e){let c=mt(d);r.set(c,(r.get(c)||0)+1)}S.innerHTML=e.map(d=>{let c=mt(d),v=(r.get(c)||0)>1,b=d.id===a,h="";return d.running?h='<span class="dot run"></span>':d.updatedAt&&d.updatedAt>qt(d.id)&&(h='<span class="dot idle"></span>'),`<div class="dsh-recent-row${b?" cur":""}" data-sid="${l(d.id)}" title="\u6253\u5F00\u5E76\u7EE7\u7EED\uFF1A${l(c)}\uFF08${l(d.id)}\uFF09">
				<span class="slot">${h}</span>
				<span class="title">${l(c)}</span>
				${v?`<span class="sid">${l(Ht(d.id))}</span>`:""}
				<span class="time">${R(d.updatedAt)}</span>
			</div>`}).join(""),y(".dsh-recent-row",S).forEach(d=>d.addEventListener("click",()=>{tt(d.dataset.sid)}))}function G(){Nt().then(t=>rt(t)).catch(()=>rt(M))}function $t(){let t=pt();if(t===void 0||(w===null&&(w=document.createElement("div"),w.id="dsh-recent",w.className="dsh-recent",w.innerHTML=`
				...
				<div class="dsh-recent-body" id="dsh-recent-body"></div>`,S=o("#dsh-recent-body",w),o("#dsh-recent-head",w).addEventListener("click",()=>{D=!D,w&&w.classList.toggle("collapsed",D),S&&(S.style.display=D?"none":"")}),o("#dsh-recent-refresh",w).addEventListener("click",r=>{r.stopPropagation(),G()}),rt(M),G()),w.parentElement===t))return;let e=m??ht(t);t.insertBefore(w,e?.nextElementSibling??null),S&&(S.style.maxHeight="46vh",S.style.overflowY="auto"),(()=>{S&&(S.style.display=D?"none":""),w&&w.classList.toggle("collapsed",D)})(),(()=>{w&&(ut(t)?w.style.display="none":w.style.display="")})()}function Bt(){wt===null&&(wt=setInterval(()=>{document.hidden||G()},6e4))}function Jt(){o("#dsh-tb-new")?.addEventListener("click",Ct),o("#dsh-tb-refresh")?.addEventListener("click",()=>B()),o("#dsh-tb-search")?.addEventListener("input",()=>{T.q=o("#dsh-tb-search").value.trim(),k()}),o("#dsh-tb-repo-filter")?.addEventListener("change",()=>{T.repo=o("#dsh-tb-repo-filter").value,k()}),o("#dsh-tb-priority-filter")?.addEventListener("change",()=>{T.priority=o("#dsh-tb-priority-filter").value,k()}),o("#dsh-tb-status-filter")?.addEventListener("change",()=>{T.status=o("#dsh-tb-status-filter").value,k()}),o("#dsh-tb-label-filter")?.addEventListener("change",()=>{T.label=o("#dsh-tb-label-filter").value,k()}),y(".dsh-tb-tab").forEach(t=>t.addEventListener("click",()=>{I=t.dataset.tbView,et()}))}async function k(){let t=new URLSearchParams;for(let s of["q","repo","priority","status","label"])T[s]&&t.set(s,T[s]);z=(await g(`/tasks?${t}`)).tasks||[],et()}async function Yt(){try{let[t,e]=await Promise.all([g("/sessions"),g("/workspaces")]);M=it(t.sessions||[]),q=e.workspaces||[];let s=o("#dsh-tb-repo-filter");if(s){let b=s.value;s.innerHTML='<option value="">\u5168\u90E8\u4ED3\u5E93</option>'+q.map(h=>`<option value="${l(h.path)}" title="${l(h.path)}">${l(H(h.path))}</option>`).join(""),b&&[...s.options].some(h=>h.value===b)&&(s.value=b)}let a=await g("/tasks").catch(()=>({tasks:[]})),r=new Set;for(let b of a.tasks||[])for(let h of b.labels||[])r.add(h);nt=[...r].sort();let d=o("#dsh-tb-label-filter");if(d){let b=d.value;d.innerHTML='<option value="">\u5168\u90E8\u6807\u7B7E</option>'+nt.map(h=>`<option value="${l(h)}">${l(h)}</option>`).join(""),b&&[...d.options].some(h=>h.value===b)&&(d.value=b)}let c=o("#dsh-tb-priority-filter");c&&!c.options.length&&(c.innerHTML='<option value="">\u5168\u90E8\u4F18\u5148\u7EA7</option>'+Object.entries(N).map(([b,h])=>`<option value="${b}">${h.label}</option>`).join(""));let v=o("#dsh-tb-status-filter");v&&!v.options.length&&(v.innerHTML='<option value="">\u5168\u90E8\u72B6\u6001</option>'+Object.entries(x).map(([b,h])=>`<option value="${b}">${h.label}</option>`).join(""))}catch{}}async function Vt(){for(let t of q)if(!(!t.path||F[t.path]!==void 0))try{let e=await fetch(`/ide/api/git?op=status&path=${encodeURIComponent(t.path)}`).then(s=>s.json());F[t.path]=e.git?{branch:e.branch?.name||"(detached)",dirty:(e.files||[]).length}:null}catch{F[t.path]=null}}function B(){return Yt().then(()=>Vt()).then(k).catch(k)}function kt(t){return(t.labels||[]).map(e=>`<span class="dsh-tb-pill" style="color:${Ot(e)}">${l(e)}</span>`).join("")}function Wt(t){let e=x[t.status]||x.todo;return`<span class="dsh-tb-pill" style="color:${e.color}">${e.label}</span>`}function Et(t){let e=N[t.priority]||N.medium;return`<span class="dsh-tb-pill" style="color:${e.color}">${e.label}</span>`}function Kt(t){let e=(s,a,r)=>`<button type="button" class="dsh-tb-statusbtn ${r||""}" data-status="${s}">${a}</button>`;switch(t.status){case"todo":return`${e("in_progress","\u25B6 \u5F00\u59CB\u6267\u884C")} ${e("blocked","\u26D4 \u963B\u585E","ghost")}`;case"in_progress":return`${e("in_review","\u63D0\u4EA4\u8BC4\u5BA1")} ${e("blocked","\u26D4 \u963B\u585E","ghost")}`;case"in_review":return`${e("done","\u2705 \u786E\u8BA4\u5B8C\u6210","primary")} ${e("in_progress","\u21A9 \u9000\u56DE\u4FEE\u6539")}`;case"done":return`${e("in_progress","\u21A9 \u91CD\u65B0\u6253\u5F00","ghost")}`;case"blocked":return`${e("in_progress","\u25B6 \u6062\u590D\u8FDB\u884C")}`;default:return""}}let Q=null;function Ft(){let t=new Set,e=document.querySelectorAll('[class*="centerCol"], [class*="sidebarCol"], [class*="frame"], [class*="App"], body > div');for(let s of e){for(let a of Object.keys(s)){if(!a.startsWith("__reactFiber$")&&!a.startsWith("__reactContainer$"))continue;let r=s[a];for(;r&&r.return;)r=r.return;r&&t.add(r)}if(t.size>0)break}return[...t]}function St(){if(Q&&Q.open)return Q;let t={open:null,startSession:null},e=Ft(),s=0;for(;e.length>0&&s++<4e4;){let a=e.shift(),r=a.memoizedProps;if(r!==null&&typeof r=="object"){t.open===null&&typeof r.open=="function"&&(t.open=r.open),t.startSession===null&&typeof r.startSession=="function"&&(t.startSession=r.startSession);let d=r.sessions??r.sessionService;if(d!==null&&typeof d=="object"&&(t.open===null&&typeof d.open=="function"&&(t.open=d.open),t.startSession===null&&typeof d.start=="function"&&(t.startSession=d.start)),t.open!==null&&t.startSession!==null)break}a.child&&e.push(a.child),a.sibling&&e.push(a.sibling)}return Q=t,t}function Ut(){return St().open}function Z(){try{let t=localStorage.getItem("dsh.sessions.current");if(t===null)return null;let e=JSON.parse(t);return typeof e?.sessionId=="string"?e.sessionId:null}catch{return null}}function Xt(t,e=4e3){let s=Date.now()+e;return new Promise(a=>{let r=()=>{let d=Z();if(d!==null&&d!==t)return a(d);if(Date.now()>s)return a(null);setTimeout(r,120)};r()})}function Gt(){let t=St();if(typeof t.startSession=="function")try{return t.startSession(),!0}catch{}let e=document.querySelector('button[class*="newSession"]');return e!==null?(e.click(),!0):!1}function Lt(){let t=[...document.querySelectorAll('[class*="_handle"], [class*="handle"]')].find(b=>b.offsetParent!==null&&getComputedStyle(b).cursor==="col-resize");if(!t)return!1;let e=t.getBoundingClientRect(),s=Math.round(e.left+e.width/2),a=Math.round(e.top+Math.min(200,e.height/2)),r=Element.prototype,d=r.setPointerCapture,c=r.hasPointerCapture,v=r.releasePointerCapture;try{r.setPointerCapture=function(){},r.hasPointerCapture=function(){return!0},r.releasePointerCapture=function(){};let b=(h,n)=>new PointerEvent(h,{bubbles:!0,cancelable:!0,composed:!0,pointerId:1,pointerType:"mouse",isPrimary:!0,buttons:1,clientX:n,clientY:a});return t.dispatchEvent(b("pointerdown",s)),t.dispatchEvent(b("pointermove",s+1)),t.dispatchEvent(b("pointerup",s+1)),!0}catch{return!1}finally{r.setPointerCapture=d,r.hasPointerCapture=c,r.releasePointerCapture=v}}function tt(t){t=W(t),Dt(t);try{let e=Ut();if(typeof e=="function"){try{localStorage.setItem("dsh.sessions.current",JSON.stringify({sessionId:t}))}catch{}e(t),U()&&X(!1),y(".dsh-tb-modal-mask").forEach(s=>s.remove()),G();return}}catch{}try{localStorage.setItem("dsh.sessions.current",JSON.stringify({sessionId:t})),localStorage.setItem("dsh-taskboard.unstick",String(Date.now()))}catch{}location.reload()}async function Tt(t){let e=Z();if(Gt()){let s=await Xt(e,4500);if(s!==null){try{await g(`/tasks/${t}/sessions`,{method:"POST",body:JSON.stringify({sessionId:s,action:"link"})})}catch{}return await B(),j("\u2713 \u5DF2\u65B0\u5EFA\u5BF9\u8BDD\u5E76\u7ED1\u5B9A\u5230\u8BE5\u4EFB\u52A1"),!0}}try{let{sessionId:s}=await g(`/tasks/${t}/session`,{method:"POST",body:"{}"});return tt(s),!0}catch(s){return alert(`\u65B0\u5EFA\u5BF9\u8BDD\u5931\u8D25\uFF1A${s.message}`),!1}}function j(t){let e=document.createElement("div");e.className="dsh-tb-toast",e.textContent=t,document.body.appendChild(e),setTimeout(()=>e.remove(),1600)}function Qt(){var u;let t={};for(let i of P)t[i]=0;let e={};for(let i of z){t[i.status]!==void 0&&t[i.status]++;let f=i.repo||"\uFF08\u672A\u6307\u5B9A\uFF09";(e[f]||(e[f]=[])).push(i)}let s=z.length,a=t.done||0,r=s?Math.round(a/s*100):0,d={};for(let i of M)i.repo&&(d[u=i.repo]||(d[u]=[])).push(i);let c=[...z].sort((i,f)=>f.updatedAt-i.updatedAt).slice(0,10),b='<div class="dsh-tb-ov-stat dsh-tb-ov-add" id="dsh-tb-ov-new" title="\u65B0\u5EFA\u4EFB\u52A1"><div class="dsh-tb-ov-addbtn">\uFF0B \u65B0\u5EFA\u4EFB\u52A1</div></div>'+[["\u5168\u90E8\u4EFB\u52A1",s,"#e6edf3",""],...P.map(i=>[x[i].label,t[i]||0,x[i].color,i]),["\u5B8C\u6210\u7387",`${r}%`,"#3fb950",null]].map(([i,f,E,$])=>{let lt=i==="\u5B8C\u6210\u7387"?r:s?Math.round(f/s*100):0,st=$!==null?" dsh-tb-ov-stat-click":"",ct=$===""?"\u67E5\u770B\u5168\u90E8\u4EFB\u52A1":$?`\u67E5\u770B\u300C${i}\u300D\u7684\u4EFB\u52A1`:"";return`<div class="dsh-tb-ov-stat${st}" ${$!==null?`data-status="${$}"`:""} title="${ct}"><div class="n" style="color:${E}">${f}</div><div class="l">${i}</div><div class="mini"><i style="width:${lt}%;background:${E}"></i></div></div>`}).join(""),h;q.length?h=q.map(i=>{let f=e[i.path]||e[i.title]||[],E=P.map(L=>f.filter(O=>O.status===L).length),$=f.length,lt=E[P.indexOf("done")]||0,st=$?Math.round(lt/$*100):0,ct=$?P.map((L,O)=>E[O]?`<i style="width:${Math.round(E[O]/$*100)}%;background:${x[L].color}" title="${x[L].label} ${E[O]}"></i>`:"").join(""):"",ne=P.map((L,O)=>E[O]?`<span class="st"><i class="dot" style="background:${x[L].color}"></i>${x[L].label} <b>${E[O]}</b></span>`:"").join(""),ie=(d[i.path]||d[i.title]||[]).length,K=F[i.path],re=K?`<span class="branch">\u2387 ${l(K.branch)}</span>${K.dirty?`<span class="branch dirty" title="${K.dirty} \u4E2A\u672A\u63D0\u4EA4\u6587\u4EF6">\u25CF${K.dirty}</span>`:""}`:"",jt=(d[i.path]||d[i.title]||[]).slice(0,3).map(L=>`<div class="s" data-sid="${l(L.id)}" title="\u6253\u5F00\u4F1A\u8BDD ${l(L.id)}">\u25B8 ${l(L.title)}</div>`).join("");return`<div class="dsh-tb-ov-ws" data-repo="${l(i.path)}">
					...
					<div class="comp"><span class="pct">\u5B8C\u6210\u7387 ${st}%</span><div class="track"><i style="width:${st}%"></i></div></div>
					<div class="meta">${re}<span class="sess">\u4F1A\u8BDD ${ie}</span></div>
		...
		</div>`,o("#dsh-tb-ov-new")?.addEventListener("click",Ct),y(".dsh-tb-ov-stat[data-status]").forEach(i=>i.addEventListener("click",()=>{T.status=i.dataset.status,I="list",et(),k()})),y(".dsh-tb-ov-ws").forEach(i=>i.addEventListener("click",()=>{T.repo=i.dataset.repo;let f=o("#dsh-tb-repo-filter");f&&(f.value=T.repo),I="kanban",et(),k()})),y(".dsh-tb-ov-ws .s").forEach(i=>i.addEventListener("click",f=>{f.stopPropagation(),tt(i.dataset.sid)})),y(".dsh-tb-ov-item").forEach(i=>i.addEventListener("click",()=>{i.dataset.id&&J(i.dataset.id)})))}function Zt(t){let e=x[t.status]||x.todo,s=N[t.priority]||N.medium,a=[];a.push(Et(t)),t.status==="in_review"&&a.push('<span class="dsh-tb-pill" style="color:#bc8cff;font-weight:600">\u25C9 \u5F85 review</span>'),t.repo&&a.push(`<span class="dsh-tb-pill" style="color:#79c0ff">${l(H(t.repo))}</span>`),t.feature&&a.push(`<span class="dsh-tb-pill" style="color:#d2a8ff">${l(t.feature)}</span>`),a.push(kt(t)),t.review&&t.review!=="none"&&a.push(`<span class="dsh-tb-pill" style="color:${t.review==="approved"?"#3fb950":t.review==="rejected"?"#f85149":"#d29922"}">${at[t.review]}</span>`),t.test&&t.test!=="none"&&a.push(`<span class="dsh-tb-pill" style="color:${t.test==="passed"?"#3fb950":t.test==="failed"?"#f85149":"#d29922"}">${ot[t.test]}</span>`);let r=[];return t.sessionIds?.length&&r.push(`<span>\u4F1A\u8BDD ${t.sessionIds.length}</span>`),t.notes?.length&&r.push(`<span>\u8BC4\u8BBA ${t.notes.length}</span>`),r.push(`<span>${R(t.updatedAt)}</span>`),`<div class="dsh-tb-card" data-id="${l(t.id)}" style="border-left:3px solid ${e.color}">
		...
		</div>`}function te(){let t=o("#dsh-tb-body"),e=o("#dsh-tb-count");if(!t)return;t.innerHTML='<div class="dsh-tb-columns"></div>';let s=o(".dsh-tb-columns",t);e&&(e.textContent=`${z.length} \u4E2A\u4EFB\u52A1`);for(let[a,r]of Object.entries(x)){let d=document.createElement("div");d.className="dsh-tb-col";let c=z.filter(b=>b.status===a);d.innerHTML=`<div class="dsh-tb-col-head">${r.label} <b>${c.length}</b></div><div class="dsh-tb-col-body"></div>`;let v=o(".dsh-tb-col-body",d);c.length?c.forEach(b=>v.insertAdjacentHTML("beforeend",Zt(b))):v.innerHTML='<div class="dsh-tb-empty">\u2014</div>',s.appendChild(d)}y(".dsh-tb-card",s).forEach(a=>a.addEventListener("click",()=>J(a.dataset.id)))}function ee(){let t=o("#dsh-tb-body"),e=o("#dsh-tb-count");if(!t)return;if(e&&(e.textContent=`${z.length} \u4E2A\u4EFB\u52A1`),!z.length){t.innerHTML='<div class="dsh-tb-ov"><div class="dsh-tb-empty">\uFF08\u6682\u65E0\u4EFB\u52A1\uFF0C\u70B9\u300C\uFF0B \u65B0\u5EFA\u300D\u521B\u5EFA\uFF09</div></div>';return}let s=z.map(a=>`<tr data-id="${l(a.id)}">
			<td>${Wt(a)}</td>
			<td>${Et(a)}</td>
			...
			<td>${a.review!=="none"?`<span class="dsh-tb-pill" style="color:${a.review==="approved"?"#3fb950":a.review==="rejected"?"#f85149":"#d29922"}">${at[a.review]}</span>`:""}</td>
			<td>${a.test!=="none"?`<span class="dsh-tb-pill" style="color:${a.test==="passed"?"#3fb950":a.test==="failed"?"#f85149":"#d29922"}">${ot[a.test]}</span>`:""}</td>
			...
			<table class="dsh-tb-table">
		...
		</div>`,y("tr[data-id]",t).forEach(a=>a.addEventListener("click",()=>J(a.dataset.id)))}function et(){y(".dsh-tb-tab").forEach(e=>{e.dataset.tbView===I?e.classList.add("dsh-tb-tab-on"):e.classList.remove("dsh-tb-tab-on")});let t=o("#dsh-tb-toolbar");t&&(t.style.display=I==="overview"?"none":"flex"),I==="kanban"?te():I==="list"?ee():Qt()}function Ct(){let t=document.createElement("div");t.className="dsh-tb-modal-mask";let e=q.map(c=>`<option value="${l(c.path)}" title="${l(c.path)}">${l(c.path)}</option>`).join(""),s=(c,v)=>{o(c,t)?.classList.add("err"),v&&(o(v,t).hidden=!1)},a=()=>{t.innerHTML=`<div class="dsh-tb-modal" style="width:min(560px,92vw)">
				<h3>\u65B0\u5EFA\u4EFB\u52A1</h3>
				...
				<div class="dsh-tb-actions"><button data-act="cancel">\u53D6\u6D88</button></div>
			</div>`,document.body.appendChild(a);let r=it(s.sessions||M),d=o("#tb-pick-list",a),c=v=>{let b=String(v||"").trim().toLowerCase(),h=r.filter(u=>!b||String(u.title||"").toLowerCase().includes(b)),n={};for(let u of h){let i=u.repo||"\uFF08\u672A\u6307\u5B9A\uFF09";(n[i]||(n[i]=[])).push(u)}let p=Object.entries(n).sort((u,i)=>i[1].length-u[1].length).map(([u,i])=>(i.sort((f,E)=>(E.updatedAt||0)-(f.updatedAt||0)),`<div class="dsh-tb-pick-group">
							<div class="dsh-tb-pick-grouphead">${l(H(u))} <b>${i.length}</b></div>
						...
						</div>`)).join("");d.innerHTML=p||'<div class="dsh-tb-empty">\uFF08\u65E0\u5339\u914D\u4F1A\u8BDD\uFF09</div>',y(".dsh-tb-pick-item",a).forEach(u=>u.addEventListener("click",async()=>{let i=u.dataset.sid;try{let{task:f,injected:E,injectionNote:$}=await g(`/tasks/${t}/sessions`,{method:"POST",body:JSON.stringify({sessionId:i,action:"link"})});a.remove(),await k(),e(f),j(E?"\u2713 \u5DF2\u5173\u8054\u4F1A\u8BDD\uFF08\u5DF2\u6CE8\u5165\u4EFB\u52A1\u4E0A\u4E0B\u6587\uFF09":$||"\u5DF2\u5173\u8054\u4F1A\u8BDD")}catch(f){alert(`\u5173\u8054\u5931\u8D25\uFF1A${f.message}`)}}))};c(""),o("#tb-pick-search",a).addEventListener("input",()=>c(o("#tb-pick-search",a).value)),o('[data-act="cancel"]',a).addEventListener("click",()=>a.remove()),a.addEventListener("click",v=>{v.target===a&&a.remove()}),o("#tb-pick-search",a).focus()}).catch(()=>alert("\u83B7\u53D6\u4F1A\u8BDD\u5217\u8868\u5931\u8D25"))}async function J(t){let{task:e}=await g(`/tasks/${t}`);dt(e)}function dt(t){let e=t.id,s=document.createElement("div");s.className="dsh-tb-modal-mask";let a=M.map(n=>`<option value="${l(n.id)}">${l(n.title||n.id)}${n.repo?` \xB7 ${l(H(n.repo))}`:""}</option>`).join("");s.innerHTML=`<div class="dsh-tb-modal" style="width:min(700px,92vw)">
			<h3>${l(t.title)}</h3>
			...
			<div class="dsh-tb-row">
				<div class="dsh-tb-field"><label>Review</label><select id="tb-d-review">${Object.entries(at).map(([n,p])=>`<option value="${n}" ${n===t.review?"selected":""}>${p}</option>`).join("")}</select></div>
				<div class="dsh-tb-field"><label>\u6D4B\u8BD5</label><select id="tb-d-test">${Object.entries(ot).map(([n,p])=>`<option value="${n}" ${n===t.test?"selected":""}>${p}</option>`).join("")}</select></div>
				<div class="dsh-tb-field"><label>\u6807\u7B7E</label><input id="tb-d-labels" value="${l((t.labels||[]).join(", "))}" placeholder="\u9017\u53F7\u5206\u9694" list="tb-labels-datalist2" /><datalist id="tb-labels-datalist2">${nt.map(n=>`<option value="${l(n)}"></option>`).join("")}</datalist></div>
			</div>
			...
			<div class="dsh-tb-field"><label>\u6C60\u5185\u4F1A\u8BDD\uFF08\u5171\u4EAB\u4E0A\u4E0B\u6587\u53EF\u8BBF\u95EE\u8005\uFF1B\u70B9\u51FB\u6253\u5F00\u53EF\u7EE7\u7EED\uFF09</label>
				<div id="tb-d-sessions">${(t.sessionIds||[]).map(n=>{let p=W(n),u=M.find(f=>f.id===p),i=u?l(u.title||p):'<span class="dsh-tb-sess-gone">\uFF08\u4F1A\u8BDD\u5DF2\u4E0D\u5B58\u5728\uFF09</span>';return`<div class="dsh-tb-sess"><span class="dsh-tb-sess-title" title="${l(p)}">${i}${u&&u.running?' <span class="dsh-tb-run">\u25CF \u8FD0\u884C\u4E2D</span>':""}</span>${u?`<button data-sid="${l(p)}" data-act="open" class="dsh-tb-open">\u6253\u5F00</button>`:""}<button data-sid="${l(n)}" data-act="unlink">\u89E3\u9664</button></div>`}).join("")||'<div class="dsh-tb-note">\uFF08\u672A\u5173\u8054\u4F1A\u8BDD\uFF09</div>'}</div>
		...
		</div>`,document.body.appendChild(s);let r=async()=>{let n=o("#tb-d-ctx-body",s);if(n)try{let{context:p}=await g(`/tasks/${e}/context`);n.textContent=p||"\uFF08\u6682\u65E0\u5171\u4EAB\u4E0A\u4E0B\u6587\uFF09"}catch{n.textContent="\uFF08\u83B7\u53D6\u5931\u8D25\uFF09"}},d=async()=>{try{let{context:n}=await g(`/tasks/${e}/context`);await navigator.clipboard.writeText(n||""),j("\u2713 \u5DF2\u590D\u5236\u6700\u65B0\u5171\u4EAB\u4E0A\u4E0B\u6587")}catch{try{let{context:n}=await g(`/tasks/${e}/context`),p=document.createElement("textarea");p.value=n||"",document.body.appendChild(p),p.select(),document.execCommand("copy"),p.remove(),j("\u2713 \u5DF2\u590D\u5236\u6700\u65B0\u5171\u4EAB\u4E0A\u4E0B\u6587")}catch(n){alert(`\u590D\u5236\u5931\u8D25\uFF1A${n.message}`)}}};r(),o("#tb-d-ctx-refresh",s)?.addEventListener("click",r),o("#tb-d-ctx-copy",s)?.addEventListener("click",d);let c=()=>({repo:o("#tb-d-repo",s).value.trim(),feature:o("#tb-d-feature",s).value.trim(),priority:o("#tb-d-priority",s).value,progress:Number(o("#tb-d-progress",s).value),review:o("#tb-d-review",s).value,test:o("#tb-d-test",s).value,labels:o("#tb-d-labels",s).value.split(",").map(n=>n.trim()).filter(Boolean),description:o("#tb-d-desc",s).value}),v=()=>s.remove(),b=n=>{s.remove(),dt(n)},h=async n=>{let{task:p}=await g(`/tasks/${e}`,{method:"PATCH",body:JSON.stringify(n)});return await k(),p};s.addEventListener("click",n=>{n.target===s&&v()}),o('[data-act="cancel"]',s).addEventListener("click",v),o('[data-act="save"]',s).addEventListener("click",async()=>{try{let n=await h(c());b(n),j("\u2713 \u5DF2\u4FDD\u5B58")}catch(n){alert(`\u4FDD\u5B58\u5931\u8D25\uFF1A${n.message}`)}}),o('[data-act="del"]',s).addEventListener("click",async()=>{if(confirm(`\u5220\u9664\u4EFB\u52A1\u300C${t.title}\u300D\uFF1F`))try{await g(`/tasks/${e}`,{method:"DELETE"}),v(),await B()}catch(n){alert(`\u5220\u9664\u5931\u8D25\uFF1A${n.message}`)}}),o('[data-act="copyctx"]',s)?.addEventListener("click",d),o("#tb-d-note",s).addEventListener("keydown",async n=>{if(n.key==="Enter"&&(n.ctrlKey||n.metaKey)){let p=o("#tb-d-note",s).value.trim();if(!p)return;try{let{task:u}=await g(`/tasks/${e}/notes`,{method:"POST",body:JSON.stringify({text:p})});await k(),b(u),j("\u2713 \u8FDB\u5C55\u5DF2\u8BB0\u5F55")}catch(u){alert(`\u8BB0\u5F55\u5931\u8D25\uFF1A${u.message}`)}}}),o("#tb-d-sess-open",s)?.addEventListener("click",()=>{se(e,n=>{s.remove(),dt(n)})}),y("#tb-d-sessions [data-act]",s).forEach(n=>n.addEventListener("click",async()=>{let p=n.dataset.sid;if(n.dataset.act==="open")tt(p);else{let{task:u}=await g(`/tasks/${e}/sessions`,{method:"POST",body:JSON.stringify({sessionId:p,action:"unlink"})});await k(),b(u),j("\u5DF2\u89E3\u9664\u4F1A\u8BDD")}})),o("#tb-d-sess-new",s)?.addEventListener("click",async()=>{await Tt(e)}),y(".dsh-tb-statusbtn",s).forEach(n=>n.addEventListener("click",async()=>{try{let p=await h({status:n.dataset.status});b(p),j("\u2713 \u72B6\u6001\u5DF2\u66F4\u65B0")}catch(p){alert(`\u72B6\u6001\u6D41\u8F6C\u5931\u8D25\uFF1A${p.message}`)}})),o('[data-act="split"]',s)?.addEventListener("click",()=>{let n=document.createElement("div");n.className="dsh-tb-modal-mask",n.innerHTML=`<div class="dsh-tb-modal">
			...
			</div>`,document.body.appendChild(n);let p=()=>n.remove();o('[data-act="cancel"]',n).addEventListener("click",p),n.addEventListener("click",u=>{u.target===n&&p()}),o('[data-act="do"]',n).addEventListener("click",async()=>{let u=o("#tb-split-titles",n).value.split(`
`).map(i=>i.trim()).filter(Boolean);if(!u.length){o("#tb-split-titles",n).focus();return}try{let{tasks:i}=await g(`/tasks/${e}/split`,{method:"POST",body:JSON.stringify({titles:u})});p(),await B(),j(`\u2713 \u5DF2\u62C6\u5206\u4E3A ${i.length} \u4E2A\u5B50\u4EFB\u52A1`),i?.[0]&&J(i[0].id)}catch(i){alert(`\u62C6\u5206\u5931\u8D25\uFF1A${i.message}`)}}),o("#tb-split-titles",n).focus()})}gt(),xt(),$t(),Bt(),new MutationObserver(()=>{gt(),xt(),$t()}).observe(document.body,{childList:!0,subtree:!0}),document.addEventListener("keydown",t=>{t.ctrlKey&&t.shiftKey&&(t.key==="B"||t.key==="b")&&(t.preventDefault(),X())}),document.addEventListener("click",t=>{if(!U())return;let e=t.target;e instanceof Element&&(e.closest(`[${A}]`)||e.closest('[data-pane="sidebar"], [class*="sidebarCol"]')&&X(!1))});function ae(){let t=ft();if(!t||t.clientWidth===0)return;let e=t.style.width;try{t.style.width=`${t.clientWidth-2}px`,setTimeout(()=>{t.style.width=e||"";try{window.dispatchEvent(new Event("resize"))}catch{}},180)}catch{}}try{localStorage.getItem("dsh-taskboard.unstick")!==null&&(localStorage.removeItem("dsh-taskboard.unstick"),setTimeout(()=>{Lt()},900),setTimeout(()=>{Lt()},2e3))}catch{}let oe=[400,1e3,2e3],zt=()=>{for(let t of oe)setTimeout(ae,t)};document.readyState==="loading"?window.addEventListener("load",zt):zt()})();})();
