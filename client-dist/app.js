(()=>{(()=>{if(window.__dshTaskboardLoaded)return;window.__dshTaskboardLoaded=!0;let $t="/taskboard/api",g={todo:{label:"\u5F85\u529E",color:"#8b949e"},in_progress:{label:"\u8FDB\u884C\u4E2D",color:"#3478f6"},in_review:{label:"\u8BC4\u5BA1\u4E2D",color:"#bc8cff"},blocked:{label:"\u5DF2\u963B\u585E",color:"#f85149"},done:{label:"\u5DF2\u5B8C\u6210",color:"#3fb950"}},N=["todo","in_progress","in_review","blocked","done"],P={low:{label:"\u4F4E",color:"#8b949e"},medium:{label:"\u4E2D",color:"#d29922"},high:{label:"\u9AD8",color:"#e3862e"},urgent:{label:"\u7D27\u6025",color:"#f85149"}},tt={none:"\u2014",pending:"\u8BC4\u5BA1\u5F85\u5904\u7406",approved:"\u8BC4\u5BA1\u901A\u8FC7",rejected:"\u8BC4\u5BA1\u9A73\u56DE"},et={none:"\u2014",pending:"\u6D4B\u8BD5\u5F85\u5904\u7406",passed:"\u6D4B\u8BD5\u901A\u8FC7",failed:"\u6D4B\u8BD5\u5931\u8D25"},rt=["#79c0ff","#d2a8ff","#7ee787","#ffa657","#ff7b72","#f2cc60","#a5d6ff","#ffd7a8"],S="data-dsh-taskboard-active",C="data-dsh-taskboard-entry",O="data-dsh-taskboard-view",D="data-dsh-ssh-active",kt='<svg viewBox="0 0 16 16" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="2" y="2.5" width="12" height="11" rx="1.5"/><path d="M2 6.5h12M6.5 6.5v7"/></svg>',o=(t,e=document)=>e.querySelector(t),y=(t,e=document)=>[...e.querySelectorAll(t)],r=t=>String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e]),H=t=>{let e=String(t||"").split("/").filter(Boolean);return e.length?e[e.length-1]:String(t||"")},Et=t=>{let e=0;for(let s of String(t))e=e*31+s.charCodeAt(0)>>>0;return rt[e%rt.length]},R=t=>{if(!t)return"";let e=new Date(t),s=Date.now()-t;return s<6e4?"\u521A\u521A":s<36e5?`${Math.floor(s/6e4)} \u5206\u949F\u524D`:s<864e5?`${Math.floor(s/36e5)} \u5C0F\u65F6\u524D`:`${e.getMonth()+1}/${e.getDate()} ${String(e.getHours()).padStart(2,"0")}:${String(e.getMinutes()).padStart(2,"0")}`},j=[],A=[],q=[],st=[],z={q:"",repo:"",priority:"",status:"",label:""},I="overview",U={};async function x(t,e={}){let s=await fetch($t+t,{headers:{"content-type":"application/json"},...e}),d=await s.json().catch(()=>({}));if(!s.ok)throw new Error(d.error||`HTTP ${s.status}`);return d}let Lt=`
<style>
...
<div id="dsh-tb-view" ${O}="">
  ...
  <div class="dsh-tb-body" id="dsh-tb-body"></div>
</div>`;function nt(){let t=document.querySelector('[data-pane="sidebar"], [class*="sidebarCol"]');return t===null?void 0:t.querySelector('[class*="logoRow"]')?.parentElement??t.firstElementChild}function lt(t){let e=t.querySelector('button[class*="newSession"]');if(e!==null)return e;for(let s of t.children)if(s.tagName==="BUTTON")return s}function ct(){return document.querySelector('[data-pane="conversation"]')??document.querySelector('[class*="centerCol"]')??void 0}function bt(t){return[...t.classList].some(e=>/collapsed/i.test(e))}let m=null,V=null;function X(){return document.documentElement.hasAttribute(S)}function pt(){m!==null&&(X()?m.setAttribute("data-active",""):m.removeAttribute("data-active"))}function F(t){let e=t!==void 0?t:!X();e&&document.documentElement.removeAttribute(D),e?document.documentElement.setAttribute(S,""):document.documentElement.removeAttribute(S),pt(),e&&J()}function ht(){if(V!==null)return;let t=ct();t!==void 0&&(V=document.createElement("div"),V.setAttribute(O,""),V.innerHTML=Tt,t.appendChild(V),Mt())}function ft(){let t=nt();if(t===void 0||(m===null&&(m=document.createElement("button"),m.type="button",m.setAttribute(C,""),m.innerHTML=`<span>${kt}</span><span>\u4EFB\u52A1\u770B\u677F</span>`,m.title="\u4EFB\u52A1\u770B\u677F",m.addEventListener("click",()=>F()),pt()),m.parentElement===t))return;let e=lt(t),s=e?.closest('[class*="logoRow"]'),d=s!=null&&s.parentElement===t?s:e;t.insertBefore(m,d?.nextElementSibling??null),(()=>{m!==null&&(bt(t)?m.setAttribute("data-icon-only",""):m.removeAttribute("data-icon-only"))})()}let zt=15,w=null,L=null,B=!1,ut=null;function St(){return x("/sessions").then(t=>t.sessions||[]).catch(()=>A)}function jt(t){return[...t||[]].filter(e=>e&&e.updatedAt).sort((e,s)=>(s.updatedAt||0)-(e.updatedAt||0)).slice(0,zt)}function Ct(){try{let t=JSON.parse(localStorage.getItem("dsh.sessions.current")||"null");return t&&t.sessionId?t.sessionId:null}catch{return null}}function at(t){if(L===null)return;let e=jt(t),s=o("#dsh-recent-cnt");if(s&&(s.textContent=String(e.length)),!e.length){L.innerHTML='<div class="dsh-recent-empty">\uFF08\u6682\u65E0\u6700\u8FD1\u4F1A\u8BDD\uFF09</div>';return}let d=Ct();L.innerHTML=e.map(n=>{let b=n.title&&String(n.title).trim()?n.title:n.id,c=n.id===d,u=c?"":n.running?'<span class="dot run"></span>':'<span class="dot idle"></span>';return`<div class="dsh-recent-row${c?" cur":""}" data-sid="${r(n.id)}" title="\u6253\u5F00\u5E76\u7EE7\u7EED\uFF1A${r(b)}">
				<span class="lbl">
					${u}
					<span class="t">${r(b)}</span>
				</span>
				<span class="meta">
					${n.preview?`<span class="prev">${r(n.preview)}</span>`:""}
					${n.repo?`<span class="repo">${r(H(n.repo))}</span>`:""}
					<span class="tm">${R(n.updatedAt)}</span>
				</span>
			</div>`}).join(""),y(".dsh-recent-row",L).forEach(n=>n.addEventListener("click",()=>{W(n.dataset.sid)}))}function G(){St().then(t=>at(t)).catch(()=>at(A))}function vt(){let t=nt();if(t===void 0||(w===null&&(w=document.createElement("div"),w.id="dsh-recent",w.className="dsh-recent",w.innerHTML=`
				...
				<div class="dsh-recent-body" id="dsh-recent-body"></div>`,L=o("#dsh-recent-body",w),o("#dsh-recent-head",w).addEventListener("click",()=>{B=!B,w&&w.classList.toggle("collapsed",B),L&&(L.style.display=B?"none":"")}),o("#dsh-recent-refresh",w).addEventListener("click",n=>{n.stopPropagation(),G()}),at(A),G()),w.parentElement===t))return;let e=m??lt(t);t.insertBefore(w,e?.nextElementSibling??null),L&&(L.style.maxHeight="46vh",L.style.overflowY="auto"),(()=>{L&&(L.style.display=B?"none":""),w&&w.classList.toggle("collapsed",B)})(),(()=>{w&&(bt(t)?w.style.display="none":w.style.display="")})()}function At(){ut===null&&(ut=setInterval(()=>{document.hidden||G()},6e4))}function Mt(){o("#dsh-tb-new")?.addEventListener("click",wt),o("#dsh-tb-refresh")?.addEventListener("click",()=>J()),o("#dsh-tb-search")?.addEventListener("input",()=>{z.q=o("#dsh-tb-search").value.trim(),k()}),o("#dsh-tb-repo-filter")?.addEventListener("change",()=>{z.repo=o("#dsh-tb-repo-filter").value,k()}),o("#dsh-tb-priority-filter")?.addEventListener("change",()=>{z.priority=o("#dsh-tb-priority-filter").value,k()}),o("#dsh-tb-status-filter")?.addEventListener("change",()=>{z.status=o("#dsh-tb-status-filter").value,k()}),o("#dsh-tb-label-filter")?.addEventListener("change",()=>{z.label=o("#dsh-tb-label-filter").value,k()}),y(".dsh-tb-tab").forEach(t=>t.addEventListener("click",()=>{I=t.dataset.tbView,Q()}))}async function k(){let t=new URLSearchParams;for(let s of["q","repo","priority","status","label"])z[s]&&t.set(s,z[s]);j=(await x(`/tasks?${t}`)).tasks||[],Q()}async function _t(){try{let[t,e]=await Promise.all([x("/sessions"),x("/workspaces")]);A=t.sessions||[],q=e.workspaces||[];let s=o("#dsh-tb-repo-filter");if(s){let p=s.value;s.innerHTML='<option value="">\u5168\u90E8\u4ED3\u5E93</option>'+q.map(h=>`<option value="${r(h.path)}" title="${r(h.path)}">${r(H(h.path))}</option>`).join(""),p&&[...s.options].some(h=>h.value===p)&&(s.value=p)}let d=await x("/tasks").catch(()=>({tasks:[]})),n=new Set;for(let p of d.tasks||[])for(let h of p.labels||[])n.add(h);st=[...n].sort();let b=o("#dsh-tb-label-filter");if(b){let p=b.value;b.innerHTML='<option value="">\u5168\u90E8\u6807\u7B7E</option>'+st.map(h=>`<option value="${r(h)}">${r(h)}</option>`).join(""),p&&[...b.options].some(h=>h.value===p)&&(b.value=p)}let c=o("#dsh-tb-priority-filter");c&&!c.options.length&&(c.innerHTML='<option value="">\u5168\u90E8\u4F18\u5148\u7EA7</option>'+Object.entries(P).map(([p,h])=>`<option value="${p}">${h.label}</option>`).join(""));let u=o("#dsh-tb-status-filter");u&&!u.options.length&&(u.innerHTML='<option value="">\u5168\u90E8\u72B6\u6001</option>'+Object.entries(g).map(([p,h])=>`<option value="${p}">${h.label}</option>`).join(""))}catch{}}async function Ot(){for(let t of q)if(!(!t.path||U[t.path]!==void 0))try{let e=await fetch(`/ide/api/git?op=status&path=${encodeURIComponent(t.path)}`).then(s=>s.json());U[t.path]=e.git?{branch:e.branch?.name||"(detached)",dirty:(e.files||[]).length}:null}catch{U[t.path]=null}}function J(){return _t().then(()=>Ot()).then(k).catch(k)}function xt(t){return(t.labels||[]).map(e=>`<span class="dsh-tb-pill" style="color:${Et(e)}">${r(e)}</span>`).join("")}function Ht(t){let e=g[t.status]||g.todo;return`<span class="dsh-tb-pill" style="color:${e.color}">${e.label}</span>`}function gt(t){let e=P[t.priority]||P.medium;return`<span class="dsh-tb-pill" style="color:${e.color}">${e.label}</span>`}function It(t){let e=(s,d,n)=>`<button type="button" class="dsh-tb-statusbtn ${n||""}" data-status="${s}">${d}</button>`;switch(t.status){case"todo":return`${e("in_progress","\u25B6 \u5F00\u59CB\u6267\u884C")} ${e("blocked","\u26D4 \u963B\u585E","ghost")}`;case"in_progress":return`${e("in_review","\u63D0\u4EA4\u8BC4\u5BA1")} ${e("blocked","\u26D4 \u963B\u585E","ghost")}`;case"in_review":return`${e("done","\u2705 \u786E\u8BA4\u5B8C\u6210","primary")} ${e("in_progress","\u21A9 \u9000\u56DE\u4FEE\u6539")}`;case"done":return`${e("in_progress","\u21A9 \u91CD\u65B0\u6253\u5F00","ghost")}`;case"blocked":return`${e("in_progress","\u25B6 \u6062\u590D\u8FDB\u884C")}`;default:return""}}function Nt(){let t=document.querySelectorAll('[class*="sessionRow"], [class*="root"]');for(let e of t){let s=null;for(let n of Object.keys(e))if(n.startsWith("__reactFiber$")){s=n;break}if(s===null)continue;let d=e[s];for(let n=0;n<30&&d!==null;n++){let b=d.memoizedProps;if(b!==null&&typeof b=="object"&&typeof b.open=="function"&&typeof b.startSession=="function")return b.open;d=d.return}}return null}function W(t){try{let e=Nt();if(typeof e=="function"){e(t),X()&&F(!1),y(".dsh-tb-modal-mask").forEach(s=>s.remove()),G();return}}catch{}try{localStorage.setItem("dsh.sessions.current",JSON.stringify({sessionId:t}))}catch{}location.reload()}function M(t){let e=document.createElement("div");e.className="dsh-tb-toast",e.textContent=t,document.body.appendChild(e),setTimeout(()=>e.remove(),1600)}function Pt(){var v;let t={};for(let i of N)t[i]=0;let e={};for(let i of j){t[i.status]!==void 0&&t[i.status]++;let f=i.repo||"\uFF08\u672A\u6307\u5B9A\uFF09";(e[f]||(e[f]=[])).push(i)}let s=j.length,d=t.done||0,n=s?Math.round(d/s*100):0,b={};for(let i of A)i.repo&&(b[v=i.repo]||(b[v]=[])).push(i);let c=[...j].sort((i,f)=>f.updatedAt-i.updatedAt).slice(0,10),p='<div class="dsh-tb-ov-stat dsh-tb-ov-add" id="dsh-tb-ov-new" title="\u65B0\u5EFA\u4EFB\u52A1"><div class="dsh-tb-ov-addbtn">\uFF0B \u65B0\u5EFA\u4EFB\u52A1</div></div>'+[["\u5168\u90E8\u4EFB\u52A1",s,"#e6edf3",""],...N.map(i=>[g[i].label,t[i]||0,g[i].color,i]),["\u5B8C\u6210\u7387",`${n}%`,"#3fb950",null]].map(([i,f,E,$])=>{let dt=i==="\u5B8C\u6210\u7387"?n:s?Math.round(f/s*100):0,Z=$!==null?" dsh-tb-ov-stat-click":"",it=$===""?"\u67E5\u770B\u5168\u90E8\u4EFB\u52A1":$?`\u67E5\u770B\u300C${i}\u300D\u7684\u4EFB\u52A1`:"";return`<div class="dsh-tb-ov-stat${Z}" ${$!==null?`data-status="${$}"`:""} title="${it}"><div class="n" style="color:${E}">${f}</div><div class="l">${i}</div><div class="mini"><i style="width:${dt}%;background:${E}"></i></div></div>`}).join(""),h;q.length?h=q.map(i=>{let f=e[i.path]||e[i.title]||[],E=N.map(T=>f.filter(_=>_.status===T).length),$=f.length,dt=E[N.indexOf("done")]||0,Z=$?Math.round(dt/$*100):0,it=$?N.map((T,_)=>E[_]?`<i style="width:${Math.round(E[_]/$*100)}%;background:${g[T].color}" title="${g[T].label} ${E[_]}"></i>`:"").join(""):"",Jt=N.map((T,_)=>E[_]?`<span class="st"><i class="dot" style="background:${g[T].color}"></i>${g[T].label} <b>${E[_]}</b></span>`:"").join(""),Wt=(b[i.path]||b[i.title]||[]).length,K=U[i.path],Kt=K?`<span class="branch">\u2387 ${r(K.branch)}</span>${K.dirty?`<span class="branch dirty" title="${K.dirty} \u4E2A\u672A\u63D0\u4EA4\u6587\u4EF6">\u25CF${K.dirty}</span>`:""}`:"",yt=(b[i.path]||b[i.title]||[]).slice(0,3).map(T=>`<div class="s" data-sid="${r(T.id)}" title="\u6253\u5F00\u4F1A\u8BDD ${r(T.id)}">\u25B8 ${r(T.title)}</div>`).join("");return`<div class="dsh-tb-ov-ws" data-repo="${r(i.path)}">
					<div class="ws-head"><h4>${r(i.title)}</h4><span class="ws-total">${$} \u4E2A\u4EFB\u52A1</span></div>
					<div class="path">${r(i.path||"")}</div>
					${$?`<div class="stack">${it}</div><div class="ws-stats">${Jt}</div>`:'<div class="ws-stats" style="color:var(--dsw-alias-label-secondary,#9aa7b4)">\u6682\u65E0\u4EFB\u52A1\uFF0C\u70B9\u300C\uFF0B \u65B0\u5EFA\u300D\u521B\u5EFA</div>'}
					<div class="comp"><span class="pct">\u5B8C\u6210\u7387 ${Z}%</span><div class="track"><i style="width:${Z}%"></i></div></div>
					<div class="meta">${Kt}<span class="sess">\u4F1A\u8BDD ${Wt}</span></div>
					${yt?`<div class="sesslist">${yt}</div>`:""}
				</div>`}).join(""):h='<div class="dsh-tb-empty">\uFF08\u6682\u65E0\u5DE5\u4F5C\u533A\uFF09</div>';let a=c.length?c.map(i=>{let f=g[i.status]||g.todo;return`<div class="dsh-tb-ov-item" data-id="${r(i.id)}">
		...
		</div>`,o("#dsh-tb-ov-new")?.addEventListener("click",wt),y(".dsh-tb-ov-stat[data-status]").forEach(i=>i.addEventListener("click",()=>{z.status=i.dataset.status,I="list",Q(),k()})),y(".dsh-tb-ov-ws").forEach(i=>i.addEventListener("click",()=>{z.repo=i.dataset.repo;let f=o("#dsh-tb-repo-filter");f&&(f.value=z.repo),I="kanban",Q(),k()})),y(".dsh-tb-ov-ws .s").forEach(i=>i.addEventListener("click",f=>{f.stopPropagation(),W(i.dataset.sid)})),y(".dsh-tb-ov-item").forEach(i=>i.addEventListener("click",()=>{i.dataset.id&&Y(i.dataset.id)})))}function Rt(t){let e=g[t.status]||g.todo,s=P[t.priority]||P.medium,d=[];d.push(gt(t)),t.status==="in_review"&&d.push('<span class="dsh-tb-pill" style="color:#bc8cff;font-weight:600">\u25C9 \u5F85 review</span>'),t.repo&&d.push(`<span class="dsh-tb-pill" style="color:#79c0ff">${r(H(t.repo))}</span>`),t.feature&&d.push(`<span class="dsh-tb-pill" style="color:#d2a8ff">${r(t.feature)}</span>`),d.push(xt(t)),t.review&&t.review!=="none"&&d.push(`<span class="dsh-tb-pill" style="color:${t.review==="approved"?"#3fb950":t.review==="rejected"?"#f85149":"#d29922"}">${tt[t.review]}</span>`),t.test&&t.test!=="none"&&d.push(`<span class="dsh-tb-pill" style="color:${t.test==="passed"?"#3fb950":t.test==="failed"?"#f85149":"#d29922"}">${et[t.test]}</span>`);let n=[];return t.sessionIds?.length&&n.push(`<span>\u4F1A\u8BDD ${t.sessionIds.length}</span>`),t.notes?.length&&n.push(`<span>\u8BC4\u8BBA ${t.notes.length}</span>`),n.push(`<span>${R(t.updatedAt)}</span>`),`<div class="dsh-tb-card" data-id="${r(t.id)}" style="border-left:3px solid ${e.color}">
			...
			<div class="dsh-tb-card-foot">${n.join(" \xB7 ")}</div>
		</div>`}function qt(){let t=o("#dsh-tb-body"),e=o("#dsh-tb-count");if(!t)return;t.innerHTML='<div class="dsh-tb-columns"></div>';let s=o(".dsh-tb-columns",t);e&&(e.textContent=`${j.length} \u4E2A\u4EFB\u52A1`);for(let[d,n]of Object.entries(g)){let b=document.createElement("div");b.className="dsh-tb-col";let c=j.filter(p=>p.status===d);b.innerHTML=`<div class="dsh-tb-col-head">${n.label} <b>${c.length}</b></div><div class="dsh-tb-col-body"></div>`;let u=o(".dsh-tb-col-body",b);c.length?c.forEach(p=>u.insertAdjacentHTML("beforeend",Rt(p))):u.innerHTML='<div class="dsh-tb-empty">\u2014</div>',s.appendChild(b)}y(".dsh-tb-card",s).forEach(d=>d.addEventListener("click",()=>Y(d.dataset.id)))}function Bt(){let t=o("#dsh-tb-body"),e=o("#dsh-tb-count");if(!t)return;if(e&&(e.textContent=`${j.length} \u4E2A\u4EFB\u52A1`),!j.length){t.innerHTML='<div class="dsh-tb-ov"><div class="dsh-tb-empty">\uFF08\u6682\u65E0\u4EFB\u52A1\uFF0C\u70B9\u300C\uFF0B \u65B0\u5EFA\u300D\u521B\u5EFA\uFF09</div></div>';return}let s=j.map(d=>`<tr data-id="${r(d.id)}">
			<td>${Ht(d)}</td>
			<td>${gt(d)}</td>
			<td style="max-width:340px"><div style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${r(d.title)}</div></td>
			<td>${d.repo?`<span class="dsh-tb-pill" style="color:#79c0ff">${r(H(d.repo))}</span>`:""}</td>
			<td style="max-width:140px"><div style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${r(d.feature||"")}</div></td>
			<td>${xt(d)}</td>
			...
			<td style="white-space:nowrap">${R(d.updatedAt)}</td>
		...
		</div>`,y("tr[data-id]",t).forEach(d=>d.addEventListener("click",()=>Y(d.dataset.id)))}function Q(){y(".dsh-tb-tab").forEach(e=>{e.dataset.tbView===I?e.classList.add("dsh-tb-tab-on"):e.classList.remove("dsh-tb-tab-on")});let t=o("#dsh-tb-toolbar");t&&(t.style.display=I==="overview"?"none":"flex"),I==="kanban"?qt():I==="list"?Bt():Pt()}function wt(){let t=document.createElement("div");t.className="dsh-tb-modal-mask";let e=q.map(c=>`<option value="${r(c.path)}" title="${r(c.path)}">${r(c.path)}</option>`).join(""),s=(c,u)=>{o(c,t)?.classList.add("err"),u&&(o(u,t).hidden=!1)},d=()=>{t.innerHTML=`<div class="dsh-tb-modal" style="width:min(560px,92vw)">
				<h3>\u65B0\u5EFA\u4EFB\u52A1</h3>
			...
			</div>`,o('[data-act="newsess"]',t).addEventListener("click",async()=>{try{let{sessionId:u}=await x(`/tasks/${c.id}/session`,{method:"POST",body:"{}"});W(u)}catch(u){alert(`\u65B0\u5EFA\u4F1A\u8BDD\u5931\u8D25\uFF1A${u.message}`)}}),o('[data-act="bind"]',t).addEventListener("click",()=>{t.remove(),Y(c.id)}),o('[data-act="detail"]',t).addEventListener("click",()=>{t.remove(),Y(c.id)}),o('[data-act="close"]',t).addEventListener("click",()=>t.remove())},b=()=>t.remove();t.addEventListener("click",c=>{c.target===t&&b()}),document.body.appendChild(t),d()}function Yt(t,e){x("/sessions").then(s=>{let d=document.createElement("div");d.className="dsh-tb-modal-mask",d.innerHTML=`<div class="dsh-tb-modal" style="width:min(600px,92vw)">
				<h3>\u9009\u62E9\u4F1A\u8BDD\u5173\u8054</h3>
			...
			</div>`,document.body.appendChild(d);let n=s.sessions||A,b=o("#tb-pick-list",d),c=u=>{let p=String(u||"").trim().toLowerCase(),h=n.filter(v=>!p||String(v.title||"").toLowerCase().includes(p)),a={};for(let v of h){let i=v.repo||"\uFF08\u672A\u6307\u5B9A\uFF09";(a[i]||(a[i]=[])).push(v)}let l=Object.entries(a).sort((v,i)=>i[1].length-v[1].length).map(([v,i])=>(i.sort((f,E)=>(E.updatedAt||0)-(f.updatedAt||0)),`<div class="dsh-tb-pick-group">
							<div class="dsh-tb-pick-grouphead">${r(H(v))} <b>${i.length}</b></div>
							${i.map(f=>`<div class="dsh-tb-pick-item" data-sid="${r(f.id)}">
								<div class="t">${r(f.title||f.id)}${f.running?' <span class="dsh-tb-run">\u25CF \u8FD0\u884C\u4E2D</span>':""}</div>
								<div class="m">${f.updatedAt?R(f.updatedAt):""}</div>
							</div>`).join("")}
						</div>`)).join("");b.innerHTML=l||'<div class="dsh-tb-empty">\uFF08\u65E0\u5339\u914D\u4F1A\u8BDD\uFF09</div>',y(".dsh-tb-pick-item",d).forEach(v=>v.addEventListener("click",async()=>{let i=v.dataset.sid;try{let{task:f,injected:E,injectionNote:$}=await x(`/tasks/${t}/sessions`,{method:"POST",body:JSON.stringify({sessionId:i,action:"link"})});d.remove(),await k(),e(f),M(E?"\u2713 \u5DF2\u5173\u8054\u4F1A\u8BDD\uFF08\u5DF2\u6CE8\u5165\u4EFB\u52A1\u4E0A\u4E0B\u6587\uFF09":$||"\u5DF2\u5173\u8054\u4F1A\u8BDD")}catch(f){alert(`\u5173\u8054\u5931\u8D25\uFF1A${f.message}`)}}))};c(""),o("#tb-pick-search",d).addEventListener("input",()=>c(o("#tb-pick-search",d).value)),o('[data-act="cancel"]',d).addEventListener("click",()=>d.remove()),d.addEventListener("click",u=>{u.target===d&&d.remove()}),o("#tb-pick-search",d).focus()}).catch(()=>alert("\u83B7\u53D6\u4F1A\u8BDD\u5217\u8868\u5931\u8D25"))}async function Y(t){let{task:e}=await x(`/tasks/${t}`);ot(e)}function ot(t){let e=t.id,s=document.createElement("div");s.className="dsh-tb-modal-mask";let d=A.map(a=>`<option value="${r(a.id)}">${r(a.title||a.id)}${a.repo?` \xB7 ${r(H(a.repo))}`:""}</option>`).join("");s.innerHTML=`<div class="dsh-tb-modal" style="width:min(700px,92vw)">
			<h3>${r(t.title)}</h3>
			...
			<div class="dsh-tb-row">
				<div class="dsh-tb-field"><label>\u72B6\u6001\uFF08AI \u81EA\u52A8\u6D41\u8F6C\uFF0C\u4F60\u53EA\u9700\u5728"\u8BC4\u5BA1\u4E2D"\u65F6\u5904\u7406\uFF09</label>
					<div class="dsh-tb-statusline">
						...
						<div class="dsh-tb-statusactions">${It(t)}</div>
					</div>
				</div>
				<div class="dsh-tb-field"><label>\u4F18\u5148\u7EA7</label><select id="tb-d-priority">${Object.entries(P).map(([a,l])=>`<option value="${a}" ${a===t.priority?"selected":""}>${l.label}</option>`).join("")}</select></div>
			...
			<div class="dsh-tb-row">
				...
				<div class="dsh-tb-field"><label>\u6807\u7B7E</label><input id="tb-d-labels" value="${r((t.labels||[]).join(", "))}" placeholder="\u9017\u53F7\u5206\u9694" list="tb-labels-datalist2" /><datalist id="tb-labels-datalist2">${st.map(a=>`<option value="${r(a)}"></option>`).join("")}</datalist></div>
			</div>
			...
			<div class="dsh-tb-field"><label>\u8FDB\u5C55\u8BB0\u5F55\uFF08${(t.notes||[]).length}\uFF09</label>
				<div id="tb-d-notes">${(t.notes||[]).map(a=>`<div class="dsh-tb-note"><span class="dsh-tb-note-time">${R(a.at)}</span><br/>${r(a.text)}</div>`).join("")||'<div class="dsh-tb-note">\uFF08\u6682\u65E0\u8BB0\u5F55\uFF09</div>'}</div>
		...
		</div>`,document.body.appendChild(s);let n=async()=>{let a=o("#tb-d-ctx-body",s);if(a)try{let{context:l}=await x(`/tasks/${e}/context`);a.textContent=l||"\uFF08\u6682\u65E0\u5171\u4EAB\u4E0A\u4E0B\u6587\uFF09"}catch{a.textContent="\uFF08\u83B7\u53D6\u5931\u8D25\uFF09"}},b=async()=>{try{let{context:a}=await x(`/tasks/${e}/context`);await navigator.clipboard.writeText(a||""),M("\u2713 \u5DF2\u590D\u5236\u6700\u65B0\u5171\u4EAB\u4E0A\u4E0B\u6587")}catch{try{let{context:a}=await x(`/tasks/${e}/context`),l=document.createElement("textarea");l.value=a||"",document.body.appendChild(l),l.select(),document.execCommand("copy"),l.remove(),M("\u2713 \u5DF2\u590D\u5236\u6700\u65B0\u5171\u4EAB\u4E0A\u4E0B\u6587")}catch(a){alert(`\u590D\u5236\u5931\u8D25\uFF1A${a.message}`)}}};n(),o("#tb-d-ctx-refresh",s)?.addEventListener("click",n),o("#tb-d-ctx-copy",s)?.addEventListener("click",b);let c=()=>({repo:o("#tb-d-repo",s).value.trim(),feature:o("#tb-d-feature",s).value.trim(),priority:o("#tb-d-priority",s).value,progress:Number(o("#tb-d-progress",s).value),review:o("#tb-d-review",s).value,test:o("#tb-d-test",s).value,labels:o("#tb-d-labels",s).value.split(",").map(a=>a.trim()).filter(Boolean),description:o("#tb-d-desc",s).value}),u=()=>s.remove(),p=a=>{s.remove(),ot(a)},h=async a=>{let{task:l}=await x(`/tasks/${e}`,{method:"PATCH",body:JSON.stringify(a)});return await k(),l};s.addEventListener("click",a=>{a.target===s&&u()}),o('[data-act="cancel"]',s).addEventListener("click",u),o('[data-act="save"]',s).addEventListener("click",async()=>{try{let a=await h(c());p(a),M("\u2713 \u5DF2\u4FDD\u5B58")}catch(a){alert(`\u4FDD\u5B58\u5931\u8D25\uFF1A${a.message}`)}}),o('[data-act="del"]',s).addEventListener("click",async()=>{if(confirm(`\u5220\u9664\u4EFB\u52A1\u300C${t.title}\u300D\uFF1F`))try{await x(`/tasks/${e}`,{method:"DELETE"}),u(),await J()}catch(a){alert(`\u5220\u9664\u5931\u8D25\uFF1A${a.message}`)}}),o('[data-act="copyctx"]',s)?.addEventListener("click",b),o("#tb-d-note",s).addEventListener("keydown",async a=>{if(a.key==="Enter"&&(a.ctrlKey||a.metaKey)){let l=o("#tb-d-note",s).value.trim();if(!l)return;try{let{task:v}=await x(`/tasks/${e}/notes`,{method:"POST",body:JSON.stringify({text:l})});await k(),p(v),M("\u2713 \u8FDB\u5C55\u5DF2\u8BB0\u5F55")}catch(v){alert(`\u8BB0\u5F55\u5931\u8D25\uFF1A${v.message}`)}}}),o("#tb-d-sess-open",s)?.addEventListener("click",()=>{Yt(e,a=>{s.remove(),ot(a)})}),y("#tb-d-sessions [data-act]",s).forEach(a=>a.addEventListener("click",async()=>{let l=a.dataset.sid;if(a.dataset.act==="open")W(l);else{let{task:v}=await x(`/tasks/${e}/sessions`,{method:"POST",body:JSON.stringify({sessionId:l,action:"unlink"})});await k(),p(v),M("\u5DF2\u89E3\u9664\u4F1A\u8BDD")}})),o("#tb-d-sess-new",s)?.addEventListener("click",async()=>{try{let{sessionId:a}=await x(`/tasks/${e}/session`,{method:"POST",body:"{}"});W(a)}catch(a){alert(`\u65B0\u5EFA\u4F1A\u8BDD\u5931\u8D25\uFF1A${a.message}`)}}),y(".dsh-tb-statusbtn",s).forEach(a=>a.addEventListener("click",async()=>{try{let l=await h({status:a.dataset.status});p(l),M("\u2713 \u72B6\u6001\u5DF2\u66F4\u65B0")}catch(l){alert(`\u72B6\u6001\u6D41\u8F6C\u5931\u8D25\uFF1A${l.message}`)}})),o('[data-act="split"]',s)?.addEventListener("click",()=>{let a=document.createElement("div");a.className="dsh-tb-modal-mask",a.innerHTML=`<div class="dsh-tb-modal">
...
`).map(i=>i.trim()).filter(Boolean);if(!v.length){o("#tb-split-titles",a).focus();return}try{let{tasks:i}=await x(`/tasks/${e}/split`,{method:"POST",body:JSON.stringify({titles:v})});l(),await J(),M(`\u2713 \u5DF2\u62C6\u5206\u4E3A ${i.length} \u4E2A\u5B50\u4EFB\u52A1`),i?.[0]&&Y(i[0].id)}catch(i){alert(`\u62C6\u5206\u5931\u8D25\uFF1A${i.message}`)}}),o("#tb-split-titles",a).focus()})}ht(),ft(),vt(),At(),new MutationObserver(()=>{ht(),ft(),vt()}).observe(document.body,{childList:!0,subtree:!0}),document.addEventListener("keydown",t=>{t.ctrlKey&&t.shiftKey&&(t.key==="B"||t.key==="b")&&(t.preventDefault(),F())}),document.addEventListener("click",t=>{if(!X())return;let e=t.target;e instanceof Element&&(e.closest(`[${C}]`)||e.closest('[data-pane="sidebar"], [class*="sidebarCol"]')&&F(!1))});function Dt(){let t=ct();if(!t||t.clientWidth===0)return;let e=t.style.width;try{t.style.width=`${t.clientWidth-1}px`,t.offsetWidth,t.style.width=e||""}catch{}}let Vt=[300,800,1600],mt=()=>{for(let t of Vt)setTimeout(Dt,t)};document.readyState==="loading"?window.addEventListener("load",mt):mt()})();})();
