...
<div id="dsh-tb-view" ${O}="">
  ...
  <div class="dsh-tb-body" id="dsh-tb-body"></div>
</div>`;function nt(){let t=document.querySelector('[data-pane="sidebar"], [class*="sidebarCol"]');return t===null?void 0:t.querySelector('[class*="logoRow"]')?.parentElement??t.firstElementChild}function lt(t){let e=t.querySelector('button[class*="newSession"]');if(e!==null)return e;for(let s of t.children)if(s.tagName==="BUTTON")return s}function ct(){return document.querySelector('[data-pane="conversation"]')??document.querySelector('[class*="centerCol"]')??void 0}function bt(t){return[...t.classList].some(e=>/collapsed/i.test(e))}let m=null,V=null;function tt(){return document.documentElement.hasAttribute(S)}function pt(){m!==null&&(tt()?m.setAttribute("data-active",""):m.removeAttribute("data-active"))}function et(t){let e=t!==void 0?t:!tt();e&&document.documentElement.removeAttribute(D),e?document.documentElement.setAttribute(S,""):document.documentElement.removeAttribute(S),pt(),e&&J()}function ht(){if(V!==null)return;let t=ct();t!==void 0&&(V=document.createElement("div"),V.setAttribute(O,""),V.innerHTML=Tt,t.appendChild(V),At())}function ft(){let t=nt();if(t===void 0||(m===null&&(m=document.createElement("button"),m.type="button",m.setAttribute(C,""),m.innerHTML=`<span>${kt}</span><span>\u4EFB\u52A1\u770B\u677F</span>`,m.title="\u4EFB\u52A1\u770B\u677F",m.addEventListener("click",()=>et()),pt()),m.parentElement===t))return;let e=lt(t),s=e?.closest('[class*="logoRow"]'),a=s!=null&&s.parentElement===t?s:e;t.insertBefore(m,a?.nextElementSibling??null),(()=>{m!==null&&(bt(t)?m.setAttribute("data-icon-only",""):m.removeAttribute("data-icon-only"))})()}let zt=15,w=null,L=null,B=!1,ut=null;function St(){return x("/sessions").then(t=>t.sessions||[]).catch(()=>A)}function jt(t){return[...t||[]].filter(e=>e&&e.updatedAt).sort((e,s)=>(s.updatedAt||0)-(e.updatedAt||0)).slice(0,zt)}function st(t){if(L===null)return;let e=jt(t),s=d("#dsh-recent-cnt");if(s&&(s.textContent=String(e.length)),!e.length){L.innerHTML='<div class="dsh-recent-empty">\uFF08\u6682\u65E0\u6700\u8FD1\u4F1A\u8BDD\uFF09</div>';return}L.innerHTML=e.map(a=>{let b=a.title&&String(a.title).trim()?a.title:a.id,l=a.running?'<span class="dot run"></span>':'<span class="dot idle"></span>';return`<div class="dsh-recent-row" data-sid="${r(a.id)}" title="\u6253\u5F00\u5E76\u7EE7\u7EED\uFF1A${r(b)}">
				<span class="lbl">
					${l}
					<span class="t">${r(b)}</span>
				</span>
				<span class="meta">
					${a.preview?`<span class="prev">${r(a.preview)}</span>`:""}
					${a.repo?`<span class="repo">${r(H(a.repo))}</span>`:""}
					<span class="tm">${I(a.updatedAt)}</span>
				</span>
			</div>`}).join(""),$(".dsh-recent-row",L).forEach(a=>a.addEventListener("click",()=>{W(a.dataset.sid)}))}function at(){St().then(t=>st(t)).catch(()=>st(A))}function vt(){let t=nt();if(t===void 0||(w===null&&(w=document.createElement("div"),w.id="dsh-recent",w.className="dsh-recent",w.innerHTML=`
				...
				<div class="dsh-recent-body" id="dsh-recent-body"></div>`,L=d("#dsh-recent-body",w),d("#dsh-recent-head",w).addEventListener("click",()=>{B=!B,w&&w.classList.toggle("collapsed",B),L&&(L.style.display=B?"none":"")}),d("#dsh-recent-refresh",w).addEventListener("click",b=>{b.stopPropagation(),at()}),st(A),at()),w.parentElement===t))return;let e=m??lt(t);t.insertBefore(w,e?.nextElementSibling??null),L&&(L.style.maxHeight="46vh",L.style.overflowY="auto"),(()=>{L&&(L.style.display=B?"none":""),w&&w.classList.toggle("collapsed",B)})(),(()=>{w&&(bt(t)?w.style.display="none":w.style.display="")})()}function Ct(){ut===null&&(ut=setInterval(()=>{document.hidden||at()},6e4))}function At(){d("#dsh-tb-new")?.addEventListener("click",wt),d("#dsh-tb-refresh")?.addEventListener("click",()=>J()),d("#dsh-tb-search")?.addEventListener("input",()=>{z.q=d("#dsh-tb-search").value.trim(),k()}),d("#dsh-tb-repo-filter")?.addEventListener("change",()=>{z.repo=d("#dsh-tb-repo-filter").value,k()}),d("#dsh-tb-priority-filter")?.addEventListener("change",()=>{z.priority=d("#dsh-tb-priority-filter").value,k()}),d("#dsh-tb-status-filter")?.addEventListener("change",()=>{z.status=d("#dsh-tb-status-filter").value,k()}),d("#dsh-tb-label-filter")?.addEventListener("change",()=>{z.label=d("#dsh-tb-label-filter").value,k()}),$(".dsh-tb-tab").forEach(t=>t.addEventListener("click",()=>{N=t.dataset.tbView,X()}))}async function k(){let t=new URLSearchParams;for(let s of["q","repo","priority","status","label"])z[s]&&t.set(s,z[s]);j=(await x(`/tasks?${t}`)).tasks||[],X()}async function Mt(){try{let[t,e]=await Promise.all([x("/sessions"),x("/workspaces")]);A=t.sessions||[],q=e.workspaces||[];let s=d("#dsh-tb-repo-filter");if(s){let p=s.value;s.innerHTML='<option value="">\u5168\u90E8\u4ED3\u5E93</option>'+q.map(h=>`<option value="${r(h.path)}" title="${r(h.path)}">${r(H(h.path))}</option>`).join(""),p&&[...s.options].some(h=>h.value===p)&&(s.value=p)}let a=await x("/tasks").catch(()=>({tasks:[]})),b=new Set;for(let p of a.tasks||[])for(let h of p.labels||[])b.add(h);Z=[...b].sort();let l=d("#dsh-tb-label-filter");if(l){let p=l.value;l.innerHTML='<option value="">\u5168\u90E8\u6807\u7B7E</option>'+Z.map(h=>`<option value="${r(h)}">${r(h)}</option>`).join(""),p&&[...l.options].some(h=>h.value===p)&&(l.value=p)}let c=d("#dsh-tb-priority-filter");c&&!c.options.length&&(c.innerHTML='<option value="">\u5168\u90E8\u4F18\u5148\u7EA7</option>'+Object.entries(R).map(([p,h])=>`<option value="${p}">${h.label}</option>`).join(""));let v=d("#dsh-tb-status-filter");v&&!v.options.length&&(v.innerHTML='<option value="">\u5168\u90E8\u72B6\u6001</option>'+Object.entries(g).map(([p,h])=>`<option value="${p}">${h.label}</option>`).join(""))}catch{}}async function _t(){for(let t of q)if(!(!t.path||U[t.path]!==void 0))try{let e=await fetch(`/ide/api/git?op=status&path=${encodeURIComponent(t.path)}`).then(s=>s.json());U[t.path]=e.git?{branch:e.branch?.name||"(detached)",dirty:(e.files||[]).length}:null}catch{U[t.path]=null}}function J(){return Mt().then(()=>_t()).then(k).catch(k)}function xt(t){return(t.labels||[]).map(e=>`<span class="dsh-tb-pill" style="color:${Et(e)}">${r(e)}</span>`).join("")}function Ot(t){let e=g[t.status]||g.todo;return`<span class="dsh-tb-pill" style="color:${e.color}">${e.label}</span>`}function gt(t){let e=R[t.priority]||R.medium;return`<span class="dsh-tb-pill" style="color:${e.color}">${e.label}</span>`}function Ht(t){let e=(s,a,b)=>`<button type="button" class="dsh-tb-statusbtn ${b||""}" data-status="${s}">${a}</button>`;switch(t.status){case"todo":return`${e("in_progress","\u25B6 \u5F00\u59CB\u6267\u884C")} ${e("blocked","\u26D4 \u963B\u585E","ghost")}`;case"in_progress":return`${e("in_review","\u63D0\u4EA4\u8BC4\u5BA1")} ${e("blocked","\u26D4 \u963B\u585E","ghost")}`;case"in_review":return`${e("done","\u2705 \u786E\u8BA4\u5B8C\u6210","primary")} ${e("in_progress","\u21A9 \u9000\u56DE\u4FEE\u6539")}`;case"done":return`${e("in_progress","\u21A9 \u91CD\u65B0\u6253\u5F00","ghost")}`;case"blocked":return`${e("in_progress","\u25B6 \u6062\u590D\u8FDB\u884C")}`;default:return""}}function Nt(){let t=document.querySelectorAll('[class*="sessionRow"], [class*="root"]');for(let e of t){let s=null;for(let b of Object.keys(e))if(b.startsWith("__reactFiber$")){s=b;break}if(s===null)continue;let a=e[s];for(let b=0;b<30&&a!==null;b++){let l=a.memoizedProps;if(l!==null&&typeof l=="object"&&typeof l.open=="function"&&typeof l.startSession=="function")return l.open;a=a.return}}return null}function W(t){try{let e=Nt();if(typeof e=="function"){e(t);return}}catch{}try{localStorage.setItem("dsh.sessions.current",JSON.stringify({sessionId:t}))}catch{}location.reload()}function M(t){let e=document.createElement("div");e.className="dsh-tb-toast",e.textContent=t,document.body.appendChild(e),setTimeout(()=>e.remove(),1600)}function Pt(){var u;let t={};for(let i of P)t[i]=0;let e={};for(let i of j){t[i.status]!==void 0&&t[i.status]++;let f=i.repo||"\uFF08\u672A\u6307\u5B9A\uFF09";(e[f]||(e[f]=[])).push(i)}let s=j.length,a=t.done||0,b=s?Math.round(a/s*100):0,l={};for(let i of A)i.repo&&(l[u=i.repo]||(l[u]=[])).push(i);let c=[...j].sort((i,f)=>f.updatedAt-i.updatedAt).slice(0,10),p='<div class="dsh-tb-ov-stat dsh-tb-ov-add" id="dsh-tb-ov-new" title="\u65B0\u5EFA\u4EFB\u52A1"><div class="dsh-tb-ov-addbtn">\uFF0B \u65B0\u5EFA\u4EFB\u52A1</div></div>'+[["\u5168\u90E8\u4EFB\u52A1",s,"#e6edf3",""],...P.map(i=>[g[i].label,t[i]||0,g[i].color,i]),["\u5B8C\u6210\u7387",`${b}%`,"#3fb950",null]].map(([i,f,E,y])=>{let dt=i==="\u5B8C\u6210\u7387"?b:s?Math.round(f/s*100):0,F=y!==null?" dsh-tb-ov-stat-click":"",it=y===""?"\u67E5\u770B\u5168\u90E8\u4EFB\u52A1":y?`\u67E5\u770B\u300C${i}\u300D\u7684\u4EFB\u52A1`:"";return`<div class="dsh-tb-ov-stat${F}" ${y!==null?`data-status="${y}"`:""} title="${it}"><div class="n" style="color:${E}">${f}</div><div class="l">${i}</div><div class="mini"><i style="width:${dt}%;background:${E}"></i></div></div>`}).join(""),h;q.length?h=q.map(i=>{let f=e[i.path]||e[i.title]||[],E=P.map(T=>f.filter(_=>_.status===T).length),y=f.length,dt=E[P.indexOf("done")]||0,F=y?Math.round(dt/y*100):0,it=y?P.map((T,_)=>E[_]?`<i style="width:${Math.round(E[_]/y*100)}%;background:${g[T].color}" title="${g[T].label} ${E[_]}"></i>`:"").join(""):"",Vt=P.map((T,_)=>E[_]?`<span class="st"><i class="dot" style="background:${g[T].color}"></i>${g[T].label} <b>${E[_]}</b></span>`:"").join(""),Jt=(l[i.path]||l[i.title]||[]).length,K=U[i.path],Wt=K?`<span class="branch">\u2387 ${r(K.branch)}</span>${K.dirty?`<span class="branch dirty" title="${K.dirty} \u4E2A\u672A\u63D0\u4EA4\u6587\u4EF6">\u25CF${K.dirty}</span>`:""}`:"",yt=(l[i.path]||l[i.title]||[]).slice(0,3).map(T=>`<div class="s" data-sid="${r(T.id)}" title="\u6253\u5F00\u4F1A\u8BDD ${r(T.id)}">\u25B8 ${r(T.title)}</div>`).join("");return`<div class="dsh-tb-ov-ws" data-repo="${r(i.path)}">
					<div class="ws-head"><h4>${r(i.title)}</h4><span class="ws-total">${y} \u4E2A\u4EFB\u52A1</span></div>
					<div class="path">${r(i.path||"")}</div>
					${y?`<div class="stack">${it}</div><div class="ws-stats">${Vt}</div>`:'<div class="ws-stats" style="color:var(--dsw-alias-label-secondary,#9aa7b4)">\u6682\u65E0\u4EFB\u52A1\uFF0C\u70B9\u300C\uFF0B \u65B0\u5EFA\u300D\u521B\u5EFA</div>'}
					<div class="comp"><span class="pct">\u5B8C\u6210\u7387 ${F}%</span><div class="track"><i style="width:${F}%"></i></div></div>
					<div class="meta">${Wt}<span class="sess">\u4F1A\u8BDD ${Jt}</span></div>
					${yt?`<div class="sesslist">${yt}</div>`:""}
				</div>`}).join(""):h='<div class="dsh-tb-empty">\uFF08\u6682\u65E0\u5DE5\u4F5C\u533A\uFF09</div>';let o=c.length?c.map(i=>{let f=g[i.status]||g.todo;return`<div class="dsh-tb-ov-item" data-id="${r(i.id)}">
		...
		</div>`,d("#dsh-tb-ov-new")?.addEventListener("click",wt),$(".dsh-tb-ov-stat[data-status]").forEach(i=>i.addEventListener("click",()=>{z.status=i.dataset.status,N="list",X(),k()})),$(".dsh-tb-ov-ws").forEach(i=>i.addEventListener("click",()=>{z.repo=i.dataset.repo;let f=d("#dsh-tb-repo-filter");f&&(f.value=z.repo),N="kanban",X(),k()})),$(".dsh-tb-ov-ws .s").forEach(i=>i.addEventListener("click",f=>{f.stopPropagation(),W(i.dataset.sid)})),$(".dsh-tb-ov-item").forEach(i=>i.addEventListener("click",()=>{i.dataset.id&&Y(i.dataset.id)})))}function Rt(t){let e=g[t.status]||g.todo,s=R[t.priority]||R.medium,a=[];a.push(gt(t)),t.status==="in_review"&&a.push('<span class="dsh-tb-pill" style="color:#bc8cff;font-weight:600">\u25C9 \u5F85 review</span>'),t.repo&&a.push(`<span class="dsh-tb-pill" style="color:#79c0ff">${r(H(t.repo))}</span>`),t.feature&&a.push(`<span class="dsh-tb-pill" style="color:#d2a8ff">${r(t.feature)}</span>`),a.push(xt(t)),t.review&&t.review!=="none"&&a.push(`<span class="dsh-tb-pill" style="color:${t.review==="approved"?"#3fb950":t.review==="rejected"?"#f85149":"#d29922"}">${G[t.review]}</span>`),t.test&&t.test!=="none"&&a.push(`<span class="dsh-tb-pill" style="color:${t.test==="passed"?"#3fb950":t.test==="failed"?"#f85149":"#d29922"}">${Q[t.test]}</span>`);let b=[];return t.sessionIds?.length&&b.push(`<span>\u4F1A\u8BDD ${t.sessionIds.length}</span>`),t.notes?.length&&b.push(`<span>\u8BC4\u8BBA ${t.notes.length}</span>`),b.push(`<span>${I(t.updatedAt)}</span>`),`<div class="dsh-tb-card" data-id="${r(t.id)}" style="border-left:3px solid ${e.color}">
			...
			<div class="dsh-tb-card-foot">${b.join(" \xB7 ")}</div>
		</div>`}function It(){let t=d("#dsh-tb-body"),e=d("#dsh-tb-count");if(!t)return;t.innerHTML='<div class="dsh-tb-columns"></div>';let s=d(".dsh-tb-columns",t);e&&(e.textContent=`${j.length} \u4E2A\u4EFB\u52A1`);for(let[a,b]of Object.entries(g)){let l=document.createElement("div");l.className="dsh-tb-col";let c=j.filter(p=>p.status===a);l.innerHTML=`<div class="dsh-tb-col-head">${b.label} <b>${c.length}</b></div><div class="dsh-tb-col-body"></div>`;let v=d(".dsh-tb-col-body",l);c.length?c.forEach(p=>v.insertAdjacentHTML("beforeend",Rt(p))):v.innerHTML='<div class="dsh-tb-empty">\u2014</div>',s.appendChild(l)}$(".dsh-tb-card",s).forEach(a=>a.addEventListener("click",()=>Y(a.dataset.id)))}function qt(){let t=d("#dsh-tb-body"),e=d("#dsh-tb-count");if(!t)return;if(e&&(e.textContent=`${j.length} \u4E2A\u4EFB\u52A1`),!j.length){t.innerHTML='<div class="dsh-tb-ov"><div class="dsh-tb-empty">\uFF08\u6682\u65E0\u4EFB\u52A1\uFF0C\u70B9\u300C\uFF0B \u65B0\u5EFA\u300D\u521B\u5EFA\uFF09</div></div>';return}let s=j.map(a=>`<tr data-id="${r(a.id)}">
			<td>${Ot(a)}</td>
			<td>${gt(a)}</td>
			<td style="max-width:340px"><div style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${r(a.title)}</div></td>
			<td>${a.repo?`<span class="dsh-tb-pill" style="color:#79c0ff">${r(H(a.repo))}</span>`:""}</td>
			...
			<td>${a.review!=="none"?`<span class="dsh-tb-pill" style="color:${a.review==="approved"?"#3fb950":a.review==="rejected"?"#f85149":"#d29922"}">${G[a.review]}</span>`:""}</td>
			<td>${a.test!=="none"?`<span class="dsh-tb-pill" style="color:${a.test==="passed"?"#3fb950":a.test==="failed"?"#f85149":"#d29922"}">${Q[a.test]}</span>`:""}</td>
			<td style="white-space:nowrap">${I(a.updatedAt)}</td>
			...
			<table class="dsh-tb-table">
		...
		</div>`,$("tr[data-id]",t).forEach(a=>a.addEventListener("click",()=>Y(a.dataset.id)))}function X(){$(".dsh-tb-tab").forEach(e=>{e.dataset.tbView===N?e.classList.add("dsh-tb-tab-on"):e.classList.remove("dsh-tb-tab-on")});let t=d("#dsh-tb-toolbar");t&&(t.style.display=N==="overview"?"none":"flex"),N==="kanban"?It():N==="list"?qt():Pt()}function wt(){let t=document.createElement("div");t.className="dsh-tb-modal-mask";let e=q.map(c=>`<option value="${r(c.path)}" title="${r(c.path)}">${r(c.path)}</option>`).join(""),s=(c,v)=>{d(c,t)?.classList.add("err"),v&&(d(v,t).hidden=!1)},a=()=>{t.innerHTML=`<div class="dsh-tb-modal" style="width:min(560px,92vw)">
				<h3>\u65B0\u5EFA\u4EFB\u52A1</h3>
			...
			</div>`,d("#tb-f-title",t).addEventListener("input",()=>{d("#tb-f-title",t).classList.remove("err"),d("#tb-hint-title",t).hidden=!0}),d("#tb-f-repo",t).addEventListener("change",()=>{d("#tb-f-repo",t).classList.remove("err"),d("#tb-hint-repo",t).hidden=!0}),d('[data-act="cancel"]',t).addEventListener("click",l),d('[data-act="save"]',t).addEventListener("click",async()=>{let c=d("#tb-f-title",t).value.trim(),v=d("#tb-f-repo",t).value;d("#tb-hint-title",t).hidden=!0,d("#tb-hint-repo",t).hidden=!0,$(".err",t).forEach(h=>h.classList.remove("err"));let p=!0;if(c||(s("#tb-f-title","#tb-hint-title"),p=!1),v||(s("#tb-f-repo","#tb-hint-repo"),p=!1),!!p)try{let{task:h}=await x("/tasks",{method:"POST",body:JSON.stringify({title:c,repo:v,feature:d("#tb-f-feature",t).value.trim(),description:d("#tb-f-desc",t).value})});J(),b(h)}catch(h){alert(`\u521B\u5EFA\u5931\u8D25\uFF1A${h.message}`)}}),d("#tb-f-title",t).focus()},b=c=>{t.innerHTML=`<div class="dsh-tb-modal" style="width:min(560px,92vw)">
				<h3>\u2705 \u4EFB\u52A1\u5DF2\u521B\u5EFA</h3>
			...
			</div>`,d('[data-act="newsess"]',t).addEventListener("click",async()=>{try{let{sessionId:v}=await x(`/tasks/${c.id}/session`,{method:"POST",body:"{}"});W(v)}catch(v){alert(`\u65B0\u5EFA\u4F1A\u8BDD\u5931\u8D25\uFF1A${v.message}`)}}),d('[data-act="bind"]',t).addEventListener("click",()=>{t.remove(),Y(c.id)}),d('[data-act="detail"]',t).addEventListener("click",()=>{t.remove(),Y(c.id)}),d('[data-act="close"]',t).addEventListener("click",()=>t.remove())},l=()=>t.remove();t.addEventListener("click",c=>{c.target===t&&l()}),document.body.appendChild(t),a()}function Bt(t,e){x("/sessions").then(s=>{let a=document.createElement("div");a.className="dsh-tb-modal-mask",a.innerHTML=`<div class="dsh-tb-modal" style="width:min(600px,92vw)">
				<h3>\u9009\u62E9\u4F1A\u8BDD\u5173\u8054</h3>
				...
				<div class="dsh-tb-actions"><button data-act="cancel">\u53D6\u6D88</button></div>
			</div>`,document.body.appendChild(a);let b=s.sessions||A,l=d("#tb-pick-list",a),c=v=>{let p=String(v||"").trim().toLowerCase(),h=b.filter(u=>!p||String(u.title||"").toLowerCase().includes(p)),o={};for(let u of h){let i=u.repo||"\uFF08\u672A\u6307\u5B9A\uFF09";(o[i]||(o[i]=[])).push(u)}let n=Object.entries(o).sort((u,i)=>i[1].length-u[1].length).map(([u,i])=>(i.sort((f,E)=>(E.updatedAt||0)-(f.updatedAt||0)),`<div class="dsh-tb-pick-group">
							<div class="dsh-tb-pick-grouphead">${r(H(u))} <b>${i.length}</b></div>
							${i.map(f=>`<div class="dsh-tb-pick-item" data-sid="${r(f.id)}">
								...
								<div class="m">${f.updatedAt?I(f.updatedAt):""}</div>
							</div>`).join("")}
						</div>`)).join("");l.innerHTML=n||'<div class="dsh-tb-empty">\uFF08\u65E0\u5339\u914D\u4F1A\u8BDD\uFF09</div>',$(".dsh-tb-pick-item",a).forEach(u=>u.addEventListener("click",async()=>{let i=u.dataset.sid;try{let{task:f,injected:E,injectionNote:y}=await x(`/tasks/${t}/sessions`,{method:"POST",body:JSON.stringify({sessionId:i,action:"link"})});a.remove(),await k(),e(f),M(E?"\u2713 \u5DF2\u5173\u8054\u4F1A\u8BDD\uFF08\u5DF2\u6CE8\u5165\u4EFB\u52A1\u4E0A\u4E0B\u6587\uFF09":y||"\u5DF2\u5173\u8054\u4F1A\u8BDD")}catch(f){alert(`\u5173\u8054\u5931\u8D25\uFF1A${f.message}`)}}))};c(""),d("#tb-pick-search",a).addEventListener("input",()=>c(d("#tb-pick-search",a).value)),d('[data-act="cancel"]',a).addEventListener("click",()=>a.remove()),a.addEventListener("click",v=>{v.target===a&&a.remove()}),d("#tb-pick-search",a).focus()}).catch(()=>alert("\u83B7\u53D6\u4F1A\u8BDD\u5217\u8868\u5931\u8D25"))}async function Y(t){let{task:e}=await x(`/tasks/${t}`);ot(e)}function ot(t){let e=t.id,s=document.createElement("div");s.className="dsh-tb-modal-mask";let a=A.map(o=>`<option value="${r(o.id)}">${r(o.title||o.id)}${o.repo?` \xB7 ${r(H(o.repo))}`:""}</option>`).join("");s.innerHTML=`<div class="dsh-tb-modal" style="width:min(700px,92vw)">
			<h3>${r(t.title)}</h3>
			...
			<div class="dsh-tb-row">
				...
				<div class="dsh-tb-field"><label>\u4F18\u5148\u7EA7</label><select id="tb-d-priority">${Object.entries(R).map(([o,n])=>`<option value="${o}" ${o===t.priority?"selected":""}>${n.label}</option>`).join("")}</select></div>
			...
			<div class="dsh-tb-row">
				<div class="dsh-tb-field"><label>Review</label><select id="tb-d-review">${Object.entries(G).map(([o,n])=>`<option value="${o}" ${o===t.review?"selected":""}>${n}</option>`).join("")}</select></div>
				<div class="dsh-tb-field"><label>\u6D4B\u8BD5</label><select id="tb-d-test">${Object.entries(Q).map(([o,n])=>`<option value="${o}" ${o===t.test?"selected":""}>${n}</option>`).join("")}</select></div>
				<div class="dsh-tb-field"><label>\u6807\u7B7E</label><input id="tb-d-labels" value="${r((t.labels||[]).join(", "))}" placeholder="\u9017\u53F7\u5206\u9694" list="tb-labels-datalist2" /><datalist id="tb-labels-datalist2">${Z.map(o=>`<option value="${r(o)}"></option>`).join("")}</datalist></div>
			</div>
			...
			<div class="dsh-tb-field"><label>\u8FDB\u5C55\u8BB0\u5F55\uFF08${(t.notes||[]).length}\uFF09</label>
				<div id="tb-d-notes">${(t.notes||[]).map(o=>`<div class="dsh-tb-note"><span class="dsh-tb-note-time">${I(o.at)}</span><br/>${r(o.text)}</div>`).join("")||'<div class="dsh-tb-note">\uFF08\u6682\u65E0\u8BB0\u5F55\uFF09</div>'}</div>
			...
			<div class="dsh-tb-field"><label>\u6C60\u5185\u4F1A\u8BDD\uFF08\u5171\u4EAB\u4E0A\u4E0B\u6587\u53EF\u8BBF\u95EE\u8005\uFF1B\u70B9\u51FB\u6253\u5F00\u53EF\u7EE7\u7EED\uFF09</label>
				<div id="tb-d-sessions">${(t.sessionIds||[]).map(o=>{let n=A.find(u=>u.id===o);return`<div class="dsh-tb-sess"><span class="dsh-tb-sess-title" title="${r(o)}">${r(n?n.title:o)}${n&&n.running?' <span class="dsh-tb-run">\u25CF \u8FD0\u884C\u4E2D</span>':""}</span><button data-sid="${r(o)}" data-act="open" class="dsh-tb-open">\u6253\u5F00</button><button data-sid="${r(o)}" data-act="unlink">\u89E3\u9664</button></div>`}).join("")||'<div class="dsh-tb-note">\uFF08\u672A\u5173\u8054\u4F1A\u8BDD\uFF09</div>'}</div>
		...
		</div>`,document.body.appendChild(s);let b=async()=>{let o=d("#tb-d-ctx-body",s);if(o)try{let{context:n}=await x(`/tasks/${e}/context`);o.textContent=n||"\uFF08\u6682\u65E0\u5171\u4EAB\u4E0A\u4E0B\u6587\uFF09"}catch{o.textContent="\uFF08\u83B7\u53D6\u5931\u8D25\uFF09"}},l=async()=>{try{let{context:o}=await x(`/tasks/${e}/context`);await navigator.clipboard.writeText(o||""),M("\u2713 \u5DF2\u590D\u5236\u6700\u65B0\u5171\u4EAB\u4E0A\u4E0B\u6587")}catch{try{let{context:o}=await x(`/tasks/${e}/context`),n=document.createElement("textarea");n.value=o||"",document.body.appendChild(n),n.select(),document.execCommand("copy"),n.remove(),M("\u2713 \u5DF2\u590D\u5236\u6700\u65B0\u5171\u4EAB\u4E0A\u4E0B\u6587")}catch(o){alert(`\u590D\u5236\u5931\u8D25\uFF1A${o.message}`)}}};b(),d("#tb-d-ctx-refresh",s)?.addEventListener("click",b),d("#tb-d-ctx-copy",s)?.addEventListener("click",l);let c=()=>({repo:d("#tb-d-repo",s).value.trim(),feature:d("#tb-d-feature",s).value.trim(),priority:d("#tb-d-priority",s).value,progress:Number(d("#tb-d-progress",s).value),review:d("#tb-d-review",s).value,test:d("#tb-d-test",s).value,labels:d("#tb-d-labels",s).value.split(",").map(o=>o.trim()).filter(Boolean),description:d("#tb-d-desc",s).value}),v=()=>s.remove(),p=o=>{s.remove(),ot(o)},h=async o=>{let{task:n}=await x(`/tasks/${e}`,{method:"PATCH",body:JSON.stringify(o)});return await k(),n};s.addEventListener("click",o=>{o.target===s&&v()}),d('[data-act="cancel"]',s).addEventListener("click",v),d('[data-act="save"]',s).addEventListener("click",async()=>{try{let o=await h(c());p(o),M("\u2713 \u5DF2\u4FDD\u5B58")}catch(o){alert(`\u4FDD\u5B58\u5931\u8D25\uFF1A${o.message}`)}}),d('[data-act="del"]',s).addEventListener("click",async()=>{if(confirm(`\u5220\u9664\u4EFB\u52A1\u300C${t.title}\u300D\uFF1F`))try{await x(`/tasks/${e}`,{method:"DELETE"}),v(),await J()}catch(o){alert(`\u5220\u9664\u5931\u8D25\uFF1A${o.message}`)}}),d('[data-act="copyctx"]',s)?.addEventListener("click",l),d("#tb-d-note",s).addEventListener("keydown",async o=>{if(o.key==="Enter"&&(o.ctrlKey||o.metaKey)){let n=d("#tb-d-note",s).value.trim();if(!n)return;try{let{task:u}=await x(`/tasks/${e}/notes`,{method:"POST",body:JSON.stringify({text:n})});await k(),p(u),M("\u2713 \u8FDB\u5C55\u5DF2\u8BB0\u5F55")}catch(u){alert(`\u8BB0\u5F55\u5931\u8D25\uFF1A${u.message}`)}}}),d("#tb-d-sess-open",s)?.addEventListener("click",()=>{Bt(e,o=>{s.remove(),ot(o)})}),$("#tb-d-sessions [data-act]",s).forEach(o=>o.addEventListener("click",async()=>{let n=o.dataset.sid;if(o.dataset.act==="open")W(n);else{let{task:u}=await x(`/tasks/${e}/sessions`,{method:"POST",body:JSON.stringify({sessionId:n,action:"unlink"})});await k(),p(u),M("\u5DF2\u89E3\u9664\u4F1A\u8BDD")}})),d("#tb-d-sess-new",s)?.addEventListener("click",async()=>{try{let{sessionId:o}=await x(`/tasks/${e}/session`,{method:"POST",body:"{}"});W(o)}catch(o){alert(`\u65B0\u5EFA\u4F1A\u8BDD\u5931\u8D25\uFF1A${o.message}`)}}),$(".dsh-tb-statusbtn",s).forEach(o=>o.addEventListener("click",async()=>{try{let n=await h({status:o.dataset.status});p(n),M("\u2713 \u72B6\u6001\u5DF2\u66F4\u65B0")}catch(n){alert(`\u72B6\u6001\u6D41\u8F6C\u5931\u8D25\uFF1A${n.message}`)}})),d('[data-act="split"]',s)?.addEventListener("click",()=>{let o=document.createElement("div");o.className="dsh-tb-modal-mask",o.innerHTML=`<div class="dsh-tb-modal">
			...
			</div>`,document.body.appendChild(o);let n=()=>o.remove();d('[data-act="cancel"]',o).addEventListener("click",n),o.addEventListener("click",u=>{u.target===o&&n()}),d('[data-act="do"]',o).addEventListener("click",async()=>{let u=d("#tb-split-titles",o).value.split(`
`).map(i=>i.trim()).filter(Boolean);if(!u.length){d("#tb-split-titles",o).focus();return}try{let{tasks:i}=await x(`/tasks/${e}/split`,{method:"POST",body:JSON.stringify({titles:u})});n(),await J(),M(`\u2713 \u5DF2\u62C6\u5206\u4E3A ${i.length} \u4E2A\u5B50\u4EFB\u52A1`),i?.[0]&&Y(i[0].id)}catch(i){alert(`\u62C6\u5206\u5931\u8D25\uFF1A${i.message}`)}}),d("#tb-split-titles",o).focus()})}ht(),ft(),vt(),Ct(),new MutationObserver(()=>{ht(),ft(),vt()}).observe(document.body,{childList:!0,subtree:!0}),document.addEventListener("keydown",t=>{t.ctrlKey&&t.shiftKey&&(t.key==="B"||t.key==="b")&&(t.preventDefault(),et())}),document.addEventListener("click",t=>{if(!tt())return;let e=t.target;e instanceof Element&&(e.closest(`[${C}]`)||e.closest('[data-pane="sidebar"], [class*="sidebarCol"]')&&et(!1))});function Yt(){let t=ct();if(!t||t.clientWidth===0)return;let e=t.style.width;try{t.style.width=`${t.clientWidth-1}px`,t.offsetWidth,t.style.width=e||""}catch{}}let Dt=[300,800,1600],mt=()=>{for(let t of Dt)setTimeout(Yt,t)};document.readyState==="loading"?window.addEventListener("load",mt):mt()})();})();
