(()=>{(()=>{if(window.__dshTaskboardLoaded)return;window.__dshTaskboardLoaded=!0;let Yt="/taskboard/api",w={todo:{label:"\u5F85\u529E",color:"#8b949e"},in_progress:{label:"\u8FDB\u884C\u4E2D",color:"#3478f6"},in_review:{label:"\u8BC4\u5BA1\u4E2D",color:"#bc8cff"},blocked:{label:"\u5DF2\u963B\u585E",color:"#f85149"},done:{label:"\u5DF2\u5B8C\u6210",color:"#3fb950"}},D=["todo","in_progress","in_review","blocked","done"],q={low:{label:"\u4F4E",color:"#8b949e"},medium:{label:"\u4E2D",color:"#d29922"},high:{label:"\u9AD8",color:"#e3862e"},urgent:{label:"\u7D27\u6025",color:"#f85149"}},lt={none:"\u2014",pending:"\u8BC4\u5BA1\u5F85\u5904\u7406",approved:"\u8BC4\u5BA1\u901A\u8FC7",rejected:"\u8BC4\u5BA1\u9A73\u56DE"},ct={none:"\u2014",pending:"\u6D4B\u8BD5\u5F85\u5904\u7406",passed:"\u6D4B\u8BD5\u901A\u8FC7",failed:"\u6D4B\u8BD5\u5931\u8D25"},wt=["#79c0ff","#d2a8ff","#7ee787","#ffa657","#ff7b72","#f2cc60","#a5d6ff","#ffd7a8"],I="data-dsh-taskboard-active",_="data-dsh-taskboard-entry",P="data-dsh-taskboard-view",X="data-dsh-ssh-active",Kt='<svg viewBox="0 0 16 16" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="2" y="2.5" width="12" height="11" rx="1.5"/><path d="M2 6.5h12M6.5 6.5v7"/></svg>',o=(t,e=document)=>e.querySelector(t),y=(t,e=document)=>[...e.querySelectorAll(t)],l=t=>String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e]),B=t=>{let e=String(t||"").split("/").filter(Boolean);return e.length?e[e.length-1]:String(t||"")},Vt=t=>{let e=0;for(let s of String(t))e=e*31+s.charCodeAt(0)>>>0;return wt[e%wt.length]},J=t=>{if(!t)return"";let e=new Date(t),s=Date.now()-t;return s<6e4?"\u521A\u521A":s<36e5?`${Math.floor(s/6e4)} \u5206\u949F\u524D`:s<864e5?`${Math.floor(s/36e5)} \u5C0F\u65F6\u524D`:`${e.getMonth()+1}/${e.getDate()} ${String(e.getHours()).padStart(2,"0")}:${String(e.getMinutes()).padStart(2,"0")}`},M=[],H=[],Y=[],bt=[],j={q:"",repo:"",priority:"",status:"",label:""},R="overview",at={};async function g(t,e={}){let s=await fetch(Yt+t,{headers:{"content-type":"application/json"},...e}),a=await s.json().catch(()=>({}));if(!s.ok)throw new Error(a.error||`HTTP ${s.status}`);return a}let Wt=`
<style>
...
<div id="dsh-tb-view" ${P}="">
  ...
  <div class="dsh-tb-body" id="dsh-tb-body"></div>
</div>`;function yt(){let t=document.querySelector('[data-pane="sidebar"], [class*="sidebarCol"]');return t===null?void 0:t.querySelector('[class*="logoRow"]')?.parentElement??t.firstElementChild}function $t(t){let e=t.querySelector('button[class*="newSession"]');if(e!==null)return e;for(let s of t.children)if(s.tagName==="BUTTON")return s}function kt(){return document.querySelector('[data-pane="conversation"]')??document.querySelector('[class*="centerCol"]')??void 0}function Et(t){return[...t.classList].some(e=>/collapsed/i.test(e))}let $=null,G=null;function Q(){return document.documentElement.hasAttribute(I)}function St(){$!==null&&(Q()?$.setAttribute("data-active",""):$.removeAttribute("data-active"))}function Z(t){let e=t!==void 0?t:!Q();e&&document.documentElement.removeAttribute(X),e?document.documentElement.setAttribute(I,""):document.documentElement.removeAttribute(I),St(),e&&et()}function Lt(){if(G!==null)return;let t=kt();t!==void 0&&(G=document.createElement("div"),G.setAttribute(P,""),G.innerHTML=Ft,t.appendChild(G),te())}function Tt(){let t=yt();if(t===void 0||($===null&&($=document.createElement("button"),$.type="button",$.setAttribute(_,""),$.innerHTML=`<span>${Kt}</span><span>\u4EFB\u52A1\u770B\u677F</span>`,$.title="\u4EFB\u52A1\u770B\u677F",$.addEventListener("click",()=>Z()),St()),$.parentElement===t))return;let e=$t(t),s=e?.closest('[class*="logoRow"]'),a=s!=null&&s.parentElement===t?s:e;t.insertBefore($,a?.nextElementSibling??null),(()=>{$!==null&&(Et(t)?$.setAttribute("data-icon-only",""):$.removeAttribute("data-icon-only"))})()}let tt=15,Ct="dsh-tb-recent-hidden",zt="dsh-tb-recent-expanded",k=!1,A=new Set,ot=[],C=15,pt=50,nt=new Map;try{k=localStorage.getItem(zt)==="1",A=new Set(JSON.parse(localStorage.getItem(Ct)||"[]"))}catch{}function jt(){try{localStorage.setItem(Ct,JSON.stringify([...A]))}catch{}}let m=null,x=null,K=!1,At=null;function V(t){let e=String(t??"");return e&&!e.startsWith("session-")?`session-${e}`:e}function ht(t){return(t||[]).map(e=>e&&e.id&&V(e.id)!==e.id?{...e,id:V(e.id)}:e)}function Ut(){return g("/sessions").then(t=>ht(t.sessions||[])).catch(()=>H)}function ft(t){return t.title&&String(t.title).trim()?String(t.title).trim():String(t.id)}function Xt(t){return String(t).replace(/^session-/,"").slice(0,8)}function It(t){let e=new Set,s=[];for(let a of[...t||[]].filter(i=>i&&i.updatedAt).sort((i,c)=>(c.updatedAt||0)-(i.updatedAt||0)))if(!e.has(a.id)&&!(a.empty===!0||ft(a)===String(a.id))&&!A.has(a.id)&&(e.add(a.id),s.push(a),s.length>=(k?C:tt)))break;return s}function ut(){let t=k,e=C;k=!0,C=Number.MAX_SAFE_INTEGER;let s=It(ot).length;return k=t,C=e,s}function Mt(t){return`dsh-tb-read:${t}`}function Gt(t){if(nt.has(t))return nt.get(t);let e=0;try{e=Number(localStorage.getItem(Mt(t)))||0}catch{}return nt.set(t,e),e}function Qt(t){let e=Date.now();nt.set(t,e);try{localStorage.setItem(Mt(t),String(e))}catch{}}function W(){if(x===null)return;let t=It(ot),e=ut(),s=x.scrollTop,a=o("#dsh-recent-cnt");a&&(a.textContent=k&&e>t.length?`${t.length}`:e>t.length?`${t.length}/${e}`:`${t.length}`);let i=o("#dsh-recent-more");if(i){let d=e-C;i.style.display=e>tt?"":"none",k?d>0?i.textContent=`\u25BC \u7EE7\u7EED\u52A0\u8F7D\uFF08\u5DF2\u663E\u793A ${C}/${e}\uFF0C\u4E0B\u62C9\u81EA\u52A8\u52A0\u8F7D\uFF09`:i.textContent=`\u25B2 \u6536\u8D77\uFF08\u53EA\u770B ${tt} \u6761\uFF09`:i.textContent=`\u25BC \u5C55\u5F00\u5168\u90E8 ${e} \u6761`,i.classList.toggle("open",k)}let c=o("#dsh-recent-undo");if(c&&(c.style.display=A.size>0?"":"none",c.title=`\u6062\u590D ${A.size} \u6761\u88AB\u9690\u85CF\u7684\u5BF9\u8BDD`,c.textContent=`\u21BA${A.size}`),x.style.maxHeight=k?"72vh":"46vh",!t.length){x.innerHTML=`<div class="dsh-recent-empty">${A.size>0?"\uFF08\u90FD\u9690\u85CF\u4E86\uFF0C\u70B9 \u21BA \u6062\u590D\uFF09":"\uFF08\u6682\u65E0\u6700\u8FD1\u4F1A\u8BDD\uFF09"}</div>`;return}let b=F(),u=new Map;for(let d of t){let p=ft(d);u.set(p,(u.get(p)||0)+1)}x.innerHTML=t.map(d=>{let p=ft(d),n=(u.get(p)||0)>1,h=d.id===b,f="";return d.running?f='<span class="dot run"></span>':d.updatedAt&&d.updatedAt>Gt(d.id)&&(f='<span class="dot idle"></span>'),`<div class="dsh-recent-row${h?" cur":""}" data-sid="${l(d.id)}" title="\u6253\u5F00\u5E76\u7EE7\u7EED\uFF1A${l(p)}\uFF08${l(d.id)}\uFF09">
				<span class="slot">${f}</span>
				<span class="title">${l(p)}</span>
				${n?`<span class="sid">${l(Xt(d.id))}</span>`:""}
				<span class="time">${J(d.updatedAt)}</span>
				<button class="hide" data-hide="${l(d.id)}" title="\u4ECE\u300C\u6700\u65B0\u5BF9\u8BDD\u300D\u91CC\u79FB\u9664\uFF08\u4E0D\u5220\u9664\u4F1A\u8BDD\u672C\u8EAB\uFF0C\u53EF \u21BA \u6062\u590D\uFF09">\u2715</button>
			</div>`}).join(""),y(".dsh-recent-row",x).forEach(d=>d.addEventListener("click",()=>{rt(d.dataset.sid)})),y(".dsh-recent-row .hide",x).forEach(d=>d.addEventListener("click",p=>{p.stopPropagation(),A.add(d.dataset.hide),jt(),W(),L("\u5DF2\u4ECE\u300C\u6700\u65B0\u5BF9\u8BDD\u300D\u79FB\u9664\uFF08\u70B9 \u21BA \u53EF\u6062\u590D\uFF09")})),x.scrollTop=s}function O(){return Ut().then(t=>{ot=t,W()}).catch(()=>{ot=H,W()})}function _t(){let t=yt();if(t===void 0||(m===null&&(m=document.createElement("div"),m.id="dsh-recent",m.className="dsh-recent",m.innerHTML=`
				...
				<button class="dsh-recent-more" id="dsh-recent-more" style="display:none"></button>`,x=o("#dsh-recent-body",m),o("#dsh-recent-head",m).addEventListener("click",()=>{K=!K,m&&m.classList.toggle("collapsed",K),x&&(x.style.display=K?"none":"")}),o("#dsh-recent-refresh",m).addEventListener("click",i=>{i.stopPropagation(),O()}),o("#dsh-recent-more",m).addEventListener("click",i=>{i.stopPropagation(),k?C<ut()?C+=pt:(k=!1,C=tt):(k=!0,C=tt+pt);try{localStorage.setItem(zt,k?"1":"0")}catch{}W()}),x.addEventListener("scroll",()=>{!k||x===null||C>=ut()||x.scrollTop+x.clientHeight<x.scrollHeight-60||(C+=pt,W())}),o("#dsh-recent-undo",m).addEventListener("click",i=>{i.stopPropagation();let c=A.size;A.clear(),jt(),O(),L(`\u5DF2\u6062\u590D ${c} \u6761\u5BF9\u8BDD`)}),W(),O()),m.parentElement===t))return;let e=$??$t(t);t.insertBefore(m,e?.nextElementSibling??null),x&&(x.style.maxHeight="46vh",x.style.overflowY="auto"),(()=>{x&&(x.style.display=K?"none":""),m&&m.classList.toggle("collapsed",K)})(),(()=>{m&&(Et(t)?m.style.display="none":m.style.display="")})()}function Zt(){if(At!==null)return;At=setInterval(()=>{document.hidden||O()},15e3),window.addEventListener("focus",()=>O()),document.addEventListener("visibilitychange",()=>{document.hidden||O()});let t=F();setInterval(()=>{if(document.hidden)return;let e=F();e!==t&&(t=e,O())},2e3)}function te(){o("#dsh-tb-new")?.addEventListener("click",qt),o("#dsh-tb-refresh")?.addEventListener("click",()=>et()),o("#dsh-tb-search")?.addEventListener("input",()=>{j.q=o("#dsh-tb-search").value.trim(),E()}),o("#dsh-tb-repo-filter")?.addEventListener("change",()=>{j.repo=o("#dsh-tb-repo-filter").value,E()}),o("#dsh-tb-priority-filter")?.addEventListener("change",()=>{j.priority=o("#dsh-tb-priority-filter").value,E()}),o("#dsh-tb-status-filter")?.addEventListener("change",()=>{j.status=o("#dsh-tb-status-filter").value,E()}),o("#dsh-tb-label-filter")?.addEventListener("change",()=>{j.label=o("#dsh-tb-label-filter").value,E()}),y(".dsh-tb-tab").forEach(t=>t.addEventListener("click",()=>{R=t.dataset.tbView,it()}))}async function E(){let t=new URLSearchParams;for(let s of["q","repo","priority","status","label"])j[s]&&t.set(s,j[s]);M=(await g(`/tasks?${t}`)).tasks||[],it()}async function ee(){try{let[t,e]=await Promise.all([g("/sessions"),g("/workspaces")]);H=ht(t.sessions||[]),Y=e.workspaces||[];let s=o("#dsh-tb-repo-filter");if(s){let d=s.value;s.innerHTML='<option value="">\u5168\u90E8\u4ED3\u5E93</option>'+Y.map(p=>`<option value="${l(p.path)}" title="${l(p.path)}">${l(B(p.path))}</option>`).join(""),d&&[...s.options].some(p=>p.value===d)&&(s.value=d)}let a=await g("/tasks").catch(()=>({tasks:[]})),i=new Set;for(let d of a.tasks||[])for(let p of d.labels||[])i.add(p);bt=[...i].sort();let c=o("#dsh-tb-label-filter");if(c){let d=c.value;c.innerHTML='<option value="">\u5168\u90E8\u6807\u7B7E</option>'+bt.map(p=>`<option value="${l(p)}">${l(p)}</option>`).join(""),d&&[...c.options].some(p=>p.value===d)&&(c.value=d)}let b=o("#dsh-tb-priority-filter");b&&!b.options.length&&(b.innerHTML='<option value="">\u5168\u90E8\u4F18\u5148\u7EA7</option>'+Object.entries(q).map(([d,p])=>`<option value="${d}">${p.label}</option>`).join(""));let u=o("#dsh-tb-status-filter");u&&!u.options.length&&(u.innerHTML='<option value="">\u5168\u90E8\u72B6\u6001</option>'+Object.entries(w).map(([d,p])=>`<option value="${d}">${p.label}</option>`).join(""))}catch{}}async function se(){for(let t of Y)if(!(!t.path||at[t.path]!==void 0))try{let e=await fetch(`/ide/api/git?op=status&path=${encodeURIComponent(t.path)}`).then(s=>s.json());at[t.path]=e.git?{branch:e.branch?.name||"(detached)",dirty:(e.files||[]).length}:null}catch{at[t.path]=null}}function et(){return ee().then(()=>se()).then(E).catch(E)}function Ot(t){return(t.labels||[]).map(e=>`<span class="dsh-tb-pill" style="color:${Vt(e)}">${l(e)}</span>`).join("")}function ae(t){let e=w[t.status]||w.todo;return`<span class="dsh-tb-pill" style="color:${e.color}">${e.label}</span>`}function Nt(t){let e=q[t.priority]||q.medium;return`<span class="dsh-tb-pill" style="color:${e.color}">${e.label}</span>`}function oe(t){let e=(s,a,i)=>`<button type="button" class="dsh-tb-statusbtn ${i||""}" data-status="${s}">${a}</button>`;switch(t.status){case"todo":return`${e("in_progress","\u25B6 \u5F00\u59CB\u6267\u884C")} ${e("blocked","\u26D4 \u963B\u585E","ghost")}`;case"in_progress":return`${e("in_review","\u63D0\u4EA4\u8BC4\u5BA1")} ${e("blocked","\u26D4 \u963B\u585E","ghost")}`;case"in_review":return`${e("done","\u2705 \u786E\u8BA4\u5B8C\u6210","primary")} ${e("in_progress","\u21A9 \u9000\u56DE\u4FEE\u6539")}`;case"done":return`${e("in_progress","\u21A9 \u91CD\u65B0\u6253\u5F00","ghost")}`;case"blocked":return`${e("in_progress","\u25B6 \u6062\u590D\u8FDB\u884C")}`;default:return""}}let vt=null,Pt=0,ne=5e3;function re(){let t=new Set,e=document.querySelectorAll('[class*="centerCol"], [class*="sidebarCol"], [class*="frame"], [class*="App"], body > div');for(let s of e){for(let a of Object.keys(s)){if(!a.startsWith("__reactFiber$")&&!a.startsWith("__reactContainer$"))continue;let i=s[a];for(;i&&i.return;)i=i.return;i&&t.add(i)}if(t.size>0)break}return[...t]}function Ht(){let t=Date.now();if(vt!==null&&t-Pt<ne)return vt;let e={open:null,startSession:null},s=re(),a=0,i=t+40;for(;s.length>0&&a++<4e4&&!((a&255)===0&&Date.now()>i);){let c=s.shift(),b=c.memoizedProps;if(b!==null&&typeof b=="object"){e.open===null&&typeof b.open=="function"&&(e.open=b.open),e.startSession===null&&typeof b.startSession=="function"&&(e.startSession=b.startSession);let u=b.sessions??b.sessionService;if(u!==null&&typeof u=="object"&&(e.open===null&&typeof u.open=="function"&&(e.open=u.open),e.startSession===null&&typeof u.start=="function"&&(e.startSession=u.start)),e.open!==null&&e.startSession!==null)break}c.child&&s.push(c.child),c.sibling&&s.push(c.sibling)}return vt=e,Pt=Date.now(),e}function ie(){return Ht().open}function F(){try{let t=localStorage.getItem("dsh.sessions.current");if(t===null)return null;let e=JSON.parse(t);return typeof e?.sessionId=="string"?e.sessionId:null}catch{return null}}function Rt(){let t=[...document.querySelectorAll('[class*="_handle"], [class*="handle"]')].find(d=>d.offsetParent!==null&&getComputedStyle(d).cursor==="col-resize");if(!t)return!1;let e=t.getBoundingClientRect(),s=Math.round(e.left+e.width/2),a=Math.round(e.top+Math.min(200,e.height/2)),i=Element.prototype,c=i.setPointerCapture,b=i.hasPointerCapture,u=i.releasePointerCapture;try{i.setPointerCapture=function(){},i.hasPointerCapture=function(){return!0},i.releasePointerCapture=function(){};let d=(p,n)=>new PointerEvent(p,{bubbles:!0,cancelable:!0,composed:!0,pointerId:1,pointerType:"mouse",isPrimary:!0,buttons:1,clientX:n,clientY:a});return t.dispatchEvent(d("pointerdown",s)),t.dispatchEvent(d("pointermove",s+1)),t.dispatchEvent(d("pointerup",s+1)),!0}catch{return!1}finally{i.setPointerCapture=c,i.hasPointerCapture=b,i.releasePointerCapture=u}}function rt(t){t=V(t),Qt(t);try{let e=ie();if(typeof e=="function"){try{localStorage.setItem("dsh.sessions.current",JSON.stringify({sessionId:t}))}catch{}e(t),Q()&&Z(!1),y(".dsh-tb-modal-mask").forEach(s=>s.remove()),O();return}}catch{}try{localStorage.setItem("dsh.sessions.current",JSON.stringify({sessionId:t})),localStorage.setItem("dsh-taskboard.unstick",String(Date.now()))}catch{}location.reload()}async function Dt(t){let e=F(),s=new Set;try{s=new Set((await g("/sessions")).sessions.map(a=>V(a.id)))}catch{}if(de())return Q()&&Z(!1),y(".dsh-tb-modal-mask").forEach(a=>a.remove()),le(t),L("\u5DF2\u8FDB\u5165\u65B0\u5BF9\u8BDD\uFF1A\u53D1\u9001\u7B2C\u4E00\u6761\u6D88\u606F\u540E\u81EA\u52A8\u7ED1\u5B9A\u5230\u8BE5\u4EFB\u52A1"),ce(t,e,s),!0;try{let{sessionId:a}=await g(`/tasks/${t}/session`,{method:"POST",body:"{}"});return be("\u2713 \u5DF2\u65B0\u5EFA\u5BF9\u8BDD\u5E76\u7ED1\u5B9A\u5230\u8BE5\u4EFB\u52A1"),rt(a),!0}catch(a){return alert(`\u65B0\u5EFA\u5BF9\u8BDD\u5931\u8D25\uFF1A${a.message}`),!1}}function de(){let t=document.querySelector('button[class*="newSession"]');if(t!==null)return t.click(),!0;let e=Ht();if(typeof e.startSession=="function")try{return e.startSession(),!0}catch{}return!1}function le(t){g(`/tasks/${t}/context`).then(e=>{let s=e?.context;if(!(typeof s!="string"||!s))return navigator.clipboard?.writeText?.(s)}).catch(()=>{})}function ce(t,e,s){let a=Date.now()+18e5,i=setInterval(async()=>{let c=F();if(c!==null&&c!==e&&!s.has(c)){clearInterval(i);try{await g(`/tasks/${t}/sessions`,{method:"POST",body:JSON.stringify({sessionId:c,action:"link"})}),L("\u2713 \u5DF2\u628A\u65B0\u5BF9\u8BDD\u7ED1\u5B9A\u5230\u8BE5\u4EFB\u52A1"),E()}catch{}return}Date.now()>a&&clearInterval(i)},1e3)}function be(t){try{localStorage.setItem("dsh-taskboard.pending-toast",t)}catch{}}function L(t){let e=document.createElement("div");e.className="dsh-tb-toast",e.textContent=t,document.body.appendChild(e),setTimeout(()=>e.remove(),1600)}function pe(){var f;let t={};for(let r of D)t[r]=0;let e={};for(let r of M){t[r.status]!==void 0&&t[r.status]++;let v=r.repo||"\uFF08\u672A\u6307\u5B9A\uFF09";(e[v]||(e[v]=[])).push(r)}let s=M.length,a=t.done||0,i=s?Math.round(a/s*100):0,c={};for(let r of H)r.repo&&(c[f=r.repo]||(c[f]=[])).push(r);let b=[...M].sort((r,v)=>v.updatedAt-r.updatedAt).slice(0,10),d='<div class="dsh-tb-ov-stat dsh-tb-ov-add" id="dsh-tb-ov-new" title="\u65B0\u5EFA\u4EFB\u52A1"><div class="dsh-tb-ov-addbtn">\uFF0B \u65B0\u5EFA\u4EFB\u52A1</div></div>'+[["\u5168\u90E8\u4EFB\u52A1",s,"#e6edf3",""],...D.map(r=>[w[r].label,t[r]||0,w[r].color,r]),["\u5B8C\u6210\u7387",`${i}%`,"#3fb950",null]].map(([r,v,T,S])=>{let xt=r==="\u5B8C\u6210\u7387"?i:s?Math.round(v/s*100):0,dt=S!==null?" dsh-tb-ov-stat-click":"",mt=S===""?"\u67E5\u770B\u5168\u90E8\u4EFB\u52A1":S?`\u67E5\u770B\u300C${r}\u300D\u7684\u4EFB\u52A1`:"";return`<div class="dsh-tb-ov-stat${dt}" ${S!==null?`data-status="${S}"`:""} title="${mt}"><div class="n" style="color:${T}">${v}</div><div class="l">${r}</div><div class="mini"><i style="width:${xt}%;background:${T}"></i></div></div>`}).join(""),p;Y.length?p=Y.map(r=>{let v=e[r.path]||e[r.title]||[],T=D.map(z=>v.filter(N=>N.status===z).length),S=v.length,xt=T[D.indexOf("done")]||0,dt=S?Math.round(xt/S*100):0,mt=S?D.map((z,N)=>T[N]?`<i style="width:${Math.round(T[N]/S*100)}%;background:${w[z].color}" title="${w[z].label} ${T[N]}"></i>`:"").join(""):"",me=D.map((z,N)=>T[N]?`<span class="st"><i class="dot" style="background:${w[z].color}"></i>${w[z].label} <b>${T[N]}</b></span>`:"").join(""),we=(c[r.path]||c[r.title]||[]).length,st=at[r.path],ye=st?`<span class="branch">\u2387 ${l(st.branch)}</span>${st.dirty?`<span class="branch dirty" title="${st.dirty} \u4E2A\u672A\u63D0\u4EA4\u6587\u4EF6">\u25CF${st.dirty}</span>`:""}`:"",Jt=(c[r.path]||c[r.title]||[]).slice(0,3).map(z=>`<div class="s" data-sid="${l(z.id)}" title="\u6253\u5F00\u4F1A\u8BDD ${l(z.id)}">\u25B8 ${l(z.title)}</div>`).join("");return`<div class="dsh-tb-ov-ws" data-repo="${l(r.path)}">
				...
				${r.repo?`<span class="r">${l(B(r.repo))}</span>`:""}
		...
		</div>`,o("#dsh-tb-ov-new")?.addEventListener("click",qt),y(".dsh-tb-ov-stat[data-status]").forEach(r=>r.addEventListener("click",()=>{j.status=r.dataset.status,R="list",it(),E()})),y(".dsh-tb-ov-ws").forEach(r=>r.addEventListener("click",()=>{j.repo=r.dataset.repo;let v=o("#dsh-tb-repo-filter");v&&(v.value=j.repo),R="kanban",it(),E()})),y(".dsh-tb-ov-ws .s").forEach(r=>r.addEventListener("click",v=>{v.stopPropagation(),rt(r.dataset.sid)})),y(".dsh-tb-ov-item").forEach(r=>r.addEventListener("click",()=>{r.dataset.id&&U(r.dataset.id)})))}function he(t){let e=w[t.status]||w.todo,s=q[t.priority]||q.medium,a=[];a.push(Nt(t)),t.status==="in_review"&&a.push('<span class="dsh-tb-pill" style="color:#bc8cff;font-weight:600">\u25C9 \u5F85 review</span>'),t.repo&&a.push(`<span class="dsh-tb-pill" style="color:#79c0ff">${l(B(t.repo))}</span>`),t.feature&&a.push(`<span class="dsh-tb-pill" style="color:#d2a8ff">${l(t.feature)}</span>`),a.push(Ot(t)),t.review&&t.review!=="none"&&a.push(`<span class="dsh-tb-pill" style="color:${t.review==="approved"?"#3fb950":t.review==="rejected"?"#f85149":"#d29922"}">${lt[t.review]}</span>`),t.test&&t.test!=="none"&&a.push(`<span class="dsh-tb-pill" style="color:${t.test==="passed"?"#3fb950":t.test==="failed"?"#f85149":"#d29922"}">${ct[t.test]}</span>`);let i=[];return t.sessionIds?.length&&i.push(`<span>\u4F1A\u8BDD ${t.sessionIds.length}</span>`),t.notes?.length&&i.push(`<span>\u8BC4\u8BBA ${t.notes.length}</span>`),i.push(`<span>${J(t.updatedAt)}</span>`),`<div class="dsh-tb-card" data-id="${l(t.id)}" style="border-left:3px solid ${e.color}">
		...
		</div>`}function fe(){let t=o("#dsh-tb-body"),e=o("#dsh-tb-count");if(!t)return;t.innerHTML='<div class="dsh-tb-columns"></div>';let s=o(".dsh-tb-columns",t);e&&(e.textContent=`${M.length} \u4E2A\u4EFB\u52A1`);for(let[a,i]of Object.entries(w)){let c=document.createElement("div");c.className="dsh-tb-col";let b=M.filter(d=>d.status===a);c.innerHTML=`<div class="dsh-tb-col-head">${i.label} <b>${b.length}</b></div><div class="dsh-tb-col-body"></div>`;let u=o(".dsh-tb-col-body",c);b.length?b.forEach(d=>u.insertAdjacentHTML("beforeend",he(d))):u.innerHTML='<div class="dsh-tb-empty">\u2014</div>',s.appendChild(c)}y(".dsh-tb-card",s).forEach(a=>a.addEventListener("click",()=>U(a.dataset.id)))}function ue(){let t=o("#dsh-tb-body"),e=o("#dsh-tb-count");if(!t)return;if(e&&(e.textContent=`${M.length} \u4E2A\u4EFB\u52A1`),!M.length){t.innerHTML='<div class="dsh-tb-ov"><div class="dsh-tb-empty">\uFF08\u6682\u65E0\u4EFB\u52A1\uFF0C\u70B9\u300C\uFF0B \u65B0\u5EFA\u300D\u521B\u5EFA\uFF09</div></div>';return}let s=M.map(a=>`<tr data-id="${l(a.id)}">
			<td>${ae(a)}</td>
			<td>${Nt(a)}</td>
			<td style="max-width:340px"><div style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${l(a.title)}</div></td>
			<td>${a.repo?`<span class="dsh-tb-pill" style="color:#79c0ff">${l(B(a.repo))}</span>`:""}</td>
			...
			<td style="white-space:nowrap">${J(a.updatedAt)}</td>
			...
			<table class="dsh-tb-table">
		...
		</div>`,y("tr[data-id]",t).forEach(a=>a.addEventListener("click",()=>U(a.dataset.id)))}function it(){y(".dsh-tb-tab").forEach(e=>{e.dataset.tbView===R?e.classList.add("dsh-tb-tab-on"):e.classList.remove("dsh-tb-tab-on")});let t=o("#dsh-tb-toolbar");t&&(t.style.display=R==="overview"?"none":"flex"),R==="kanban"?fe():R==="list"?ue():pe()}function qt(){let t=document.createElement("div");t.className="dsh-tb-modal-mask";let e=Y.map(b=>`<option value="${l(b.path)}" title="${l(b.path)}">${l(b.path)}</option>`).join(""),s=(b,u)=>{o(b,t)?.classList.add("err"),u&&(o(u,t).hidden=!1)},a=()=>{t.innerHTML=`<div class="dsh-tb-modal" style="width:min(560px,92vw)">
				<h3>\u65B0\u5EFA\u4EFB\u52A1</h3>
				...
				<div class="dsh-tb-actions"><button data-act="cancel">\u53D6\u6D88</button></div>
						...
						</div>`)).join("");c.innerHTML=h||'<div class="dsh-tb-empty">\uFF08\u65E0\u5339\u914D\u4F1A\u8BDD\uFF09</div>',y(".dsh-tb-pick-item",a).forEach(f=>f.addEventListener("click",async()=>{let r=f.dataset.sid;try{let{task:v,injected:T,injectionNote:S}=await g(`/tasks/${t}/sessions`,{method:"POST",body:JSON.stringify({sessionId:r,action:"link"})});a.remove(),await E(),e(v),L(T===!0?"\u2713 \u5DF2\u5173\u8054\u4F1A\u8BDD":S||"\u5DF2\u5173\u8054\u4F1A\u8BDD")}catch(v){alert(`\u5173\u8054\u5931\u8D25\uFF1A${v.message}`)}}))};b(""),o("#tb-pick-search",a).addEventListener("input",()=>b(o("#tb-pick-search",a).value)),o('[data-act="cancel"]',a).addEventListener("click",()=>a.remove()),a.addEventListener("click",u=>{u.target===a&&a.remove()}),o("#tb-pick-search",a).focus()}).catch(()=>alert("\u83B7\u53D6\u4F1A\u8BDD\u5217\u8868\u5931\u8D25"))}async function U(t){let{task:e}=await g(`/tasks/${t}`);gt(e)}function gt(t){let e=t.id,s=document.createElement("div");s.className="dsh-tb-modal-mask";let a=H.map(n=>`<option value="${l(n.id)}">${l(n.title||n.id)}${n.repo?` \xB7 ${l(B(n.repo))}`:""}</option>`).join("");s.innerHTML=`<div class="dsh-tb-modal" style="width:min(700px,92vw)">
			<h3>${l(t.title)}</h3>
			...
			<div class="dsh-tb-row">
				...
				<div class="dsh-tb-field"><label>\u4F18\u5148\u7EA7</label><select id="tb-d-priority">${Object.entries(q).map(([n,h])=>`<option value="${n}" ${n===t.priority?"selected":""}>${h.label}</option>`).join("")}</select></div>
			...
			<div class="dsh-tb-field"><label>\u8FDB\u5C55\u8BB0\u5F55\uFF08${(t.notes||[]).length}\uFF09</label>
				<div id="tb-d-notes">${(t.notes||[]).map(n=>`<div class="dsh-tb-note"><span class="dsh-tb-note-time">${J(n.at)}</span><br/>${l(n.text)}</div>`).join("")||'<div class="dsh-tb-note">\uFF08\u6682\u65E0\u8BB0\u5F55\uFF09</div>'}</div>
			...
			<div class="dsh-tb-field"><label>\u6C60\u5185\u4F1A\u8BDD\uFF08\u5171\u4EAB\u4E0A\u4E0B\u6587\u53EF\u8BBF\u95EE\u8005\uFF1B\u70B9\u51FB\u6253\u5F00\u53EF\u7EE7\u7EED\uFF09</label>
				<div id="tb-d-sessions">${(t.sessionIds||[]).map(n=>{let h=V(n),f=H.find(v=>v.id===h),r=f?l(f.title||h):'<span class="dsh-tb-sess-gone">\uFF08\u4F1A\u8BDD\u5DF2\u4E0D\u5B58\u5728\uFF09</span>';return`<div class="dsh-tb-sess"><span class="dsh-tb-sess-title" title="${l(h)}">${r}${f&&f.running?' <span class="dsh-tb-run">\u25CF \u8FD0\u884C\u4E2D</span>':""}</span>${f?`<button data-sid="${l(h)}" data-act="open" class="dsh-tb-open">\u6253\u5F00</button>`:""}<button data-sid="${l(n)}" data-act="unlink">\u89E3\u9664</button></div>`}).join("")||'<div class="dsh-tb-note">\uFF08\u672A\u5173\u8054\u4F1A\u8BDD\uFF09</div>'}</div>
