(()=>{(()=>{if(window.__dshTaskboardLoaded)return;window.__dshTaskboardLoaded=!0;let At="/taskboard/api",g={todo:{label:"\u5F85\u529E",color:"#8b949e"},in_progress:{label:"\u8FDB\u884C\u4E2D",color:"#3478f6"},in_review:{label:"\u8BC4\u5BA1\u4E2D",color:"#bc8cff"},blocked:{label:"\u5DF2\u963B\u585E",color:"#f85149"},done:{label:"\u5DF2\u5B8C\u6210",color:"#3fb950"}},P=["todo","in_progress","in_review","blocked","done"],H={low:{label:"\u4F4E",color:"#8b949e"},medium:{label:"\u4E2D",color:"#d29922"},high:{label:"\u9AD8",color:"#e3862e"},urgent:{label:"\u7D27\u6025",color:"#f85149"}},et={none:"\u2014",pending:"\u8BC4\u5BA1\u5F85\u5904\u7406",approved:"\u8BC4\u5BA1\u901A\u8FC7",rejected:"\u8BC4\u5BA1\u9A73\u56DE"},st={none:"\u2014",pending:"\u6D4B\u8BD5\u5F85\u5904\u7406",passed:"\u6D4B\u8BD5\u901A\u8FC7",failed:"\u6D4B\u8BD5\u5931\u8D25"},ct=["#79c0ff","#d2a8ff","#7ee787","#ffa657","#ff7b72","#f2cc60","#a5d6ff","#ffd7a8"],C="data-dsh-taskboard-active",A="data-dsh-taskboard-entry",_="data-dsh-taskboard-view",Y="data-dsh-ssh-active",Mt='<svg viewBox="0 0 16 16" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="2" y="2.5" width="12" height="11" rx="1.5"/><path d="M2 6.5h12M6.5 6.5v7"/></svg>',a=(t,e=document)=>e.querySelector(t),y=(t,e=document)=>[...e.querySelectorAll(t)],r=t=>String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e]),N=t=>{let e=String(t||"").split("/").filter(Boolean);return e.length?e[e.length-1]:String(t||"")},It=t=>{let e=0;for(let s of String(t))e=e*31+s.charCodeAt(0)>>>0;return ct[e%ct.length]},R=t=>{if(!t)return"";let e=new Date(t),s=Date.now()-t;return s<6e4?"\u521A\u521A":s<36e5?`${Math.floor(s/6e4)} \u5206\u949F\u524D`:s<864e5?`${Math.floor(s/36e5)} \u5C0F\u65F6\u524D`:`${e.getMonth()+1}/${e.getDate()} ${String(e.getHours()).padStart(2,"0")}:${String(e.getMinutes()).padStart(2,"0")}`},z=[],M=[],q=[],at=[],T={q:"",repo:"",priority:"",status:"",label:""},O="overview",U={};async function x(t,e={}){let s=await fetch(At+t,{headers:{"content-type":"application/json"},...e}),o=await s.json().catch(()=>({}));if(!s.ok)throw new Error(o.error||`HTTP ${s.status}`);return o}let _t=`
<style>
...
<div id="dsh-tb-view" ${_}="">
  ...
  <div class="dsh-tb-body" id="dsh-tb-body"></div>
</div>`;function bt(){let t=document.querySelector('[data-pane="sidebar"], [class*="sidebarCol"]');return t===null?void 0:t.querySelector('[class*="logoRow"]')?.parentElement??t.firstElementChild}function pt(t){let e=t.querySelector('button[class*="newSession"]');if(e!==null)return e;for(let s of t.children)if(s.tagName==="BUTTON")return s}function ht(){return document.querySelector('[data-pane="conversation"]')??document.querySelector('[class*="centerCol"]')??void 0}function ft(t){return[...t.classList].some(e=>/collapsed/i.test(e))}let w=null,J=null;function F(){return document.documentElement.hasAttribute(C)}function ut(){w!==null&&(F()?w.setAttribute("data-active",""):w.removeAttribute("data-active"))}function X(t){let e=t!==void 0?t:!F();e&&document.documentElement.removeAttribute(Y),e?document.documentElement.setAttribute(C,""):document.documentElement.removeAttribute(C),ut(),e&&W()}function vt(){if(J!==null)return;let t=ht();t!==void 0&&(J=document.createElement("div"),J.setAttribute(_,""),J.innerHTML=Ot,t.appendChild(J),Yt())}function gt(){let t=bt();if(t===void 0||(w===null&&(w=document.createElement("button"),w.type="button",w.setAttribute(A,""),w.innerHTML=`<span>${Mt}</span><span>\u4EFB\u52A1\u770B\u677F</span>`,w.title="\u4EFB\u52A1\u770B\u677F",w.addEventListener("click",()=>X()),ut()),w.parentElement===t))return;let e=pt(t),s=e?.closest('[class*="logoRow"]'),o=s!=null&&s.parentElement===t?s:e;t.insertBefore(w,o?.nextElementSibling??null),(()=>{w!==null&&(ft(t)?w.setAttribute("data-icon-only",""):w.removeAttribute("data-icon-only"))})()}let Pt=15,m=null,L=null,D=!1,xt=null;function V(t){let e=String(t??"");return e&&!e.startsWith("session-")?`session-${e}`:e}function ot(t){return(t||[]).map(e=>e&&e.id&&V(e.id)!==e.id?{...e,id:V(e.id)}:e)}function Ht(){return x("/sessions").then(t=>ot(t.sessions||[])).catch(()=>M)}function mt(t){return t.title&&String(t.title).trim()?String(t.title).trim():String(t.id)}function Nt(t){return String(t).replace(/^session-/,"").slice(0,8)}function Rt(t){let e=new Set,s=[];for(let o of[...t||[]].filter(d=>d&&d.updatedAt).sort((d,l)=>(l.updatedAt||0)-(d.updatedAt||0)))if(!e.has(o.id)&&(e.add(o.id),s.push(o),s.length>=Pt))break;return s}function wt(t){return`dsh-tb-read:${t}`}function qt(t){try{return Number(localStorage.getItem(wt(t)))||0}catch{return 0}}function Dt(t){try{localStorage.setItem(wt(t),String(Date.now()))}catch{}}function it(t){if(L===null)return;let e=Rt(t),s=a("#dsh-recent-cnt");if(s&&(s.textContent=String(e.length)),!e.length){L.innerHTML='<div class="dsh-recent-empty">\uFF08\u6682\u65E0\u6700\u8FD1\u4F1A\u8BDD\uFF09</div>';return}let o=Lt(),d=new Map;for(let l of e){let c=mt(l);d.set(c,(d.get(c)||0)+1)}L.innerHTML=e.map(l=>{let c=mt(l),u=(d.get(c)||0)>1,b=l.id===o,h="";return l.running?h='<span class="dot run"></span>':l.updatedAt&&l.updatedAt>qt(l.id)&&(h='<span class="dot idle"></span>'),`<div class="dsh-recent-row${b?" cur":""}" data-sid="${r(l.id)}" title="\u6253\u5F00\u5E76\u7EE7\u7EED\uFF1A${r(c)}\uFF08${r(l.id)}\uFF09">
				<span class="slot">${h}</span>
				<span class="title">${r(c)}</span>
				${u?`<span class="sid">${r(Nt(l.id))}</span>`:""}
				<span class="time">${R(l.updatedAt)}</span>
			</div>`}).join(""),y(".dsh-recent-row",L).forEach(l=>l.addEventListener("click",()=>{Q(l.dataset.sid)}))}function G(){Ht().then(t=>it(t)).catch(()=>it(M))}function yt(){let t=bt();if(t===void 0||(m===null&&(m=document.createElement("div"),m.id="dsh-recent",m.className="dsh-recent",m.innerHTML=`
				...
				<div class="dsh-recent-body" id="dsh-recent-body"></div>`,L=a("#dsh-recent-body",m),a("#dsh-recent-head",m).addEventListener("click",()=>{D=!D,m&&m.classList.toggle("collapsed",D),L&&(L.style.display=D?"none":"")}),a("#dsh-recent-refresh",m).addEventListener("click",d=>{d.stopPropagation(),G()}),it(M),G()),m.parentElement===t))return;let e=w??pt(t);t.insertBefore(m,e?.nextElementSibling??null),L&&(L.style.maxHeight="46vh",L.style.overflowY="auto"),(()=>{L&&(L.style.display=D?"none":""),m&&m.classList.toggle("collapsed",D)})(),(()=>{m&&(ft(t)?m.style.display="none":m.style.display="")})()}function Bt(){xt===null&&(xt=setInterval(()=>{document.hidden||G()},6e4))}function Yt(){a("#dsh-tb-new")?.addEventListener("click",Ct),a("#dsh-tb-refresh")?.addEventListener("click",()=>W()),a("#dsh-tb-search")?.addEventListener("input",()=>{T.q=a("#dsh-tb-search").value.trim(),k()}),a("#dsh-tb-repo-filter")?.addEventListener("change",()=>{T.repo=a("#dsh-tb-repo-filter").value,k()}),a("#dsh-tb-priority-filter")?.addEventListener("change",()=>{T.priority=a("#dsh-tb-priority-filter").value,k()}),a("#dsh-tb-status-filter")?.addEventListener("change",()=>{T.status=a("#dsh-tb-status-filter").value,k()}),a("#dsh-tb-label-filter")?.addEventListener("change",()=>{T.label=a("#dsh-tb-label-filter").value,k()}),y(".dsh-tb-tab").forEach(t=>t.addEventListener("click",()=>{O=t.dataset.tbView,Z()}))}async function k(){let t=new URLSearchParams;for(let s of["q","repo","priority","status","label"])T[s]&&t.set(s,T[s]);z=(await x(`/tasks?${t}`)).tasks||[],Z()}async function Jt(){try{let[t,e]=await Promise.all([x("/sessions"),x("/workspaces")]);M=ot(t.sessions||[]),q=e.workspaces||[];let s=a("#dsh-tb-repo-filter");if(s){let b=s.value;s.innerHTML='<option value="">\u5168\u90E8\u4ED3\u5E93</option>'+q.map(h=>`<option value="${r(h.path)}" title="${r(h.path)}">${r(N(h.path))}</option>`).join(""),b&&[...s.options].some(h=>h.value===b)&&(s.value=b)}let o=await x("/tasks").catch(()=>({tasks:[]})),d=new Set;for(let b of o.tasks||[])for(let h of b.labels||[])d.add(h);at=[...d].sort();let l=a("#dsh-tb-label-filter");if(l){let b=l.value;l.innerHTML='<option value="">\u5168\u90E8\u6807\u7B7E</option>'+at.map(h=>`<option value="${r(h)}">${r(h)}</option>`).join(""),b&&[...l.options].some(h=>h.value===b)&&(l.value=b)}let c=a("#dsh-tb-priority-filter");c&&!c.options.length&&(c.innerHTML='<option value="">\u5168\u90E8\u4F18\u5148\u7EA7</option>'+Object.entries(H).map(([b,h])=>`<option value="${b}">${h.label}</option>`).join(""));let u=a("#dsh-tb-status-filter");u&&!u.options.length&&(u.innerHTML='<option value="">\u5168\u90E8\u72B6\u6001</option>'+Object.entries(g).map(([b,h])=>`<option value="${b}">${h.label}</option>`).join(""))}catch{}}async function Vt(){for(let t of q)if(!(!t.path||U[t.path]!==void 0))try{let e=await fetch(`/ide/api/git?op=status&path=${encodeURIComponent(t.path)}`).then(s=>s.json());U[t.path]=e.git?{branch:e.branch?.name||"(detached)",dirty:(e.files||[]).length}:null}catch{U[t.path]=null}}function W(){return Jt().then(()=>Vt()).then(k).catch(k)}function $t(t){return(t.labels||[]).map(e=>`<span class="dsh-tb-pill" style="color:${It(e)}">${r(e)}</span>`).join("")}function Wt(t){let e=g[t.status]||g.todo;return`<span class="dsh-tb-pill" style="color:${e.color}">${e.label}</span>`}function kt(t){let e=H[t.priority]||H.medium;return`<span class="dsh-tb-pill" style="color:${e.color}">${e.label}</span>`}function Kt(t){let e=(s,o,d)=>`<button type="button" class="dsh-tb-statusbtn ${d||""}" data-status="${s}">${o}</button>`;switch(t.status){case"todo":return`${e("in_progress","\u25B6 \u5F00\u59CB\u6267\u884C")} ${e("blocked","\u26D4 \u963B\u585E","ghost")}`;case"in_progress":return`${e("in_review","\u63D0\u4EA4\u8BC4\u5BA1")} ${e("blocked","\u26D4 \u963B\u585E","ghost")}`;case"in_review":return`${e("done","\u2705 \u786E\u8BA4\u5B8C\u6210","primary")} ${e("in_progress","\u21A9 \u9000\u56DE\u4FEE\u6539")}`;case"done":return`${e("in_progress","\u21A9 \u91CD\u65B0\u6253\u5F00","ghost")}`;case"blocked":return`${e("in_progress","\u25B6 \u6062\u590D\u8FDB\u884C")}`;default:return""}}let nt=null,Et=0,Ut=5e3;function Ft(){let t=new Set,e=document.querySelectorAll('[class*="centerCol"], [class*="sidebarCol"], [class*="frame"], [class*="App"], body > div');for(let s of e){for(let o of Object.keys(s)){if(!o.startsWith("__reactFiber$")&&!o.startsWith("__reactContainer$"))continue;let d=s[o];for(;d&&d.return;)d=d.return;d&&t.add(d)}if(t.size>0)break}return[...t]}function Xt(){let t=Date.now();if(nt!==null&&t-Et<Ut)return nt;let e={open:null,startSession:null},s=Ft(),o=0,d=t+40;for(;s.length>0&&o++<4e4&&!((o&255)===0&&Date.now()>d);){let l=s.shift(),c=l.memoizedProps;if(c!==null&&typeof c=="object"){e.open===null&&typeof c.open=="function"&&(e.open=c.open),e.startSession===null&&typeof c.startSession=="function"&&(e.startSession=c.startSession);let u=c.sessions??c.sessionService;if(u!==null&&typeof u=="object"&&(e.open===null&&typeof u.open=="function"&&(e.open=u.open),e.startSession===null&&typeof u.start=="function"&&(e.startSession=u.start)),e.open!==null&&e.startSession!==null)break}l.child&&s.push(l.child),l.sibling&&s.push(l.sibling)}return nt=e,Et=Date.now(),e}function Gt(){return Xt().open}function Lt(){try{let t=localStorage.getItem("dsh.sessions.current");if(t===null)return null;let e=JSON.parse(t);return typeof e?.sessionId=="string"?e.sessionId:null}catch{return null}}function St(){let t=[...document.querySelectorAll('[class*="_handle"], [class*="handle"]')].find(b=>b.offsetParent!==null&&getComputedStyle(b).cursor==="col-resize");if(!t)return!1;let e=t.getBoundingClientRect(),s=Math.round(e.left+e.width/2),o=Math.round(e.top+Math.min(200,e.height/2)),d=Element.prototype,l=d.setPointerCapture,c=d.hasPointerCapture,u=d.releasePointerCapture;try{d.setPointerCapture=function(){},d.hasPointerCapture=function(){return!0},d.releasePointerCapture=function(){};let b=(h,i)=>new PointerEvent(h,{bubbles:!0,cancelable:!0,composed:!0,pointerId:1,pointerType:"mouse",isPrimary:!0,buttons:1,clientX:i,clientY:o});return t.dispatchEvent(b("pointerdown",s)),t.dispatchEvent(b("pointermove",s+1)),t.dispatchEvent(b("pointerup",s+1)),!0}catch{return!1}finally{d.setPointerCapture=l,d.hasPointerCapture=c,d.releasePointerCapture=u}}function Q(t){t=V(t),Dt(t);try{let e=Gt();if(typeof e=="function"){try{localStorage.setItem("dsh.sessions.current",JSON.stringify({sessionId:t}))}catch{}e(t),F()&&X(!1),y(".dsh-tb-modal-mask").forEach(s=>s.remove()),G();return}}catch{}try{localStorage.setItem("dsh.sessions.current",JSON.stringify({sessionId:t})),localStorage.setItem("dsh-taskboard.unstick",String(Date.now()))}catch{}location.reload()}async function Tt(t){try{let{sessionId:e}=await x(`/tasks/${t}/session`,{method:"POST",body:"{}"});return Qt("\u2713 \u5DF2\u65B0\u5EFA\u5BF9\u8BDD\u5E76\u7ED1\u5B9A\u5230\u8BE5\u4EFB\u52A1"),Q(e),!0}catch(e){return alert(`\u65B0\u5EFA\u5BF9\u8BDD\u5931\u8D25\uFF1A${e.message}`),!1}}function Qt(t){try{localStorage.setItem("dsh-taskboard.pending-toast",t)}catch{}}function j(t){let e=document.createElement("div");e.className="dsh-tb-toast",e.textContent=t,document.body.appendChild(e),setTimeout(()=>e.remove(),1600)}function Zt(){var v;let t={};for(let n of P)t[n]=0;let e={};for(let n of z){t[n.status]!==void 0&&t[n.status]++;let f=n.repo||"\uFF08\u672A\u6307\u5B9A\uFF09";(e[f]||(e[f]=[])).push(n)}let s=z.length,o=t.done||0,d=s?Math.round(o/s*100):0,l={};for(let n of M)n.repo&&(l[v=n.repo]||(l[v]=[])).push(n);let c=[...z].sort((n,f)=>f.updatedAt-n.updatedAt).slice(0,10),b='<div class="dsh-tb-ov-stat dsh-tb-ov-add" id="dsh-tb-ov-new" title="\u65B0\u5EFA\u4EFB\u52A1"><div class="dsh-tb-ov-addbtn">\uFF0B \u65B0\u5EFA\u4EFB\u52A1</div></div>'+[["\u5168\u90E8\u4EFB\u52A1",s,"#e6edf3",""],...P.map(n=>[g[n].label,t[n]||0,g[n].color,n]),["\u5B8C\u6210\u7387",`${d}%`,"#3fb950",null]].map(([n,f,E,$])=>{let dt=n==="\u5B8C\u6210\u7387"?d:s?Math.round(f/s*100):0,tt=$!==null?" dsh-tb-ov-stat-click":"",lt=$===""?"\u67E5\u770B\u5168\u90E8\u4EFB\u52A1":$?`\u67E5\u770B\u300C${n}\u300D\u7684\u4EFB\u52A1`:"";return`<div class="dsh-tb-ov-stat${tt}" ${$!==null?`data-status="${$}"`:""} title="${lt}"><div class="n" style="color:${E}">${f}</div><div class="l">${n}</div><div class="mini"><i style="width:${dt}%;background:${E}"></i></div></div>`}).join(""),h;q.length?h=q.map(n=>{let f=e[n.path]||e[n.title]||[],E=P.map(S=>f.filter(I=>I.status===S).length),$=f.length,dt=E[P.indexOf("done")]||0,tt=$?Math.round(dt/$*100):0,lt=$?P.map((S,I)=>E[I]?`<i style="width:${Math.round(E[I]/$*100)}%;background:${g[S].color}" title="${g[S].label} ${E[I]}"></i>`:"").join(""):"",ne=P.map((S,I)=>E[I]?`<span class="st"><i class="dot" style="background:${g[S].color}"></i>${g[S].label} <b>${E[I]}</b></span>`:"").join(""),re=(l[n.path]||l[n.title]||[]).length,K=U[n.path],de=K?`<span class="branch">\u2387 ${r(K.branch)}</span>${K.dirty?`<span class="branch dirty" title="${K.dirty} \u4E2A\u672A\u63D0\u4EA4\u6587\u4EF6">\u25CF${K.dirty}</span>`:""}`:"",jt=(l[n.path]||l[n.title]||[]).slice(0,3).map(S=>`<div class="s" data-sid="${r(S.id)}" title="\u6253\u5F00\u4F1A\u8BDD ${r(S.id)}">\u25B8 ${r(S.title)}</div>`).join("");return`<div class="dsh-tb-ov-ws" data-repo="${r(n.path)}">
					<div class="ws-head"><h4>${r(n.title)}</h4><span class="ws-total">${$} \u4E2A\u4EFB\u52A1</span></div>
					<div class="path">${r(n.path||"")}</div>
					${$?`<div class="stack">${lt}</div><div class="ws-stats">${ne}</div>`:'<div class="ws-stats" style="color:var(--dsw-alias-label-secondary,#9aa7b4)">\u6682\u65E0\u4EFB\u52A1\uFF0C\u70B9\u300C\uFF0B \u65B0\u5EFA\u300D\u521B\u5EFA</div>'}
			...
			</div>`}).join(""):'<div class="dsh-tb-empty">\uFF08\u6682\u65E0\u4EFB\u52A1\uFF09</div>',p=a("#dsh-tb-body");p&&(p.innerHTML=`<div class="dsh-tb-ov">
		...
		</div>`,a("#dsh-tb-ov-new")?.addEventListener("click",Ct),y(".dsh-tb-ov-stat[data-status]").forEach(n=>n.addEventListener("click",()=>{T.status=n.dataset.status,O="list",Z(),k()})),y(".dsh-tb-ov-ws").forEach(n=>n.addEventListener("click",()=>{T.repo=n.dataset.repo;let f=a("#dsh-tb-repo-filter");f&&(f.value=T.repo),O="kanban",Z(),k()})),y(".dsh-tb-ov-ws .s").forEach(n=>n.addEventListener("click",f=>{f.stopPropagation(),Q(n.dataset.sid)})),y(".dsh-tb-ov-item").forEach(n=>n.addEventListener("click",()=>{n.dataset.id&&B(n.dataset.id)})))}function te(t){let e=g[t.status]||g.todo,s=H[t.priority]||H.medium,o=[];o.push(kt(t)),t.status==="in_review"&&o.push('<span class="dsh-tb-pill" style="color:#bc8cff;font-weight:600">\u25C9 \u5F85 review</span>'),t.repo&&o.push(`<span class="dsh-tb-pill" style="color:#79c0ff">${r(N(t.repo))}</span>`),t.feature&&o.push(`<span class="dsh-tb-pill" style="color:#d2a8ff">${r(t.feature)}</span>`),o.push($t(t)),t.review&&t.review!=="none"&&o.push(`<span class="dsh-tb-pill" style="color:${t.review==="approved"?"#3fb950":t.review==="rejected"?"#f85149":"#d29922"}">${et[t.review]}</span>`),t.test&&t.test!=="none"&&o.push(`<span class="dsh-tb-pill" style="color:${t.test==="passed"?"#3fb950":t.test==="failed"?"#f85149":"#d29922"}">${st[t.test]}</span>`);let d=[];return t.sessionIds?.length&&d.push(`<span>\u4F1A\u8BDD ${t.sessionIds.length}</span>`),t.notes?.length&&d.push(`<span>\u8BC4\u8BBA ${t.notes.length}</span>`),d.push(`<span>${R(t.updatedAt)}</span>`),`<div class="dsh-tb-card" data-id="${r(t.id)}" style="border-left:3px solid ${e.color}">
		...
		</div>`}function ee(){let t=a("#dsh-tb-body"),e=a("#dsh-tb-count");if(!t)return;t.innerHTML='<div class="dsh-tb-columns"></div>';let s=a(".dsh-tb-columns",t);e&&(e.textContent=`${z.length} \u4E2A\u4EFB\u52A1`);for(let[o,d]of Object.entries(g)){let l=document.createElement("div");l.className="dsh-tb-col";let c=z.filter(b=>b.status===o);l.innerHTML=`<div class="dsh-tb-col-head">${d.label} <b>${c.length}</b></div><div class="dsh-tb-col-body"></div>`;let u=a(".dsh-tb-col-body",l);c.length?c.forEach(b=>u.insertAdjacentHTML("beforeend",te(b))):u.innerHTML='<div class="dsh-tb-empty">\u2014</div>',s.appendChild(l)}y(".dsh-tb-card",s).forEach(o=>o.addEventListener("click",()=>B(o.dataset.id)))}function se(){let t=a("#dsh-tb-body"),e=a("#dsh-tb-count");if(!t)return;if(e&&(e.textContent=`${z.length} \u4E2A\u4EFB\u52A1`),!z.length){t.innerHTML='<div class="dsh-tb-ov"><div class="dsh-tb-empty">\uFF08\u6682\u65E0\u4EFB\u52A1\uFF0C\u70B9\u300C\uFF0B \u65B0\u5EFA\u300D\u521B\u5EFA\uFF09</div></div>';return}let s=z.map(o=>`<tr data-id="${r(o.id)}">
			<td>${Wt(o)}</td>
			<td>${kt(o)}</td>
			<td style="max-width:340px"><div style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${r(o.title)}</div></td>
			<td>${o.repo?`<span class="dsh-tb-pill" style="color:#79c0ff">${r(N(o.repo))}</span>`:""}</td>
			...
			<td>${o.review!=="none"?`<span class="dsh-tb-pill" style="color:${o.review==="approved"?"#3fb950":o.review==="rejected"?"#f85149":"#d29922"}">${et[o.review]}</span>`:""}</td>
			<td>${o.test!=="none"?`<span class="dsh-tb-pill" style="color:${o.test==="passed"?"#3fb950":o.test==="failed"?"#f85149":"#d29922"}">${st[o.test]}</span>`:""}</td>
			<td style="white-space:nowrap">${R(o.updatedAt)}</td>
			...
			<table class="dsh-tb-table">
		...
		</div>`,y("tr[data-id]",t).forEach(o=>o.addEventListener("click",()=>B(o.dataset.id)))}function Z(){y(".dsh-tb-tab").forEach(e=>{e.dataset.tbView===O?e.classList.add("dsh-tb-tab-on"):e.classList.remove("dsh-tb-tab-on")});let t=a("#dsh-tb-toolbar");t&&(t.style.display=O==="overview"?"none":"flex"),O==="kanban"?ee():O==="list"?se():Zt()}function Ct(){let t=document.createElement("div");t.className="dsh-tb-modal-mask";let e=q.map(c=>`<option value="${r(c.path)}" title="${r(c.path)}">${r(c.path)}</option>`).join(""),s=(c,u)=>{a(c,t)?.classList.add("err"),u&&(a(u,t).hidden=!1)},o=()=>{t.innerHTML=`<div class="dsh-tb-modal" style="width:min(560px,92vw)">
				<h3>\u65B0\u5EFA\u4EFB\u52A1</h3>
			...
			</div>`,a('[data-act="newsess"]',t).addEventListener("click",async()=>{t.remove(),await Tt(c.id)}),a('[data-act="bind"]',t).addEventListener("click",()=>{t.remove(),B(c.id)}),a('[data-act="detail"]',t).addEventListener("click",()=>{t.remove(),B(c.id)}),a('[data-act="close"]',t).addEventListener("click",()=>t.remove())},l=()=>t.remove();t.addEventListener("click",c=>{c.target===t&&l()}),document.body.appendChild(t),o()}function ae(t,e){x("/sessions").then(s=>{let o=document.createElement("div");o.className="dsh-tb-modal-mask",o.innerHTML=`<div class="dsh-tb-modal" style="width:min(600px,92vw)">
				<h3>\u9009\u62E9\u4F1A\u8BDD\u5173\u8054</h3>
				...
				<div class="dsh-tb-actions"><button data-act="cancel">\u53D6\u6D88</button></div>
						...
						</div>`)).join("");l.innerHTML=p||'<div class="dsh-tb-empty">\uFF08\u65E0\u5339\u914D\u4F1A\u8BDD\uFF09</div>',y(".dsh-tb-pick-item",o).forEach(v=>v.addEventListener("click",async()=>{let n=v.dataset.sid;try{let{task:f,injected:E,injectionNote:$}=await x(`/tasks/${t}/sessions`,{method:"POST",body:JSON.stringify({sessionId:n,action:"link"})});o.remove(),await k(),e(f),j(E?"\u2713 \u5DF2\u5173\u8054\u4F1A\u8BDD\uFF08\u5DF2\u6CE8\u5165\u4EFB\u52A1\u4E0A\u4E0B\u6587\uFF09":$||"\u5DF2\u5173\u8054\u4F1A\u8BDD")}catch(f){alert(`\u5173\u8054\u5931\u8D25\uFF1A${f.message}`)}}))};c(""),a("#tb-pick-search",o).addEventListener("input",()=>c(a("#tb-pick-search",o).value)),a('[data-act="cancel"]',o).addEventListener("click",()=>o.remove()),o.addEventListener("click",u=>{u.target===o&&o.remove()}),a("#tb-pick-search",o).focus()}).catch(()=>alert("\u83B7\u53D6\u4F1A\u8BDD\u5217\u8868\u5931\u8D25"))}async function B(t){let{task:e}=await x(`/tasks/${t}`);rt(e)}function rt(t){let e=t.id,s=document.createElement("div");s.className="dsh-tb-modal-mask";let o=M.map(i=>`<option value="${r(i.id)}">${r(i.title||i.id)}${i.repo?` \xB7 ${r(N(i.repo))}`:""}</option>`).join("");s.innerHTML=`<div class="dsh-tb-modal" style="width:min(700px,92vw)">
			<h3>${r(t.title)}</h3>
		...
		</div>`,document.body.appendChild(s);let d=async()=>{let i=a("#tb-d-ctx-body",s);if(i)try{let{context:p}=await x(`/tasks/${e}/context`);i.textContent=p||"\uFF08\u6682\u65E0\u5171\u4EAB\u4E0A\u4E0B\u6587\uFF09"}catch{i.textContent="\uFF08\u83B7\u53D6\u5931\u8D25\uFF09"}},l=async()=>{try{let{context:i}=await x(`/tasks/${e}/context`);await navigator.clipboard.writeText(i||""),j("\u2713 \u5DF2\u590D\u5236\u6700\u65B0\u5171\u4EAB\u4E0A\u4E0B\u6587")}catch{try{let{context:i}=await x(`/tasks/${e}/context`),p=document.createElement("textarea");p.value=i||"",document.body.appendChild(p),p.select(),document.execCommand("copy"),p.remove(),j("\u2713 \u5DF2\u590D\u5236\u6700\u65B0\u5171\u4EAB\u4E0A\u4E0B\u6587")}catch(i){alert(`\u590D\u5236\u5931\u8D25\uFF1A${i.message}`)}}};d(),a("#tb-d-ctx-refresh",s)?.addEventListener("click",d),a("#tb-d-ctx-copy",s)?.addEventListener("click",l);let c=()=>({repo:a("#tb-d-repo",s).value.trim(),feature:a("#tb-d-feature",s).value.trim(),priority:a("#tb-d-priority",s).value,progress:Number(a("#tb-d-progress",s).value),review:a("#tb-d-review",s).value,test:a("#tb-d-test",s).value,labels:a("#tb-d-labels",s).value.split(",").map(i=>i.trim()).filter(Boolean),description:a("#tb-d-desc",s).value}),u=()=>s.remove(),b=i=>{s.remove(),rt(i)},h=async i=>{let{task:p}=await x(`/tasks/${e}`,{method:"PATCH",body:JSON.stringify(i)});return await k(),p};s.addEventListener("click",i=>{i.target===s&&u()}),a('[data-act="cancel"]',s).addEventListener("click",u),a('[data-act="save"]',s).addEventListener("click",async()=>{try{let i=await h(c());b(i),j("\u2713 \u5DF2\u4FDD\u5B58")}catch(i){alert(`\u4FDD\u5B58\u5931\u8D25\uFF1A${i.message}`)}}),a('[data-act="del"]',s).addEventListener("click",async()=>{if(confirm(`\u5220\u9664\u4EFB\u52A1\u300C${t.title}\u300D\uFF1F`))try{await x(`/tasks/${e}`,{method:"DELETE"}),u(),await W()}catch(i){alert(`\u5220\u9664\u5931\u8D25\uFF1A${i.message}`)}}),a('[data-act="copyctx"]',s)?.addEventListener("click",l),a("#tb-d-note",s).addEventListener("keydown",async i=>{if(i.key==="Enter"&&(i.ctrlKey||i.metaKey)){let p=a("#tb-d-note",s).value.trim();if(!p)return;try{let{task:v}=await x(`/tasks/${e}/notes`,{method:"POST",body:JSON.stringify({text:p})});await k(),b(v),j("\u2713 \u8FDB\u5C55\u5DF2\u8BB0\u5F55")}catch(v){alert(`\u8BB0\u5F55\u5931\u8D25\uFF1A${v.message}`)}}}),a("#tb-d-sess-open",s)?.addEventListener("click",()=>{ae(e,i=>{s.remove(),rt(i)})}),y("#tb-d-sessions [data-act]",s).forEach(i=>i.addEventListener("click",async()=>{let p=i.dataset.sid;if(i.dataset.act==="open")Q(p);else{let{task:v}=await x(`/tasks/${e}/sessions`,{method:"POST",body:JSON.stringify({sessionId:p,action:"unlink"})});await k(),b(v),j("\u5DF2\u89E3\u9664\u4F1A\u8BDD")}})),a("#tb-d-sess-new",s)?.addEventListener("click",async()=>{await Tt(e)}),y(".dsh-tb-statusbtn",s).forEach(i=>i.addEventListener("click",async()=>{try{let p=await h({status:i.dataset.status});b(p),j("\u2713 \u72B6\u6001\u5DF2\u66F4\u65B0")}catch(p){alert(`\u72B6\u6001\u6D41\u8F6C\u5931\u8D25\uFF1A${p.message}`)}})),a('[data-act="split"]',s)?.addEventListener("click",()=>{let i=document.createElement("div");i.className="dsh-tb-modal-mask",i.innerHTML=`<div class="dsh-tb-modal">
			...
			</div>`,document.body.appendChild(i);let p=()=>i.remove();a('[data-act="cancel"]',i).addEventListener("click",p),i.addEventListener("click",v=>{v.target===i&&p()}),a('[data-act="do"]',i).addEventListener("click",async()=>{let v=a("#tb-split-titles",i).value.split(`
`).map(n=>n.trim()).filter(Boolean);if(!v.length){a("#tb-split-titles",i).focus();return}try{let{tasks:n}=await x(`/tasks/${e}/split`,{method:"POST",body:JSON.stringify({titles:v})});p(),await W(),j(`\u2713 \u5DF2\u62C6\u5206\u4E3A ${n.length} \u4E2A\u5B50\u4EFB\u52A1`),n?.[0]&&B(n[0].id)}catch(n){alert(`\u62C6\u5206\u5931\u8D25\uFF1A${n.message}`)}}),a("#tb-split-titles",i).focus()})}vt(),gt(),yt(),Bt(),new MutationObserver(()=>{vt(),gt(),yt()}).observe(document.body,{childList:!0,subtree:!0}),document.addEventListener("keydown",t=>{t.ctrlKey&&t.shiftKey&&(t.key==="B"||t.key==="b")&&(t.preventDefault(),X())}),document.addEventListener("click",t=>{if(!F())return;let e=t.target;e instanceof Element&&(e.closest(`[${A}]`)||e.closest('[data-pane="sidebar"], [class*="sidebarCol"]')&&X(!1))});function oe(){let t=ht();if(!t||t.clientWidth===0)return;let e=t.style.width;try{t.style.width=`${t.clientWidth-2}px`,setTimeout(()=>{t.style.width=e||"";try{window.dispatchEvent(new Event("resize"))}catch{}},180)}catch{}}try{localStorage.getItem("dsh-taskboard.unstick")!==null&&(localStorage.removeItem("dsh-taskboard.unstick"),setTimeout(()=>{St()},900),setTimeout(()=>{St()},2e3))}catch{}try{let t=localStorage.getItem("dsh-taskboard.pending-toast");t!==null&&(localStorage.removeItem("dsh-taskboard.pending-toast"),setTimeout(()=>{j(t)},1200))}catch{}let ie=[400,1e3,2e3],zt=()=>{for(let t of ie)setTimeout(oe,t)};document.readyState==="loading"?window.addEventListener("load",zt):zt()})();})();
