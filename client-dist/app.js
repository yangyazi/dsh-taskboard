...
<div id="dsh-tb-view" ${O}="">
  ...
  <div class="dsh-tb-body" id="dsh-tb-body"></div>
				...
				</div>`}).join(""):h='<div class="dsh-tb-empty">\uFF08\u6682\u65E0\u5DE5\u4F5C\u533A\uFF09</div>';let o=l.length?l.map(i=>{let f=g[i.status]||g.todo;return`<div class="dsh-tb-ov-item" data-id="${r(i.id)}">
			...
			<div class="dsh-tb-card-foot">${c.join(" \xB7 ")}</div>
		</div>`}function It(){let t=d("#dsh-tb-body"),e=d("#dsh-tb-count");if(!t)return;t.innerHTML='<div class="dsh-tb-columns"></div>';let s=d(".dsh-tb-columns",t);e&&(e.textContent=`${j.length} \u4E2A\u4EFB\u52A1`);for(let[a,c]of Object.entries(g)){let b=document.createElement("div");b.className="dsh-tb-col";let l=j.filter(p=>p.status===a);b.innerHTML=`<div class="dsh-tb-col-head">${c.label} <b>${l.length}</b></div><div class="dsh-tb-col-body"></div>`;let v=d(".dsh-tb-col-body",b);l.length?l.forEach(p=>v.insertAdjacentHTML("beforeend",Rt(p))):v.innerHTML='<div class="dsh-tb-empty">\u2014</div>',s.appendChild(b)}$(".dsh-tb-card",s).forEach(a=>a.addEventListener("click",()=>Y(a.dataset.id)))}function qt(){let t=d("#dsh-tb-body"),e=d("#dsh-tb-count");if(!t)return;if(e&&(e.textContent=`${j.length} \u4E2A\u4EFB\u52A1`),!j.length){t.innerHTML='<div class="dsh-tb-ov"><div class="dsh-tb-empty">\uFF08\u6682\u65E0\u4EFB\u52A1\uFF0C\u70B9\u300C\uFF0B \u65B0\u5EFA\u300D\u521B\u5EFA\uFF09</div></div>';return}let s=j.map(a=>`<tr data-id="${r(a.id)}">
			<td>${Ot(a)}</td>
			<td>${gt(a)}</td>
			...
			<table class="dsh-tb-table">
			...
			</div>`,d("#tb-f-title",t).addEventListener("input",()=>{d("#tb-f-title",t).classList.remove("err"),d("#tb-hint-title",t).hidden=!0}),d("#tb-f-repo",t).addEventListener("change",()=>{d("#tb-f-repo",t).classList.remove("err"),d("#tb-hint-repo",t).hidden=!0}),d('[data-act="cancel"]',t).addEventListener("click",b),d('[data-act="save"]',t).addEventListener("click",async()=>{let l=d("#tb-f-title",t).value.trim(),v=d("#tb-f-repo",t).value;d("#tb-hint-title",t).hidden=!0,d("#tb-hint-repo",t).hidden=!0,$(".err",t).forEach(h=>h.classList.remove("err"));let p=!0;if(l||(s("#tb-f-title","#tb-hint-title"),p=!1),v||(s("#tb-f-repo","#tb-hint-repo"),p=!1),!!p)try{let{task:h}=await x("/tasks",{method:"POST",body:JSON.stringify({title:l,repo:v,feature:d("#tb-f-feature",t).value.trim(),description:d("#tb-f-desc",t).value})});J(),c(h)}catch(h){alert(`\u521B\u5EFA\u5931\u8D25\uFF1A${h.message}`)}}),d("#tb-f-title",t).focus()},c=l=>{t.innerHTML=`<div class="dsh-tb-modal" style="width:min(560px,92vw)">
				<h3>\u2705 \u4EFB\u52A1\u5DF2\u521B\u5EFA</h3>
				...
				<div class="dsh-tb-actions"><button data-act="cancel">\u53D6\u6D88</button></div>
			</div>`,document.body.appendChild(a);let c=s.sessions||A,b=d("#tb-pick-list",a),l=v=>{let p=String(v||"").trim().toLowerCase(),h=c.filter(u=>!p||String(u.title||"").toLowerCase().includes(p)),o={};for(let u of h){let i=u.repo||"\uFF08\u672A\u6307\u5B9A\uFF09";(o[i]||(o[i]=[])).push(u)}let n=Object.entries(o).sort((u,i)=>i[1].length-u[1].length).map(([u,i])=>(i.sort((f,E)=>(E.updatedAt||0)-(f.updatedAt||0)),`<div class="dsh-tb-pick-group">
						...
						</div>`)).join("");b.innerHTML=n||'<div class="dsh-tb-empty">\uFF08\u65E0\u5339\u914D\u4F1A\u8BDD\uFF09</div>',$(".dsh-tb-pick-item",a).forEach(u=>u.addEventListener("click",async()=>{let i=u.dataset.sid;try{let{task:f,injected:E,injectionNote:y}=await x(`/tasks/${t}/sessions`,{method:"POST",body:JSON.stringify({sessionId:i,action:"link"})});a.remove(),await k(),e(f),M(E?"\u2713 \u5DF2\u5173\u8054\u4F1A\u8BDD\uFF08\u5DF2\u6CE8\u5165\u4EFB\u52A1\u4E0A\u4E0B\u6587\uFF09":y||"\u5DF2\u5173\u8054\u4F1A\u8BDD")}catch(f){alert(`\u5173\u8054\u5931\u8D25\uFF1A${f.message}`)}}))};l(""),d("#tb-pick-search",a).addEventListener("input",()=>l(d("#tb-pick-search",a).value)),d('[data-act="cancel"]',a).addEventListener("click",()=>a.remove()),a.addEventListener("click",v=>{v.target===a&&a.remove()}),d("#tb-pick-search",a).focus()}).catch(()=>alert("\u83B7\u53D6\u4F1A\u8BDD\u5217\u8868\u5931\u8D25"))}async function Y(t){let{task:e}=await x(`/tasks/${t}`);ot(e)}function ot(t){let e=t.id,s=document.createElement("div");s.className="dsh-tb-modal-mask";let a=A.map(o=>`<option value="${r(o.id)}">${r(o.title||o.id)}${o.repo?` \xB7 ${r(H(o.repo))}`:""}</option>`).join("");s.innerHTML=`<div class="dsh-tb-modal" style="width:min(700px,92vw)">
			<h3>${r(t.title)}</h3>
		...
		</div>`,document.body.appendChild(s);let c=async()=>{let o=d("#tb-d-ctx-body",s);if(o)try{let{context:n}=await x(`/tasks/${e}/context`);o.textContent=n||"\uFF08\u6682\u65E0\u5171\u4EAB\u4E0A\u4E0B\u6587\uFF09"}catch{o.textContent="\uFF08\u83B7\u53D6\u5931\u8D25\uFF09"}},b=async()=>{try{let{context:o}=await x(`/tasks/${e}/context`);await navigator.clipboard.writeText(o||""),M("\u2713 \u5DF2\u590D\u5236\u6700\u65B0\u5171\u4EAB\u4E0A\u4E0B\u6587")}catch{try{let{context:o}=await x(`/tasks/${e}/context`),n=document.createElement("textarea");n.value=o||"",document.body.appendChild(n),n.select(),document.execCommand("copy"),n.remove(),M("\u2713 \u5DF2\u590D\u5236\u6700\u65B0\u5171\u4EAB\u4E0A\u4E0B\u6587")}catch(o){alert(`\u590D\u5236\u5931\u8D25\uFF1A${o.message}`)}}};c(),d("#tb-d-ctx-refresh",s)?.addEventListener("click",c),d("#tb-d-ctx-copy",s)?.addEventListener("click",b);let l=()=>({repo:d("#tb-d-repo",s).value.trim(),feature:d("#tb-d-feature",s).value.trim(),priority:d("#tb-d-priority",s).value,progress:Number(d("#tb-d-progress",s).value),review:d("#tb-d-review",s).value,test:d("#tb-d-test",s).value,labels:d("#tb-d-labels",s).value.split(",").map(o=>o.trim()).filter(Boolean),description:d("#tb-d-desc",s).value}),v=()=>s.remove(),p=o=>{s.remove(),ot(o)},h=async o=>{let{task:n}=await x(`/tasks/${e}`,{method:"PATCH",body:JSON.stringify(o)});return await k(),n};s.addEventListener("click",o=>{o.target===s&&v()}),d('[data-act="cancel"]',s).addEventListener("click",v),d('[data-act="save"]',s).addEventListener("click",async()=>{try{let o=await h(l());p(o),M("\u2713 \u5DF2\u4FDD\u5B58")}catch(o){alert(`\u4FDD\u5B58\u5931\u8D25\uFF1A${o.message}`)}}),d('[data-act="del"]',s).addEventListener("click",async()=>{if(confirm(`\u5220\u9664\u4EFB\u52A1\u300C${t.title}\u300D\uFF1F`))try{await x(`/tasks/${e}`,{method:"DELETE"}),v(),await J()}catch(o){alert(`\u5220\u9664\u5931\u8D25\uFF1A${o.message}`)}}),d('[data-act="copyctx"]',s)?.addEventListener("click",b),d("#tb-d-note",s).addEventListener("keydown",async o=>{if(o.key==="Enter"&&(o.ctrlKey||o.metaKey)){let n=d("#tb-d-note",s).value.trim();if(!n)return;try{let{task:u}=await x(`/tasks/${e}/notes`,{method:"POST",body:JSON.stringify({text:n})});await k(),p(u),M("\u2713 \u8FDB\u5C55\u5DF2\u8BB0\u5F55")}catch(u){alert(`\u8BB0\u5F55\u5931\u8D25\uFF1A${u.message}`)}}}),d("#tb-d-sess-open",s)?.addEventListener("click",()=>{Bt(e,o=>{s.remove(),ot(o)})}),$("#tb-d-sessions [data-act]",s).forEach(o=>o.addEventListener("click",async()=>{let n=o.dataset.sid;if(o.dataset.act==="open")W(n);else{let{task:u}=await x(`/tasks/${e}/sessions`,{method:"POST",body:JSON.stringify({sessionId:n,action:"unlink"})});await k(),p(u),M("\u5DF2\u89E3\u9664\u4F1A\u8BDD")}})),d("#tb-d-sess-new",s)?.addEventListener("click",async()=>{try{let{sessionId:o}=await x(`/tasks/${e}/session`,{method:"POST",body:"{}"});W(o)}catch(o){alert(`\u65B0\u5EFA\u4F1A\u8BDD\u5931\u8D25\uFF1A${o.message}`)}}),$(".dsh-tb-statusbtn",s).forEach(o=>o.addEventListener("click",async()=>{try{let n=await h({status:o.dataset.status});p(n),M("\u2713 \u72B6\u6001\u5DF2\u66F4\u65B0")}catch(n){alert(`\u72B6\u6001\u6D41\u8F6C\u5931\u8D25\uFF1A${n.message}`)}})),d('[data-act="split"]',s)?.addEventListener("click",()=>{let o=document.createElement("div");o.className="dsh-tb-modal-mask",o.innerHTML=`<div class="dsh-tb-modal">
