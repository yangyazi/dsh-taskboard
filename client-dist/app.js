...
<div id="dsh-tb-view" ${_}="">
  ...
  <div class="dsh-tb-body" id="dsh-tb-body"></div>
				...
				<div class="dsh-recent-body" id="dsh-recent-body"></div>`,L=o("#dsh-recent-body",w),o("#dsh-recent-head",w).addEventListener("click",()=>{B=!B,w&&w.classList.toggle("collapsed",B),L&&(L.style.display=B?"none":"")}),o("#dsh-recent-refresh",w).addEventListener("click",l=>{l.stopPropagation(),G()}),at(A),G()),w.parentElement===t))return;let e=m??lt(t);t.insertBefore(w,e?.nextElementSibling??null),L&&(L.style.maxHeight="46vh",L.style.overflowY="auto"),(()=>{L&&(L.style.display=B?"none":""),w&&w.classList.toggle("collapsed",B)})(),(()=>{w&&(bt(t)?w.style.display="none":w.style.display="")})()}function _t(){ut===null&&(ut=setInterval(()=>{document.hidden||G()},6e4))}function It(){o("#dsh-tb-new")?.addEventListener("click",mt),o("#dsh-tb-refresh")?.addEventListener("click",()=>V()),o("#dsh-tb-search")?.addEventListener("input",()=>{T.q=o("#dsh-tb-search").value.trim(),k()}),o("#dsh-tb-repo-filter")?.addEventListener("change",()=>{T.repo=o("#dsh-tb-repo-filter").value,k()}),o("#dsh-tb-priority-filter")?.addEventListener("change",()=>{T.priority=o("#dsh-tb-priority-filter").value,k()}),o("#dsh-tb-status-filter")?.addEventListener("change",()=>{T.status=o("#dsh-tb-status-filter").value,k()}),o("#dsh-tb-label-filter")?.addEventListener("change",()=>{T.label=o("#dsh-tb-label-filter").value,k()}),y(".dsh-tb-tab").forEach(t=>t.addEventListener("click",()=>{H=t.dataset.tbView,Q()}))}async function k(){let t=new URLSearchParams;for(let s of["q","repo","priority","status","label"])T[s]&&t.set(s,T[s]);j=(await x(`/tasks?${t}`)).tasks||[],Q()}async function Ht(){try{let[t,e]=await Promise.all([x("/sessions"),x("/workspaces")]);A=t.sessions||[],q=e.workspaces||[];let s=o("#dsh-tb-repo-filter");if(s){let p=s.value;s.innerHTML='<option value="">\u5168\u90E8\u4ED3\u5E93</option>'+q.map(h=>`<option value="${n(h.path)}" title="${n(h.path)}">${n(I(h.path))}</option>`).join(""),p&&[...s.options].some(h=>h.value===p)&&(s.value=p)}let d=await x("/tasks").catch(()=>({tasks:[]})),l=new Set;for(let p of d.tasks||[])for(let h of p.labels||[])l.add(h);st=[...l].sort();let r=o("#dsh-tb-label-filter");if(r){let p=r.value;r.innerHTML='<option value="">\u5168\u90E8\u6807\u7B7E</option>'+st.map(h=>`<option value="${n(h)}">${n(h)}</option>`).join(""),p&&[...r.options].some(h=>h.value===p)&&(r.value=p)}let b=o("#dsh-tb-priority-filter");b&&!b.options.length&&(b.innerHTML='<option value="">\u5168\u90E8\u4F18\u5148\u7EA7</option>'+Object.entries(R).map(([p,h])=>`<option value="${p}">${h.label}</option>`).join(""));let u=o("#dsh-tb-status-filter");u&&!u.options.length&&(u.innerHTML='<option value="">\u5168\u90E8\u72B6\u6001</option>'+Object.entries(g).map(([p,h])=>`<option value="${p}">${h.label}</option>`).join(""))}catch{}}async function Nt(){for(let t of q)if(!(!t.path||U[t.path]!==void 0))try{let e=await fetch(`/ide/api/git?op=status&path=${encodeURIComponent(t.path)}`).then(s=>s.json());U[t.path]=e.git?{branch:e.branch?.name||"(detached)",dirty:(e.files||[]).length}:null}catch{U[t.path]=null}}function V(){return Ht().then(()=>Nt()).then(k).catch(k)}function gt(t){return(t.labels||[]).map(e=>`<span class="dsh-tb-pill" style="color:${Lt(e)}">${n(e)}</span>`).join("")}function Rt(t){let e=g[t.status]||g.todo;return`<span class="dsh-tb-pill" style="color:${e.color}">${e.label}</span>`}function wt(t){let e=R[t.priority]||R.medium;return`<span class="dsh-tb-pill" style="color:${e.color}">${e.label}</span>`}function Pt(t){let e=(s,d,l)=>`<button type="button" class="dsh-tb-statusbtn ${l||""}" data-status="${s}">${d}</button>`;switch(t.status){case"todo":return`${e("in_progress","\u25B6 \u5F00\u59CB\u6267\u884C")} ${e("blocked","\u26D4 \u963B\u585E","ghost")}`;case"in_progress":return`${e("in_review","\u63D0\u4EA4\u8BC4\u5BA1")} ${e("blocked","\u26D4 \u963B\u585E","ghost")}`;case"in_review":return`${e("done","\u2705 \u786E\u8BA4\u5B8C\u6210","primary")} ${e("in_progress","\u21A9 \u9000\u56DE\u4FEE\u6539")}`;case"done":return`${e("in_progress","\u21A9 \u91CD\u65B0\u6253\u5F00","ghost")}`;case"blocked":return`${e("in_progress","\u25B6 \u6062\u590D\u8FDB\u884C")}`;default:return""}}function qt(){let t=document.querySelectorAll('[class*="sessionRow"], [class*="root"]');for(let e of t){let s=null;for(let l of Object.keys(e))if(l.startsWith("__reactFiber$")){s=l;break}if(s===null)continue;let d=e[s];for(let l=0;l<30&&d!==null;l++){let r=d.memoizedProps;if(r!==null&&typeof r=="object"&&typeof r.open=="function"&&typeof r.startSession=="function")return r.open;d=d.return}}return null}function K(t){Ot(t);try{let e=qt();if(typeof e=="function"){try{localStorage.setItem("dsh.sessions.current",JSON.stringify({sessionId:t}))}catch{}e(t),X()&&F(!1),y(".dsh-tb-modal-mask").forEach(s=>s.remove()),G();return}}catch{}try{localStorage.setItem("dsh.sessions.current",JSON.stringify({sessionId:t}))}catch{}location.reload()}function M(t){let e=document.createElement("div");e.className="dsh-tb-toast",e.textContent=t,document.body.appendChild(e),setTimeout(()=>e.remove(),1600)}function Bt(){var v;let t={};for(let i of N)t[i]=0;let e={};for(let i of j){t[i.status]!==void 0&&t[i.status]++;let f=i.repo||"\uFF08\u672A\u6307\u5B9A\uFF09";(e[f]||(e[f]=[])).push(i)}let s=j.length,d=t.done||0,l=s?Math.round(d/s*100):0,r={};for(let i of A)i.repo&&(r[v=i.repo]||(r[v]=[])).push(i);let b=[...j].sort((i,f)=>f.updatedAt-i.updatedAt).slice(0,10),p='<div class="dsh-tb-ov-stat dsh-tb-ov-add" id="dsh-tb-ov-new" title="\u65B0\u5EFA\u4EFB\u52A1"><div class="dsh-tb-ov-addbtn">\uFF0B \u65B0\u5EFA\u4EFB\u52A1</div></div>'+[["\u5168\u90E8\u4EFB\u52A1",s,"#e6edf3",""],...N.map(i=>[g[i].label,t[i]||0,g[i].color,i]),["\u5B8C\u6210\u7387",`${l}%`,"#3fb950",null]].map(([i,f,E,$])=>{let dt=i==="\u5B8C\u6210\u7387"?l:s?Math.round(f/s*100):0,Z=$!==null?" dsh-tb-ov-stat-click":"",it=$===""?"\u67E5\u770B\u5168\u90E8\u4EFB\u52A1":$?`\u67E5\u770B\u300C${i}\u300D\u7684\u4EFB\u52A1`:"";return`<div class="dsh-tb-ov-stat${Z}" ${$!==null?`data-status="${$}"`:""} title="${it}"><div class="n" style="color:${E}">${f}</div><div class="l">${i}</div><div class="mini"><i style="width:${dt}%;background:${E}"></i></div></div>`}).join(""),h;q.length?h=q.map(i=>{let f=e[i.path]||e[i.title]||[],E=N.map(S=>f.filter(O=>O.status===S).length),$=f.length,dt=E[N.indexOf("done")]||0,Z=$?Math.round(dt/$*100):0,it=$?N.map((S,O)=>E[O]?`<i style="width:${Math.round(E[O]/$*100)}%;background:${g[S].color}" title="${g[S].label} ${E[O]}"></i>`:"").join(""):"",Ut=N.map((S,O)=>E[O]?`<span class="st"><i class="dot" style="background:${g[S].color}"></i>${g[S].label} <b>${E[O]}</b></span>`:"").join(""),Xt=(r[i.path]||r[i.title]||[]).length,W=U[i.path],Ft=W?`<span class="branch">\u2387 ${n(W.branch)}</span>${W.dirty?`<span class="branch dirty" title="${W.dirty} \u4E2A\u672A\u63D0\u4EA4\u6587\u4EF6">\u25CF${W.dirty}</span>`:""}`:"",$t=(r[i.path]||r[i.title]||[]).slice(0,3).map(S=>`<div class="s" data-sid="${n(S.id)}" title="\u6253\u5F00\u4F1A\u8BDD ${n(S.id)}">\u25B8 ${n(S.title)}</div>`).join("");return`<div class="dsh-tb-ov-ws" data-repo="${n(i.path)}">
				...
				${i.repo?`<span class="r">${n(I(i.repo))}</span>`:""}
		...
		</div>`,o("#dsh-tb-ov-new")?.addEventListener("click",mt),y(".dsh-tb-ov-stat[data-status]").forEach(i=>i.addEventListener("click",()=>{T.status=i.dataset.status,H="list",Q(),k()})),y(".dsh-tb-ov-ws").forEach(i=>i.addEventListener("click",()=>{T.repo=i.dataset.repo;let f=o("#dsh-tb-repo-filter");f&&(f.value=T.repo),H="kanban",Q(),k()})),y(".dsh-tb-ov-ws .s").forEach(i=>i.addEventListener("click",f=>{f.stopPropagation(),K(i.dataset.sid)})),y(".dsh-tb-ov-item").forEach(i=>i.addEventListener("click",()=>{i.dataset.id&&D(i.dataset.id)})))}function Dt(t){let e=g[t.status]||g.todo,s=R[t.priority]||R.medium,d=[];d.push(wt(t)),t.status==="in_review"&&d.push('<span class="dsh-tb-pill" style="color:#bc8cff;font-weight:600">\u25C9 \u5F85 review</span>'),t.repo&&d.push(`<span class="dsh-tb-pill" style="color:#79c0ff">${n(I(t.repo))}</span>`),t.feature&&d.push(`<span class="dsh-tb-pill" style="color:#d2a8ff">${n(t.feature)}</span>`),d.push(gt(t)),t.review&&t.review!=="none"&&d.push(`<span class="dsh-tb-pill" style="color:${t.review==="approved"?"#3fb950":t.review==="rejected"?"#f85149":"#d29922"}">${tt[t.review]}</span>`),t.test&&t.test!=="none"&&d.push(`<span class="dsh-tb-pill" style="color:${t.test==="passed"?"#3fb950":t.test==="failed"?"#f85149":"#d29922"}">${et[t.test]}</span>`);let l=[];return t.sessionIds?.length&&l.push(`<span>\u4F1A\u8BDD ${t.sessionIds.length}</span>`),t.notes?.length&&l.push(`<span>\u8BC4\u8BBA ${t.notes.length}</span>`),l.push(`<span>${P(t.updatedAt)}</span>`),`<div class="dsh-tb-card" data-id="${n(t.id)}" style="border-left:3px solid ${e.color}">
			...
			<div class="dsh-tb-card-foot">${l.join(" \xB7 ")}</div>
		</div>`}function Yt(){let t=o("#dsh-tb-body"),e=o("#dsh-tb-count");if(!t)return;t.innerHTML='<div class="dsh-tb-columns"></div>';let s=o(".dsh-tb-columns",t);e&&(e.textContent=`${j.length} \u4E2A\u4EFB\u52A1`);for(let[d,l]of Object.entries(g)){let r=document.createElement("div");r.className="dsh-tb-col";let b=j.filter(p=>p.status===d);r.innerHTML=`<div class="dsh-tb-col-head">${l.label} <b>${b.length}</b></div><div class="dsh-tb-col-body"></div>`;let u=o(".dsh-tb-col-body",r);b.length?b.forEach(p=>u.insertAdjacentHTML("beforeend",Dt(p))):u.innerHTML='<div class="dsh-tb-empty">\u2014</div>',s.appendChild(r)}y(".dsh-tb-card",s).forEach(d=>d.addEventListener("click",()=>D(d.dataset.id)))}function Jt(){let t=o("#dsh-tb-body"),e=o("#dsh-tb-count");if(!t)return;if(e&&(e.textContent=`${j.length} \u4E2A\u4EFB\u52A1`),!j.length){t.innerHTML='<div class="dsh-tb-ov"><div class="dsh-tb-empty">\uFF08\u6682\u65E0\u4EFB\u52A1\uFF0C\u70B9\u300C\uFF0B \u65B0\u5EFA\u300D\u521B\u5EFA\uFF09</div></div>';return}let s=j.map(d=>`<tr data-id="${n(d.id)}">
			<td>${Rt(d)}</td>
			<td>${wt(d)}</td>
			<td style="max-width:340px"><div style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${n(d.title)}</div></td>
			<td>${d.repo?`<span class="dsh-tb-pill" style="color:#79c0ff">${n(I(d.repo))}</span>`:""}</td>
			...
			</div>`,document.body.appendChild(d);let l=s.sessions||A,r=o("#tb-pick-list",d),b=u=>{let p=String(u||"").trim().toLowerCase(),h=l.filter(v=>!p||String(v.title||"").toLowerCase().includes(p)),a={};for(let v of h){let i=v.repo||"\uFF08\u672A\u6307\u5B9A\uFF09";(a[i]||(a[i]=[])).push(v)}let c=Object.entries(a).sort((v,i)=>i[1].length-v[1].length).map(([v,i])=>(i.sort((f,E)=>(E.updatedAt||0)-(f.updatedAt||0)),`<div class="dsh-tb-pick-group">
							<div class="dsh-tb-pick-grouphead">${n(I(v))} <b>${i.length}</b></div>
			...
			<div class="dsh-tb-row">
				...
				<div class="dsh-tb-field"><label>\u4F18\u5148\u7EA7</label><select id="tb-d-priority">${Object.entries(R).map(([a,c])=>`<option value="${a}" ${a===t.priority?"selected":""}>${c.label}</option>`).join("")}</select></div>
		...
		</div>`,document.body.appendChild(s);let l=async()=>{let a=o("#tb-d-ctx-body",s);if(a)try{let{context:c}=await x(`/tasks/${e}/context`);a.textContent=c||"\uFF08\u6682\u65E0\u5171\u4EAB\u4E0A\u4E0B\u6587\uFF09"}catch{a.textContent="\uFF08\u83B7\u53D6\u5931\u8D25\uFF09"}},r=async()=>{try{let{context:a}=await x(`/tasks/${e}/context`);await navigator.clipboard.writeText(a||""),M("\u2713 \u5DF2\u590D\u5236\u6700\u65B0\u5171\u4EAB\u4E0A\u4E0B\u6587")}catch{try{let{context:a}=await x(`/tasks/${e}/context`),c=document.createElement("textarea");c.value=a||"",document.body.appendChild(c),c.select(),document.execCommand("copy"),c.remove(),M("\u2713 \u5DF2\u590D\u5236\u6700\u65B0\u5171\u4EAB\u4E0A\u4E0B\u6587")}catch(a){alert(`\u590D\u5236\u5931\u8D25\uFF1A${a.message}`)}}};l(),o("#tb-d-ctx-refresh",s)?.addEventListener("click",l),o("#tb-d-ctx-copy",s)?.addEventListener("click",r);let b=()=>({repo:o("#tb-d-repo",s).value.trim(),feature:o("#tb-d-feature",s).value.trim(),priority:o("#tb-d-priority",s).value,progress:Number(o("#tb-d-progress",s).value),review:o("#tb-d-review",s).value,test:o("#tb-d-test",s).value,labels:o("#tb-d-labels",s).value.split(",").map(a=>a.trim()).filter(Boolean),description:o("#tb-d-desc",s).value}),u=()=>s.remove(),p=a=>{s.remove(),ot(a)},h=async a=>{let{task:c}=await x(`/tasks/${e}`,{method:"PATCH",body:JSON.stringify(a)});return await k(),c};s.addEventListener("click",a=>{a.target===s&&u()}),o('[data-act="cancel"]',s).addEventListener("click",u),o('[data-act="save"]',s).addEventListener("click",async()=>{try{let a=await h(b());p(a),M("\u2713 \u5DF2\u4FDD\u5B58")}catch(a){alert(`\u4FDD\u5B58\u5931\u8D25\uFF1A${a.message}`)}}),o('[data-act="del"]',s).addEventListener("click",async()=>{if(confirm(`\u5220\u9664\u4EFB\u52A1\u300C${t.title}\u300D\uFF1F`))try{await x(`/tasks/${e}`,{method:"DELETE"}),u(),await V()}catch(a){alert(`\u5220\u9664\u5931\u8D25\uFF1A${a.message}`)}}),o('[data-act="copyctx"]',s)?.addEventListener("click",r),o("#tb-d-note",s).addEventListener("keydown",async a=>{if(a.key==="Enter"&&(a.ctrlKey||a.metaKey)){let c=o("#tb-d-note",s).value.trim();if(!c)return;try{let{task:v}=await x(`/tasks/${e}/notes`,{method:"POST",body:JSON.stringify({text:c})});await k(),p(v),M("\u2713 \u8FDB\u5C55\u5DF2\u8BB0\u5F55")}catch(v){alert(`\u8BB0\u5F55\u5931\u8D25\uFF1A${v.message}`)}}}),o("#tb-d-sess-open",s)?.addEventListener("click",()=>{Vt(e,a=>{s.remove(),ot(a)})}),y("#tb-d-sessions [data-act]",s).forEach(a=>a.addEventListener("click",async()=>{let c=a.dataset.sid;if(a.dataset.act==="open")K(c);else{let{task:v}=await x(`/tasks/${e}/sessions`,{method:"POST",body:JSON.stringify({sessionId:c,action:"unlink"})});await k(),p(v),M("\u5DF2\u89E3\u9664\u4F1A\u8BDD")}})),o("#tb-d-sess-new",s)?.addEventListener("click",async()=>{try{let{sessionId:a}=await x(`/tasks/${e}/session`,{method:"POST",body:"{}"});K(a)}catch(a){alert(`\u65B0\u5EFA\u4F1A\u8BDD\u5931\u8D25\uFF1A${a.message}`)}}),y(".dsh-tb-statusbtn",s).forEach(a=>a.addEventListener("click",async()=>{try{let c=await h({status:a.dataset.status});p(c),M("\u2713 \u72B6\u6001\u5DF2\u66F4\u65B0")}catch(c){alert(`\u72B6\u6001\u6D41\u8F6C\u5931\u8D25\uFF1A${c.message}`)}})),o('[data-act="split"]',s)?.addEventListener("click",()=>{let a=document.createElement("div");a.className="dsh-tb-modal-mask",a.innerHTML=`<div class="dsh-tb-modal">
